...
#include "sabot_sql/planner/tableref/bound_at_clause.hpp"
	...
	if (!table_or_view) {
		...
		if (found_ctes.empty() && ref.schema_name.empty() && CTEExists(ref.table_name)) {
			throw BinderException(
			    ...
			    "\"SCHEMA\" before table name. You can try \"main.%s\" (main is the sabot_sql default schema)",
			    ref.table_name, ref.table_name, ref.table_name);
		}
		// could not find an alternative: bind again to get the error
		// note: this will always throw when using SabotSQL as a catalog, but a second look-up might succeed
