#include "sabot_sql/planner/expression/bound_lambdaref_expression.hpp"
...
hash_t BoundLambdaRefExpression::Hash() const {
	...
	result = CombineHash(result, sabot_sql::Hash<uint64_t>(lambda_idx));
	result = CombineHash(result, sabot_sql::Hash<uint64_t>(binding.column_index));
	result = CombineHash(result, sabot_sql::Hash<uint64_t>(binding.table_index));
	return CombineHash(result, sabot_sql::Hash<uint64_t>(depth));
}
