...
//                         SabotSQL
//
// sabot_sql/catalog/catalog_set.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/transaction/transaction.hpp"
#include "sabot_sql/catalog/catalog_transaction.hpp"
#include "sabot_sql/catalog/similar_catalog_entry.hpp"
#include <functional>
#include <memory>
...
public:
	SABOT_SQL_API explicit CatalogSet(Catalog &catalog, unique_ptr<DefaultGenerator> defaults = nullptr);
	~CatalogSet();

	//! Create an entry in the catalog set. Returns whether or not it was
	//! successful.
	SABOT_SQL_API bool CreateEntry(CatalogTransaction transaction, const string &name, unique_ptr<CatalogEntry> value,
	                            const LogicalDependencyList &dependencies);
	SABOT_SQL_API bool CreateEntry(ClientContext &context, const string &name, unique_ptr<CatalogEntry> value,
	                            const LogicalDependencyList &dependencies);

	SABOT_SQL_API bool AlterEntry(CatalogTransaction transaction, const string &name, AlterInfo &alter_info);

	SABOT_SQL_API bool DropEntry(CatalogTransaction transaction, const string &name, bool cascade,
	                          bool allow_drop_internal = false);
	SABOT_SQL_API bool DropEntry(ClientContext &context, const string &name, bool cascade,
	                          bool allow_drop_internal = false);
	//! Verify that the entry referenced by the dependency is still alive
	SABOT_SQL_API void VerifyExistenceOfDependency(transaction_t commit_id, CatalogEntry &entry);
	//! Verify we can still drop the entry while committing
	SABOT_SQL_API void CommitDrop(transaction_t commit_id, transaction_t start_time, CatalogEntry &entry);

	SABOT_SQL_API DuckCatalog &GetCatalog();
	...
	SABOT_SQL_API EntryLookup GetEntryDetailed(CatalogTransaction transaction, const string &name);
	SABOT_SQL_API optional_ptr<CatalogEntry> GetEntry(CatalogTransaction transaction, const string &name);
	SABOT_SQL_API optional_ptr<CatalogEntry> GetEntry(ClientContext &context, const string &name);
	...
	SABOT_SQL_API void Scan(const std::function<void(CatalogEntry &)> &callback);
	//! Scan the catalog set, invoking the callback method for every entry
	SABOT_SQL_API void ScanWithPrefix(CatalogTransaction transaction, const std::function<void(CatalogEntry &)> &callback,
	                               const string &prefix);
	SABOT_SQL_API void Scan(CatalogTransaction transaction, const std::function<void(CatalogEntry &)> &callback);
	SABOT_SQL_API void ScanWithReturn(CatalogTransaction transaction, const std::function<bool(CatalogEntry &)> &callback);
	SABOT_SQL_API void Scan(ClientContext &context, const std::function<void(CatalogEntry &)> &callback);
	SABOT_SQL_API void ScanWithReturn(ClientContext &context, const std::function<bool(CatalogEntry &)> &callback);
	...
	SABOT_SQL_API bool CreatedByOtherActiveTransaction(CatalogTransaction transaction, transaction_t timestamp);
	SABOT_SQL_API bool CommittedAfterStarting(CatalogTransaction transaction, transaction_t timestamp);
	SABOT_SQL_API bool HasConflict(CatalogTransaction transaction, transaction_t timestamp);
	SABOT_SQL_API bool UseTimestamp(CatalogTransaction transaction, transaction_t timestamp);
