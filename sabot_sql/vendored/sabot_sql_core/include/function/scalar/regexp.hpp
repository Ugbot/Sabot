...
//                         SabotSQL
//
// sabot_sql/function/scalar/regexp.hpp
//
//
//===----------------------------------------------------------------------===//
...
void ParseRegexOptions(const string &options, sabot_sql_re2::RE2::Options &result, bool *global_replace = nullptr);
void ParseRegexOptions(ClientContext &context, Expression &expr, RE2::Options &target, bool *global_replace = nullptr);

inline sabot_sql_re2::StringPiece CreateStringPiece(const string_t &input) {
	return sabot_sql_re2::StringPiece(input.GetData(), input.GetSize());
}

inline string_t Extract(const string_t &input, Vector &result, const RE2 &re, const sabot_sql_re2::StringPiece &rewrite) {
	string extracted;
...
struct RegexpBaseBindData : public FunctionData {
	RegexpBaseBindData();
	RegexpBaseBindData(sabot_sql_re2::RE2::Options options, string constant_string, bool constant_pattern = true);
...
struct RegexpMatchesBindData : public RegexpBaseBindData {
	RegexpMatchesBindData(sabot_sql_re2::RE2::Options options, string constant_string, bool constant_pattern);
	RegexpMatchesBindData(sabot_sql_re2::RE2::Options options, string constant_string, bool constant_pattern,
...
struct RegexpReplaceBindData : public RegexpBaseBindData {
	...
	RegexpReplaceBindData(sabot_sql_re2::RE2::Options options, string constant_string, bool constant_pattern,
	                      bool global_replace);
...
struct RegexpExtractBindData : public RegexpBaseBindData {
	...
	RegexpExtractBindData(sabot_sql_re2::RE2::Options options, string constant_string, bool constant_pattern,
	                      string group_string);

	string group_string;
	sabot_sql_re2::StringPiece rewrite;
...
struct RegexStringPieceArgs {
	...
	void Init(idx_t size) {
		...
		group_buffer = AllocateArray<sabot_sql_re2::StringPiece>(capacity);
	}
...
private:
	void Clear() {
		DeleteArray<sabot_sql_re2::StringPiece>(group_buffer, capacity);
...
public:
	...
	sabot_sql_re2::StringPiece *group_buffer;
};

struct RegexLocalState : public FunctionLocalState {
	explicit RegexLocalState(RegexpBaseBindData &info, bool extract_all = false)
	    : constant_pattern(sabot_sql_re2::StringPiece(info.constant_string.c_str(), info.constant_string.size()),
	                       info.options) {
