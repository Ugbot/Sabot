...
//                         SabotSQL
//
// sabot_sql/function/scalar/variant_utils.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/common/types/variant.hpp"
#include "sabot_sql/common/owning_string_map.hpp"
...
struct VariantUtils {
	SABOT_SQL_API static bool IsNestedType(const UnifiedVariantVectorData &variant, idx_t row, uint32_t value_index);
	SABOT_SQL_API static VariantDecimalData DecodeDecimalData(const UnifiedVariantVectorData &variant, idx_t row,
	                                                       uint32_t value_index);
	SABOT_SQL_API static VariantNestedData DecodeNestedData(const UnifiedVariantVectorData &variant, idx_t row,
	                                                     uint32_t value_index);
	SABOT_SQL_API static string_t DecodeStringData(const UnifiedVariantVectorData &variant, idx_t row,
	                                            uint32_t value_index);
	SABOT_SQL_API static vector<string> GetObjectKeys(const UnifiedVariantVectorData &variant, idx_t row,
	                                               const VariantNestedData &nested_data);
	SABOT_SQL_API static VariantChildDataCollectionResult FindChildValues(const UnifiedVariantVectorData &variant,
	                                                                   const VariantPathComponent &component,
	                                                                   optional_idx row, SelectionVector &res,
	                                                                   VariantNestedData *nested_data, idx_t count);
	SABOT_SQL_API static VariantNestedDataCollectionResult
	...
	SABOT_SQL_API static vector<uint32_t> ValueIsNull(const UnifiedVariantVectorData &variant, const SelectionVector &sel,
	                                               idx_t count, optional_idx row);
	SABOT_SQL_API static Value ConvertVariantToValue(const UnifiedVariantVectorData &variant, idx_t row, idx_t values_idx);
	SABOT_SQL_API static bool Verify(Vector &variant, const SelectionVector &sel_p, idx_t count);
};
