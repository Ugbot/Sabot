...
//                         SabotSQL
//
// sabot_sql/function/table/arrow/arrow_duck_schema.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/function/table/arrow/arrow_type_info.hpp"
...
public:
	explicit ArrowTypeExtensionData(const LogicalType &sabot_sql_type, const LogicalType &internal_type_p,
	                                cast_arrow_duck_t arrow_to_sabot_sql = nullptr,
	                                cast_duck_arrow_t sabot_sql_to_arrow = nullptr)
	    : arrow_to_sabot_sql(arrow_to_sabot_sql), sabot_sql_to_arrow(sabot_sql_to_arrow), sabot_sql_type(sabot_sql_type),
	      internal_type(internal_type_p) {
	}

	explicit ArrowTypeExtensionData(const LogicalType &sabot_sql_type)
	    : sabot_sql_type(sabot_sql_type), internal_type(sabot_sql_type) {
	}

	//! (Optional) Callback to function that converts an Arrow Array to a SabotSQL Vector
	cast_arrow_duck_t arrow_to_sabot_sql = nullptr;
	//! (Optional) Callback to function that converts a SabotSQL Vector to an Arrow Array
	cast_duck_arrow_t sabot_sql_to_arrow = nullptr;

	LogicalType GetInternalType() const;

	//! This function returns possible extension types to given SabotSQL types
	static unordered_map<idx_t, const shared_ptr<ArrowTypeExtensionData>>
	GetExtensionTypes(ClientContext &context, const vector<LogicalType> &sabot_sql_types);
	LogicalType GetSabotSQLType() const;

private:
	//! Original SabotSQL Type (e.g., UHUGEINT)
	LogicalType sabot_sql_type;
	//! Internal type is a type that refers to the produced sabot_sql vector when consuming the arrow format
	LogicalType internal_type;
};

class ArrowType {
public:
	//! From a SabotSQL type
