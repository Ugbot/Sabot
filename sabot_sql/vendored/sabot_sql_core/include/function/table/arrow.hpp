...
//                         SabotSQL
//
// sabot_sql/function/table/arrow.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	// Hold ownership over the Arrow Arrays owned by SabotSQL to allow for zero-copy
...
struct ArrowToSabotSQLConversion {
	...
	static void ColumnArrowToSabotSQLRunEndEncoded(Vector &vector, const ArrowArray &array, idx_t chunk_offset,
	...
	static void ColumnArrowToSabotSQL(Vector &vector, ArrowArray &array, idx_t chunk_offset,
	...
	static void ColumnArrowToSabotSQLDictionary(Vector &vector, ArrowArray &array, idx_t chunk_offset,
	                                          ArrowArrayScanState &array_state, idx_t size, const ArrowType &arrow_type,
...
public:
	...
	static unique_ptr<FunctionData> ArrowScanBindDumb(ClientContext &context, TableFunctionBindInput &input,
	                                                  vector<LogicalType> &return_types, vector<string> &names);
	//! Actual conversion from Arrow to SabotSQL
	static void ArrowToSabotSQL(ArrowScanLocalState &scan_state, const arrow_column_map_t &arrow_convert_data,
