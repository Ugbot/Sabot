...
//                         SabotSQL
//
// sabot_sql/function/cast/default_casts.hpp
//
//
//===----------------------------------------------------------------------===//
...
struct BindCastInfo {
	SABOT_SQL_API virtual ~BindCastInfo();
...
struct BoundCastData {
	SABOT_SQL_API virtual ~BoundCastData();

	SABOT_SQL_API virtual unique_ptr<BoundCastData> Copy() const = 0;
...
struct BindCastInput {
	SABOT_SQL_API BindCastInput(CastFunctionSet &function_set, optional_ptr<BindCastInfo> info,
	                         optional_ptr<ClientContext> context);
...
public:
	SABOT_SQL_API BoundCastInfo GetCastFunction(const LogicalType &source, const LogicalType &target);
};

struct DefaultCasts {
	SABOT_SQL_API static BoundCastInfo GetDefaultCastFunction(BindCastInput &input, const LogicalType &source,
	                                                       const LogicalType &target);

	SABOT_SQL_API static bool NopCast(Vector &source, Vector &result, idx_t count, CastParameters &parameters);
	SABOT_SQL_API static bool TryVectorNullCast(Vector &source, Vector &result, idx_t count, CastParameters &parameters);
	SABOT_SQL_API static bool ReinterpretCast(Vector &source, Vector &result, idx_t count, CastParameters &parameters);
