...
struct LogicalType {
	SABOT_SQL_API LogicalType();
	SABOT_SQL_API LogicalType(LogicalTypeId id); // NOLINT: Allow implicit conversion from `LogicalTypeId`
	SABOT_SQL_API LogicalType(LogicalTypeId id, shared_ptr<ExtraTypeInfo> type_info);
	SABOT_SQL_API LogicalType(const LogicalType &other);
	SABOT_SQL_API LogicalType(LogicalType &&other) noexcept;
	...
	SABOT_SQL_API bool operator==(const LogicalType &rhs) const;
	inline bool operator!=(const LogicalType &rhs) const {
		return !(*this == rhs);
	}

	SABOT_SQL_API void Serialize(Serializer &serializer) const;
	SABOT_SQL_API static LogicalType Deserialize(Deserializer &deserializer);
	...
	SABOT_SQL_API bool IsIntegral() const;
	SABOT_SQL_API bool IsFloating() const;
	SABOT_SQL_API bool IsNumeric() const;
	SABOT_SQL_API static bool IsNumeric(LogicalTypeId type);
	SABOT_SQL_API bool IsTemporal() const;
	SABOT_SQL_API hash_t Hash() const;
	SABOT_SQL_API void SetAlias(string alias);
	SABOT_SQL_API bool HasAlias() const;
	SABOT_SQL_API string GetAlias() const;

	SABOT_SQL_API bool HasExtensionInfo() const;
	SABOT_SQL_API optional_ptr<const ExtensionTypeInfo> GetExtensionInfo() const;
	SABOT_SQL_API optional_ptr<ExtensionTypeInfo> GetExtensionInfo();
	SABOT_SQL_API void SetExtensionInfo(unique_ptr<ExtensionTypeInfo> info);

	//! Returns the maximum logical type when combining the two types - or throws an exception if combining is not possible
	SABOT_SQL_API static LogicalType MaxLogicalType(ClientContext &context, const LogicalType &left, const LogicalType &right);
	SABOT_SQL_API static bool TryGetMaxLogicalType(ClientContext &context, const LogicalType &left, const LogicalType &right, LogicalType &result);
	...
	SABOT_SQL_API static LogicalType ForceMaxLogicalType(const LogicalType &left, const LogicalType &right);
	//! Normalize a type - removing literals
	SABOT_SQL_API static LogicalType NormalizeType(const LogicalType &type);


	//! Gets the decimal properties of a numeric type. Fails if the type is not numeric.
	SABOT_SQL_API bool GetDecimalProperties(uint8_t &width, uint8_t &scale) const;

	SABOT_SQL_API void Verify() const;

	SABOT_SQL_API bool IsSigned() const;
	SABOT_SQL_API bool IsUnsigned() const;

	SABOT_SQL_API bool IsValid() const;
	SABOT_SQL_API bool IsComplete() const;
	SABOT_SQL_API bool IsTemplated() const;
...
public:
	...
	SABOT_SQL_API static LogicalType DECIMAL(uint8_t width, uint8_t scale);                 // NOLINT
	SABOT_SQL_API static LogicalType VARCHAR_COLLATION(string collation);           // NOLINT
	SABOT_SQL_API static LogicalType LIST(const LogicalType &child);                // NOLINT
	SABOT_SQL_API static LogicalType STRUCT(child_list_t<LogicalType> children);    // NOLINT
	SABOT_SQL_API static LogicalType AGGREGATE_STATE(aggregate_state_t state_type); // NOLINT
	SABOT_SQL_API static LogicalType MAP(const LogicalType &child);                 // NOLINT
	SABOT_SQL_API static LogicalType MAP(LogicalType key, LogicalType value);       // NOLINT
	SABOT_SQL_API static LogicalType UNION(child_list_t<LogicalType> members);      // NOLINT
	SABOT_SQL_API static LogicalType ARRAY(const LogicalType &child, optional_idx index);   // NOLINT
	SABOT_SQL_API static LogicalType ENUM(Vector &ordered_data, idx_t size); // NOLINT
	SABOT_SQL_API static LogicalType GEOMETRY(); // NOLINT
	// ANY but with special rules (default is LogicalType::ANY, 5)
	SABOT_SQL_API static LogicalType ANY_PARAMS(LogicalType target, idx_t cast_score = 5); // NOLINT
	SABOT_SQL_API static LogicalType TEMPLATE(const string &name);							// NOLINT
	SABOT_SQL_API static LogicalType VARIANT(); // NOLINT
	//! Integer literal of the specified value
	SABOT_SQL_API static LogicalType INTEGER_LITERAL(const Value &constant);               // NOLINT
	// DEPRECATED - provided for backwards compatibility
	SABOT_SQL_API static LogicalType ENUM(const string &enum_name, Vector &ordered_data, idx_t size); // NOLINT
	SABOT_SQL_API static LogicalType USER(const string &user_type_name);                              // NOLINT
	SABOT_SQL_API static LogicalType USER(const string &user_type_name, const vector<Value> &user_type_mods); // NOLINT
	SABOT_SQL_API static LogicalType USER(string catalog, string schema, string name, vector<Value> user_type_mods); // NOLINT
	//! A list of all NUMERIC types (integral and floating point types)
	SABOT_SQL_API static const vector<LogicalType> Numeric();
	//! A list of all INTEGRAL types
	SABOT_SQL_API static const vector<LogicalType> Integral();
	//! A list of all REAL types
	SABOT_SQL_API static const vector<LogicalType> Real();
	//! A list of ALL SQL types
	SABOT_SQL_API static const vector<LogicalType> AllTypes();
...
public:
	...
	SABOT_SQL_API static LogicalType JSON(); // NOLINT
	SABOT_SQL_API bool IsJSONType() const;
};

struct DecimalType {
	SABOT_SQL_API static uint8_t GetWidth(const LogicalType &type);
	SABOT_SQL_API static uint8_t GetScale(const LogicalType &type);
	SABOT_SQL_API static uint8_t MaxWidth();
};

struct StringType {
	SABOT_SQL_API static string GetCollation(const LogicalType &type);
};

struct ListType {
	SABOT_SQL_API static const LogicalType &GetChildType(const LogicalType &type);
};

struct UserType {
	SABOT_SQL_API static const string &GetCatalog(const LogicalType &type);
	SABOT_SQL_API static const string &GetSchema(const LogicalType &type);
	SABOT_SQL_API static const string &GetTypeName(const LogicalType &type);
	SABOT_SQL_API static const vector<Value> &GetTypeModifiers(const LogicalType &type);
	SABOT_SQL_API static vector<Value> &GetTypeModifiers(LogicalType &type);
};

struct EnumType {
	SABOT_SQL_API static int64_t GetPos(const LogicalType &type, const string_t &key);
	SABOT_SQL_API static const Vector &GetValuesInsertOrder(const LogicalType &type);
	SABOT_SQL_API static idx_t GetSize(const LogicalType &type);
	SABOT_SQL_API static const string GetValue(const Value &val);
	SABOT_SQL_API static PhysicalType GetPhysicalType(const LogicalType &type);
	SABOT_SQL_API static string_t GetString(const LogicalType &type, idx_t pos);
};

struct StructType {
	SABOT_SQL_API static const child_list_t<LogicalType> &GetChildTypes(const LogicalType &type);
	SABOT_SQL_API static const LogicalType &GetChildType(const LogicalType &type, idx_t index);
	SABOT_SQL_API static const string &GetChildName(const LogicalType &type, idx_t index);
	SABOT_SQL_API static idx_t GetChildIndexUnsafe(const LogicalType &type, const string &name);
	SABOT_SQL_API static idx_t GetChildCount(const LogicalType &type);
	SABOT_SQL_API static bool IsUnnamed(const LogicalType &type);
};

struct MapType {
	SABOT_SQL_API static const LogicalType &KeyType(const LogicalType &type);
	SABOT_SQL_API static const LogicalType &ValueType(const LogicalType &type);
};

struct UnionType {
	SABOT_SQL_API static const idx_t MAX_UNION_MEMBERS = 256;
	SABOT_SQL_API static idx_t GetMemberCount(const LogicalType &type);
	SABOT_SQL_API static const LogicalType &GetMemberType(const LogicalType &type, idx_t index);
	SABOT_SQL_API static const string &GetMemberName(const LogicalType &type, idx_t index);
	SABOT_SQL_API static const child_list_t<LogicalType> CopyMemberTypes(const LogicalType &type);
};

struct ArrayType {
	SABOT_SQL_API static const LogicalType &GetChildType(const LogicalType &type);
	SABOT_SQL_API static idx_t GetSize(const LogicalType &type);
	SABOT_SQL_API static bool IsAnySize(const LogicalType &type);
	SABOT_SQL_API static constexpr idx_t MAX_ARRAY_SIZE = 100000; // 100k for now
	//! Recursively replace all ARRAY types to LIST types within the given type
	SABOT_SQL_API static LogicalType ConvertToList(const LogicalType &type);
};

struct AggregateStateType {
	SABOT_SQL_API static const string GetTypeName(const LogicalType &type);
	SABOT_SQL_API static const aggregate_state_t &GetStateType(const LogicalType &type);
};

struct AnyType {
	SABOT_SQL_API static LogicalType GetTargetType(const LogicalType &type);
	SABOT_SQL_API static idx_t GetCastScore(const LogicalType &type);
};

struct IntegerLiteral {
	//! Returns the type that this integer literal "prefers"
	SABOT_SQL_API static LogicalType GetType(const LogicalType &type);
	//! Whether or not the integer literal fits into the target numeric type
	SABOT_SQL_API static bool FitsInType(const LogicalType &type, const LogicalType &target);
};

struct TemplateType {
	// Get the name of the template type
	SABOT_SQL_API static const string &GetName(const LogicalType &type);
};

// **DEPRECATED**: Use EnumUtil directly instead.
SABOT_SQL_API string LogicalTypeIdToString(LogicalTypeId type);

SABOT_SQL_API LogicalTypeId TransformStringToLogicalTypeId(const string &str);

SABOT_SQL_API LogicalType TransformStringToLogicalType(const string &str);

SABOT_SQL_API LogicalType TransformStringToLogicalType(const string &str, ClientContext &context);

//! The PhysicalType used by the row identifiers column
extern const PhysicalType ROW_TYPE;

SABOT_SQL_API string TypeIdToString(PhysicalType type);
SABOT_SQL_API idx_t GetTypeIdSize(PhysicalType type);
SABOT_SQL_API bool TypeIsConstantSize(PhysicalType type);
SABOT_SQL_API bool TypeIsIntegral(PhysicalType type);
SABOT_SQL_API bool TypeIsNumeric(PhysicalType type);
SABOT_SQL_API bool TypeIsInteger(PhysicalType type);
