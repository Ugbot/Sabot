...
private:
	...
	struct StringWithExtraSpace {
		string_t str;
		// INLINE_BYTES instead of INLINE_LENGTH so this isn't 0-length array when building with SABOT_SQL_DEBUG_NO_INLINE
		uint64_t extra_space[string_t::INLINE_BYTES];
	};

public:
	static string_t DecompressValue(void *sabot_sql_fsst_decoder, VectorStringBuffer &str_buffer,
	                                const char *compressed_string, const idx_t compressed_string_len) {
		const auto max_uncompressed_length = compressed_string_len * 8;
		const auto fsst_decoder = static_cast<sabot_sql_fsst_decoder_t *>(sabot_sql_fsst_decoder);
		...
		const auto decompressed_string_size = sabot_sql_fsst_decompress(
	...
	static string_t DecompressInlinedValue(void *sabot_sql_fsst_decoder, const char *compressed_string,
		...
		const auto decompressed_string_size =
		    sabot_sql_fsst_decompress(fsst_decoder, compressed_string_len, compressed_string_ptr,
	...
	static string DecompressValue(void *sabot_sql_fsst_decoder, const char *compressed_string,
