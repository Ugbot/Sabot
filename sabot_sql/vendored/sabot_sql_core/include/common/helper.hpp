...
#define SABOT_SQL_WINDOWS
#elif defined(__unix__) || defined(__unix) || (defined(__APPLE__) && defined(__MACH__))
#define SABOT_SQL_POSIX
#endif
...
#define SABOT_SQL_EXPLICIT_FALLTHROUGH [[clang::fallthrough]]
#elif __has_cpp_attribute(gnu::fallthrough)
#define SABOT_SQL_EXPLICIT_FALLTHROUGH [[gnu::fallthrough]]
#else
#define SABOT_SQL_EXPLICIT_FALLTHROUGH
#endif
...
{
	return shared_ptr<DATA_TYPE>(sabot_sql_base_std::make_shared<DATA_TYPE>(std::forward<ARGS>(args)...));
}
...
#ifdef SABOT_SQL_ENABLE_DEPRECATED_API
template <typename S, typename T, typename... Args>
...
#ifdef SABOT_SQL_DEBUG_MOVE
template<class T>
typename std::remove_reference<T>::type&& move(T&& t) noexcept {
	// the nonsensical sizeof check ensures this is never instantiated
	static_assert(sizeof(T) == 0, "Use std::move instead of unqualified move or sabot_sql::move");
}
#endif

template <class T, class... ARGS>
static sabot_sql::unique_ptr<T> make_unique(ARGS&&... __args) { // NOLINT: mimic std style
...
template <class T, class... ARGS>
static sabot_sql::shared_ptr<T> make_shared(ARGS&&... __args) { // NOLINT: mimic std style
#ifndef SABOT_SQL_ENABLE_DEPRECATED_API
