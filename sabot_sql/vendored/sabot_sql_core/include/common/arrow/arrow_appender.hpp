...
//                         SabotSQL
//
// sabot_sql/common/arrow/arrow_appender.hpp
//
//
//===----------------------------------------------------------------------===//
...
class ArrowAppender {
public:
	SABOT_SQL_API ArrowAppender(vector<LogicalType> types_p, const idx_t initial_capacity, ClientProperties options,
	                         unordered_map<idx_t, const shared_ptr<ArrowTypeExtensionData>> extension_type_cast);
	SABOT_SQL_API ~ArrowAppender();

public:
	//! Append a data chunk to the underlying arrow array
	SABOT_SQL_API void Append(DataChunk &input, idx_t from, idx_t to, idx_t input_size);
	//! Returns the underlying arrow array
	SABOT_SQL_API ArrowArray Finalize();
