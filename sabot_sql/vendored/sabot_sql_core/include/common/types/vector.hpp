...
//                         SabotSQL
//
// sabot_sql/common/types/vector.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/common/bitset.hpp"
#include "sabot_sql/common/common.hpp"
#include "sabot_sql/common/enums/vector_type.hpp"
#include "sabot_sql/common/types/selection_vector.hpp"
#include "sabot_sql/common/types/validity_mask.hpp"
...
struct UnifiedVectorFormat {
	SABOT_SQL_API UnifiedVectorFormat();
	...
	SABOT_SQL_API UnifiedVectorFormat(UnifiedVectorFormat &&other) noexcept;
	SABOT_SQL_API UnifiedVectorFormat &operator=(UnifiedVectorFormat &&) noexcept;
...
#ifdef SABOT_SQL_DEBUG_NO_SAFETY
...
struct UnifiedVariantVector {
	...
	SABOT_SQL_API static const UnifiedVectorFormat &GetKeys(const RecursiveUnifiedVectorFormat &vec);
	//! The 'keys' list entry
	SABOT_SQL_API static const UnifiedVectorFormat &GetKeysEntry(const RecursiveUnifiedVectorFormat &vec);
	//! The 'children' list
	SABOT_SQL_API static const UnifiedVectorFormat &GetChildren(const RecursiveUnifiedVectorFormat &vec);
	//! The 'keys_index' inside the 'children' list
	SABOT_SQL_API static const UnifiedVectorFormat &GetChildrenKeysIndex(const RecursiveUnifiedVectorFormat &vec);
	//! The 'values_index' inside the 'children' list
	SABOT_SQL_API static const UnifiedVectorFormat &GetChildrenValuesIndex(const RecursiveUnifiedVectorFormat &vec);
	//! The 'values' list
	SABOT_SQL_API static const UnifiedVectorFormat &GetValues(const RecursiveUnifiedVectorFormat &vec);
	//! The 'type_id' inside the 'values' list
	SABOT_SQL_API static const UnifiedVectorFormat &GetValuesTypeId(const RecursiveUnifiedVectorFormat &vec);
	//! The 'byte_offset' inside the 'values' list
	SABOT_SQL_API static const UnifiedVectorFormat &GetValuesByteOffset(const RecursiveUnifiedVectorFormat &vec);
	//! The binary blob 'data' encoding the Variant for the row
	SABOT_SQL_API static const UnifiedVectorFormat &GetData(const RecursiveUnifiedVectorFormat &vec);
};
...
public:
	...
	SABOT_SQL_API Vector(Vector &other);
	//! Create a vector that slices another vector
	SABOT_SQL_API explicit Vector(const Vector &other, const SelectionVector &sel, idx_t count);
	//! Create a vector that slices another vector between a pair of offsets
	SABOT_SQL_API explicit Vector(const Vector &other, idx_t offset, idx_t end);
	//! Create a vector of size one holding the passed on value
	SABOT_SQL_API explicit Vector(const Value &value);
	//! Create a vector of size tuple_count (non-standard)
	SABOT_SQL_API explicit Vector(LogicalType type, idx_t capacity = STANDARD_VECTOR_SIZE);
	//! Create an empty standard vector with a type, equivalent to calling Vector(type, true, false)
	SABOT_SQL_API explicit Vector(const VectorCache &cache);
	//! Create a non-owning vector that references the specified data
	SABOT_SQL_API Vector(LogicalType type, data_ptr_t dataptr);
	...
	*/
	SABOT_SQL_API Vector(LogicalType type, bool create_data, bool initialize_to_zero,
	                  idx_t capacity = STANDARD_VECTOR_SIZE);
	// implicit copying of Vectors is not allowed
	Vector(const Vector &) = delete;
	// but moving of vectors is allowed
	SABOT_SQL_API Vector(Vector &&other) noexcept;

public:
	//! Create a vector that references the specified value.
	SABOT_SQL_API void Reference(const Value &value);
	//! Causes this vector to reference the data held by the other vector.
	//! The type of the "other" vector should match the type of this vector
	SABOT_SQL_API void Reference(const Vector &other);
	...
	SABOT_SQL_API void Reinterpret(const Vector &other);

	//! Causes this vector to reference the data held by the other vector, changes the type if required.
	SABOT_SQL_API void ReferenceAndSetType(const Vector &other);
	...
	SABOT_SQL_API void ResetFromCache(const VectorCache &cache);

	//! Creates a reference to a slice of the other vector
	SABOT_SQL_API void Slice(const Vector &other, idx_t offset, idx_t end);
	//! Creates a reference to a slice of the other vector
	SABOT_SQL_API void Slice(const Vector &other, const SelectionVector &sel, idx_t count);
	//! Turns the vector into a dictionary vector with the specified dictionary
	SABOT_SQL_API void Slice(const SelectionVector &sel, idx_t count);
	//! Slice the vector, keeping the result around in a cache or potentially using the cache instead of slicing
	SABOT_SQL_API void Slice(const SelectionVector &sel, idx_t count, SelCache &cache);
	//! Turn this vector into a dictionary vector
	SABOT_SQL_API void Dictionary(idx_t dictionary_size, const SelectionVector &sel, idx_t count);
	//! Creates a reference to a dictionary of the other vector
	SABOT_SQL_API void Dictionary(Vector &dict, idx_t dictionary_size, const SelectionVector &sel, idx_t count);

	//! Creates the data of this vector with the specified type. Any data that
	//! is currently in the vector is destroyed.
	SABOT_SQL_API void Initialize(bool initialize_to_zero = false, idx_t capacity = STANDARD_VECTOR_SIZE);

	//! Converts this Vector to a printable string representation
	SABOT_SQL_API string ToString(idx_t count) const;
	SABOT_SQL_API void Print(idx_t count) const;

	SABOT_SQL_API string ToString() const;
	SABOT_SQL_API void Print() const;

	//! Flatten the vector, removing any compression and turning it into a FLAT_VECTOR
	SABOT_SQL_API void Flatten(idx_t count);
	SABOT_SQL_API void Flatten(const SelectionVector &sel, idx_t count);
	...
	SABOT_SQL_API void ToUnifiedFormat(idx_t count, UnifiedVectorFormat &data);
	...
	SABOT_SQL_API void Sequence(int64_t start, int64_t increment, idx_t count);

	//! Verify that the Vector is in a consistent, not corrupt state. DEBUG
	//! FUNCTION ONLY!
	SABOT_SQL_API void Verify(idx_t count);
	//! Asserts that the CheckMapValidity returns MapInvalidReason::VALID
	SABOT_SQL_API static void VerifyMap(Vector &map, const SelectionVector &sel, idx_t count);
	SABOT_SQL_API static void VerifyUnion(Vector &map, const SelectionVector &sel, idx_t count);
	SABOT_SQL_API static void VerifyVariant(Vector &map, const SelectionVector &sel, idx_t count);
	SABOT_SQL_API static void Verify(Vector &vector, const SelectionVector &sel, idx_t count);
	SABOT_SQL_API void UTFVerify(idx_t count);
	SABOT_SQL_API void UTFVerify(const SelectionVector &sel, idx_t count);

	//! Returns the [index] element of the Vector as a Value.
	SABOT_SQL_API Value GetValue(idx_t index) const;
	//! Sets the [index] element of the Vector to the specified Value.
	SABOT_SQL_API void SetValue(idx_t index, const Value &val);
	...
	//! Resizes the vector.
	SABOT_SQL_API void Resize(idx_t cur_size, idx_t new_size);
	//! Returns a vector of ResizeInfo containing each (nested) vector to resize.
	SABOT_SQL_API void FindResizeInfos(vector<ResizeInfo> &resize_infos, const idx_t multiplier);

	SABOT_SQL_API void Serialize(Serializer &serializer, idx_t count, bool compressed_serialization = true);
	SABOT_SQL_API void Deserialize(Deserializer &deserializer, idx_t count);
	...
	// Setters
	SABOT_SQL_API void SetVectorType(VectorType vector_type);
...
struct ConstantVector {
	...
	SABOT_SQL_API static void SetNull(Vector &vector, bool is_null);
	static inline ValidityMask &Validity(Vector &vector) {
		D_ASSERT(vector.GetVectorType() == VectorType::CONSTANT_VECTOR);
		return vector.validity;
	}
	SABOT_SQL_API static const SelectionVector *ZeroSelectionVector(idx_t count, SelectionVector &owned_sel);
	SABOT_SQL_API static const SelectionVector *ZeroSelectionVector();
	//! Turns "vector" into a constant vector by referencing a value within the source vector
	SABOT_SQL_API static void Reference(Vector &vector, Vector &source, idx_t position, idx_t count);
...
struct FlatVector {
	...
	SABOT_SQL_API static void SetNull(Vector &vector, idx_t idx, bool is_null);
	static inline bool IsNull(const Vector &vector, idx_t idx) {
		D_ASSERT(vector.GetVectorType() == VectorType::FLAT_VECTOR);
		return !vector.validity.RowIsValid(idx);
	}
	SABOT_SQL_API static const SelectionVector *IncrementalSelectionVector();
};
...
struct ListVector {
	...
	SABOT_SQL_API static const Vector &GetEntry(const Vector &vector);
	//! Gets a reference to the underlying child-vector of a list
	SABOT_SQL_API static Vector &GetEntry(Vector &vector);
	//! Gets the total size of the underlying child-vector of a list
	SABOT_SQL_API static idx_t GetListSize(const Vector &vector);
	//! Sets the total size of the underlying child-vector of a list
	SABOT_SQL_API static void SetListSize(Vector &vec, idx_t size);
	//! Gets the total capacity of the underlying child-vector of a list
	SABOT_SQL_API static idx_t GetListCapacity(const Vector &vector);
	//! Sets the total capacity of the underlying child-vector of a list
	SABOT_SQL_API static void Reserve(Vector &vec, idx_t required_capacity);
	SABOT_SQL_API static void Append(Vector &target, const Vector &source, idx_t source_size, idx_t source_offset = 0);
	SABOT_SQL_API static void Append(Vector &target, const Vector &source, const SelectionVector &sel, idx_t source_size,
	                              idx_t source_offset = 0);
	SABOT_SQL_API static void PushBack(Vector &target, const Value &insert);
	//! Returns the child_vector of list starting at offset until offset + count, and its length
	SABOT_SQL_API static idx_t GetConsecutiveChildList(Vector &list, Vector &result, idx_t offset, idx_t count);
	//! Returns information to only copy a section of a list child vector
	SABOT_SQL_API static ConsecutiveChildListInfo GetConsecutiveChildListInfo(Vector &list, idx_t offset, idx_t count);
	//! Slice and flatten a child vector to only contain a consecutive subsection of the child entries
	SABOT_SQL_API static void GetConsecutiveChildSelVector(Vector &list, SelectionVector &sel, idx_t offset, idx_t count);
	//! Share the entry of the other list vector
	SABOT_SQL_API static void ReferenceEntry(Vector &vector, Vector &other);
...
struct StringVector {
	...
	SABOT_SQL_API static string_t AddString(Vector &vector, const char *data, idx_t len);
	...
	SABOT_SQL_API static string_t AddStringOrBlob(Vector &vector, const char *data, idx_t len);
	//! Add a string to the string heap of the vector (auxiliary data)
	SABOT_SQL_API static string_t AddString(Vector &vector, const char *data);
	//! Add a string to the string heap of the vector (auxiliary data)
	SABOT_SQL_API static string_t AddString(Vector &vector, string_t data);
	//! Add a string to the string heap of the vector (auxiliary data)
	SABOT_SQL_API static string_t AddString(Vector &vector, const string &data);
	//! Add a string or a blob to the string heap of the vector (auxiliary data)
	//! This function is the same as ::AddString, except the added data does not need to be valid UTF8
	SABOT_SQL_API static string_t AddStringOrBlob(Vector &vector, string_t data);
	//! Allocates an empty string of the specified size, and returns a writable pointer that can be used to store the
	//! result of an operation
	SABOT_SQL_API static string_t EmptyString(Vector &vector, idx_t len);
	//! Returns a reference to the underlying VectorStringBuffer - throws an error if vector is not of type VARCHAR
	SABOT_SQL_API static VectorStringBuffer &GetStringBuffer(Vector &vector);
	//! Adds a reference to a handle that stores strings of this vector
	SABOT_SQL_API static void AddHandle(Vector &vector, BufferHandle handle);
	//! Adds a reference to an unspecified vector buffer that stores strings of this vector
	SABOT_SQL_API static void AddBuffer(Vector &vector, buffer_ptr<VectorBuffer> buffer);
	//! Add a reference from this vector to the string heap of the provided vector
	SABOT_SQL_API static void AddHeapReference(Vector &vector, Vector &other);
};
...
struct FSSTVector {
	...
	SABOT_SQL_API static string_t AddCompressedString(Vector &vector, string_t data);
	SABOT_SQL_API static string_t AddCompressedString(Vector &vector, const char *data, idx_t len);
	SABOT_SQL_API static void RegisterDecoder(Vector &vector, buffer_ptr<void> &sabot_sql_fsst_decoder,
	                                       const idx_t string_block_limit);
	SABOT_SQL_API static void *GetDecoder(const Vector &vector);
	SABOT_SQL_API static vector<unsigned char> &GetDecompressBuffer(const Vector &vector);
	//! Setting the string count is required to be able to correctly flatten the vector
	SABOT_SQL_API static void SetCount(Vector &vector, idx_t count);
	SABOT_SQL_API static idx_t GetCount(Vector &vector);
};

enum class MapInvalidReason : uint8_t { VALID, NULL_KEY, DUPLICATE_KEY, NOT_ALIGNED, INVALID_PARAMS };

struct MapVector {
	SABOT_SQL_API static const Vector &GetKeys(const Vector &vector);
	SABOT_SQL_API static const Vector &GetValues(const Vector &vector);
	SABOT_SQL_API static Vector &GetKeys(Vector &vector);
	SABOT_SQL_API static Vector &GetValues(Vector &vector);
	SABOT_SQL_API static MapInvalidReason
	CheckMapValidity(Vector &map, idx_t count, const SelectionVector &sel = *FlatVector::IncrementalSelectionVector());
	SABOT_SQL_API static void EvalMapInvalidReason(MapInvalidReason reason);
	SABOT_SQL_API static void MapConversionVerify(Vector &vector, idx_t count);
};

struct StructVector {
	SABOT_SQL_API static const vector<unique_ptr<Vector>> &GetEntries(const Vector &vector);
	SABOT_SQL_API static vector<unique_ptr<Vector>> &GetEntries(Vector &vector);
};
...
struct ArrayVector {
	...
	SABOT_SQL_API static idx_t GetTotalSize(const Vector &vector);
...
struct VariantVector {
	...
	SABOT_SQL_API static Vector &GetKeys(Vector &vec);
	SABOT_SQL_API static Vector &GetKeys(const Vector &vec);
	//! Gets a reference to the 'children' list of a Variant
	SABOT_SQL_API static Vector &GetChildren(Vector &vec);
	SABOT_SQL_API static Vector &GetChildren(const Vector &vec);
	//! Gets a reference to the 'keys_index' inside the 'children' list of a Variant
	SABOT_SQL_API static Vector &GetChildrenKeysIndex(Vector &vec);
	SABOT_SQL_API static Vector &GetChildrenKeysIndex(const Vector &vec);
	//! Gets a reference to the 'values_index' inside the 'children' list of a Variant
	SABOT_SQL_API static Vector &GetChildrenValuesIndex(Vector &vec);
	SABOT_SQL_API static Vector &GetChildrenValuesIndex(const Vector &vec);
	//! Gets a reference to the 'values' list of a Variant
	SABOT_SQL_API static Vector &GetValues(Vector &vec);
	SABOT_SQL_API static Vector &GetValues(const Vector &vec);
	//! Gets a reference to the 'type_id' inside the 'values' list of a Variant
	SABOT_SQL_API static Vector &GetValuesTypeId(Vector &vec);
	SABOT_SQL_API static Vector &GetValuesTypeId(const Vector &vec);
	//! Gets a reference to the 'byte_offset' inside the 'values' list of a Variant
	SABOT_SQL_API static Vector &GetValuesByteOffset(Vector &vec);
	SABOT_SQL_API static Vector &GetValuesByteOffset(const Vector &vec);
	//! Gets a reference to the binary blob 'value', which encodes the data of the row
	SABOT_SQL_API static Vector &GetData(Vector &vec);
	SABOT_SQL_API static Vector &GetData(const Vector &vec);
};
...
struct UnionVector {
	...
	SABOT_SQL_API static const Vector &GetTags(const Vector &v);
	SABOT_SQL_API static Vector &GetTags(Vector &v);
	...
	SABOT_SQL_API static bool TryGetTag(const Vector &vector, idx_t index, union_tag_t &tag);

	//! Get the member vector of a union vector by index
	SABOT_SQL_API static const Vector &GetMember(const Vector &vector, idx_t member_index);
	SABOT_SQL_API static Vector &GetMember(Vector &vector, idx_t member_index);
	...
	SABOT_SQL_API static void SetToMember(Vector &vector, union_tag_t tag, Vector &member_vector, idx_t count,
	                                   bool keep_tags_for_null);

	SABOT_SQL_API static UnionInvalidReason
