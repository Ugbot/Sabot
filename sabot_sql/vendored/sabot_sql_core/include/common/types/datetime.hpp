...
//                         SabotSQL
//
// sabot_sql/common/types/datetime.hpp
//
//
//===----------------------------------------------------------------------===//
...
//! Time
template <>
struct hash<sabot_sql::dtime_t> {
	std::size_t operator()(const sabot_sql::dtime_t &k) const {
		using std::hash;
...
template <>
struct hash<sabot_sql::dtime_tz_t> {
	std::size_t operator()(const sabot_sql::dtime_tz_t &k) const {
		using std::hash;
