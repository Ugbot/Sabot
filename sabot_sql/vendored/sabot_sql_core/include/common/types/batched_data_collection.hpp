...
//                         SabotSQL
//
// sabot_sql/common/types/batched_chunk_collection.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	SABOT_SQL_API BatchedDataCollection(ClientContext &context, vector<LogicalType> types, bool buffer_managed = false);
	SABOT_SQL_API BatchedDataCollection(ClientContext &context, vector<LogicalType> types, batch_map_t batches,
	                                 bool buffer_managed = false);

	//! Appends a datachunk with the given batch index to the batched collection
	SABOT_SQL_API void Append(DataChunk &input, idx_t batch_index);

	//! Merge the other batched chunk collection into this batched collection
	SABOT_SQL_API void Merge(BatchedDataCollection &other);
	...
	SABOT_SQL_API void InitializeScan(BatchedChunkScanState &state);

	//! Scan a chunk from the batched chunk collection, in-order of batch index
	SABOT_SQL_API void Scan(BatchedChunkScanState &state, DataChunk &output);

	//! Fetch a column data collection from the batched data collection - this consumes all of the data stored within
	SABOT_SQL_API unique_ptr<ColumnDataCollection> FetchCollection();

	//! Inspect how many tuples this batched data collection contains
	SABOT_SQL_API idx_t Count() const;

	//! Inspect the types of the collection
	SABOT_SQL_API const vector<LogicalType> &Types() const;

	//! Inspect how many batches this collection contains
	SABOT_SQL_API idx_t BatchCount() const;

	//! Retrieve the batch index of the nth batch in the collection
	SABOT_SQL_API idx_t IndexToBatchIndex(idx_t index) const;

	//! Inspect how big a given batch is
	SABOT_SQL_API idx_t BatchSize(idx_t batch_index) const;
