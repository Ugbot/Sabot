...
//                         SabotSQL
//
// sabot_sql/common/types/validity_mask.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	SABOT_SQL_API explicit ValidityData(idx_t count);
	SABOT_SQL_API ValidityData(const ValidityMask &original, idx_t count);
};
...
public:
	SABOT_SQL_API void Resize(idx_t new_size);
	SABOT_SQL_API idx_t Capacity() const;
	SABOT_SQL_API void SliceInPlace(const ValidityMask &other, idx_t target_offset, idx_t source_offset, idx_t count);
	SABOT_SQL_API void Slice(const ValidityMask &other, idx_t source_offset, idx_t count);
	SABOT_SQL_API void CopySel(const ValidityMask &other, const SelectionVector &sel, idx_t source_offset,
	                        idx_t target_offset, idx_t count);
	SABOT_SQL_API void Combine(const ValidityMask &other, idx_t count);
	SABOT_SQL_API string ToString(idx_t count) const;
	SABOT_SQL_API string ToString() const;

	SABOT_SQL_API static bool IsAligned(idx_t count);
