...
//                         SabotSQL
//
// sabot_sql/common/types/vector_buffer.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/common/types/string_heap.hpp"
...
public:
	void AddDecoder(buffer_ptr<void> &sabot_sql_fsst_decoder_p, const idx_t string_block_limit) {
		sabot_sql_fsst_decoder = sabot_sql_fsst_decoder_p;
		decompress_buffer.resize(string_block_limit + 1);
	}
	void *GetDecoder() {
		return sabot_sql_fsst_decoder.get();
	}
...
private:
	buffer_ptr<void> sabot_sql_fsst_decoder;
