...
//                         SabotSQL
//
// sabot_sql/common/types/interval.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	...
	SABOT_SQL_API static interval_t FromMicro(int64_t micros);
...
namespace std {
template <>
struct hash<sabot_sql::interval_t> {
	size_t operator()(const sabot_sql::interval_t &val) const {
		int64_t months, days, micros;
		val.Normalize(months, days, micros);
		using std::hash;

		return hash<int32_t> {}(sabot_sql::UnsafeNumericCast<int32_t>(days)) ^
		       hash<int32_t> {}(sabot_sql::UnsafeNumericCast<int32_t>(months)) ^ hash<int64_t> {}(micros);
	}
};
} // namespace std
