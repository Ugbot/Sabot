...
//                         SabotSQL
//
// sabot_sql/common/types/date_lookup_cache.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	...
	int64_t ExtractElement(date_t date, ValidityMask &mask, idx_t idx) const {
		if (SABOT_SQL_UNLIKELY(date.days < CACHE_MIN_DATE || date.days >= CACHE_MAX_DATE)) {
			if (SABOT_SQL_UNLIKELY(!Value::IsFinite(date))) {
				mask.SetInvalid(idx);
				return 0;
			}
