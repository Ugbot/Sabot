...
//                         SabotSQL
//
// sabot_sql/common/types/blob.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	...
	SABOT_SQL_API static idx_t GetStringSize(string_t blob);
	...
	SABOT_SQL_API static void ToString(string_t blob, char *output);
	//! Convert a blob object to a string
	SABOT_SQL_API static string ToString(string_t blob);

	//! Returns the blob size of a string -> blob conversion
	SABOT_SQL_API static bool TryGetBlobSize(string_t str, idx_t &result_size, CastParameters &parameters);
	SABOT_SQL_API static idx_t GetBlobSize(string_t str);
	SABOT_SQL_API static idx_t GetBlobSize(string_t str, CastParameters &parameters);
	//! Convert a string to a blob. This function should ONLY be called after calling GetBlobSize, since it does NOT
	//! perform data validation.
	SABOT_SQL_API static void ToBlob(string_t str, data_ptr_t output);
	//! Convert a string object to a blob
	SABOT_SQL_API static string ToBlob(string_t str);
	SABOT_SQL_API static string ToBlob(string_t str, CastParameters &parameters);

	// base 64 conversion functions
	SABOT_SQL_API static string ToBase64(string_t blob);
	//! Returns the string size of a blob -> base64 conversion
	SABOT_SQL_API static idx_t ToBase64Size(string_t blob);
	//! Converts a blob to a base64 string, output should have space for at least ToBase64Size(blob) bytes
	SABOT_SQL_API static void ToBase64(string_t blob, char *output);

	SABOT_SQL_API static string FromBase64(string_t blob);
	//! Returns the string size of a base64 string -> blob conversion
	SABOT_SQL_API static idx_t FromBase64Size(string_t str);
	//! Converts a base64 string to a blob, output should have space for at least FromBase64Size(blob) bytes
	SABOT_SQL_API static void FromBase64(string_t str, data_ptr_t output, idx_t output_size);
};
} // namespace sabot_sql
