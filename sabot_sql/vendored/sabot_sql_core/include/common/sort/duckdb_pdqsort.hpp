...
namespace sabot_sql_pdqsort {

using sabot_sql::data_ptr_t;
using sabot_sql::data_t;
using sabot_sql::FastMemcmp;
using sabot_sql::FastMemcpy;
using sabot_sql::idx_t;
using sabot_sql::make_unsafe_uniq_array_uninitialized;
using sabot_sql::unique_ptr;
using sabot_sql::unsafe_unique_array;
...
struct PDQConstants {
	...
	const sabot_sql::idx_t entry_size;
...
struct PDQIterator {
	...
	inline friend idx_t operator-(const PDQIterator &lhs, const PDQIterator &rhs) {
		D_ASSERT(sabot_sql::NumericCast<idx_t>(*lhs - *rhs) % lhs.entry_size == 0);
		D_ASSERT(*lhs - *rhs >= 0);
		return sabot_sql::NumericCast<idx_t>(*lhs - *rhs) / lhs.entry_size;
	}
...
#endif
	ip = (ip + cacheline_size - 1) & -sabot_sql::UnsafeNumericCast<uintptr_t>(cacheline_size);
