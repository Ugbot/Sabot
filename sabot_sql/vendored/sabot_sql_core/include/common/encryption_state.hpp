...
//                         SabotSQL
//
// sabot_sql/common/encryption_state.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	SABOT_SQL_API explicit EncryptionState(EncryptionTypes::CipherType cipher_p, idx_t key_len);
	SABOT_SQL_API virtual ~EncryptionState();

public:
	SABOT_SQL_API virtual void InitializeEncryption(const_data_ptr_t iv, idx_t iv_len, const_data_ptr_t key, idx_t key_len,
	                                             const_data_ptr_t aad = nullptr, idx_t aad_len = 0);
	SABOT_SQL_API virtual void InitializeDecryption(const_data_ptr_t iv, idx_t iv_len, const_data_ptr_t key, idx_t key_len,
	                                             const_data_ptr_t aad = nullptr, idx_t aad_len = 0);
	SABOT_SQL_API virtual size_t Process(const_data_ptr_t in, idx_t in_len, data_ptr_t out, idx_t out_len);
	SABOT_SQL_API virtual size_t Finalize(data_ptr_t out, idx_t out_len, data_ptr_t tag, idx_t tag_len);
	SABOT_SQL_API virtual void GenerateRandomData(data_ptr_t data, idx_t len);
...
public:
	SABOT_SQL_API explicit EncryptionUtil() {};
