...
//                         SabotSQL
//
// sabot_sql/common/serializer/serializer.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/common/serializer/serialization_traits.hpp"
#include "sabot_sql/common/serializer/serialization_data.hpp"
...
#include "sabot_sql/execution/operator/csv_scanner/csv_option.hpp"
...
protected:
	...
	void WriteValue(const sabot_sql::unordered_set<T, HASH, CMP> &set) {
		auto count = set.size();
		OnListBegin(count);
		for (auto &item : set) {
			WriteValue(item);
	...
	// Serialized the same way as a list/vector
	template <class T, class HASH, class CMP>
	void WriteValue(const sabot_sql::set<T, HASH, CMP> &set) {
		auto count = set.size();
		OnListBegin(count);
		for (auto &item : set) {
			WriteValue(item);
	...
	void WriteValue(const sabot_sql::unordered_map<K, V, HASH, CMP> &map) {
		auto count = map.size();
		OnListBegin(count);
		for (auto &item : map) {
			OnObjectBegin();
	...
	// serialized as a list of pairs
	template <class K, class V, class HASH, class CMP>
	void WriteValue(const sabot_sql::map<K, V, HASH, CMP> &map) {
		auto count = map.size();
		OnListBegin(count);
		for (auto &item : map) {
			OnObjectBegin();
	...
	// serialized as a list of pairs
	template <class V>
	void WriteValue(const sabot_sql::InsertionOrderPreservingMap<V> &map) {
		auto count = map.size();
		OnListBegin(count);
		for (auto &entry : map) {
			OnObjectBegin();
