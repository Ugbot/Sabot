...
//                         SabotSQL
//
// sabot_sql/common/serializer/serialization_data.hpp
//
//
//===----------------------------------------------------------------------===//
...
struct SerializationData {
	...
	sabot_sql::unordered_map<std::string, sabot_sql::stack<sabot_sql::reference<CustomData>>> customs;
	...
	typename std::enable_if<std::is_base_of<CustomData, T>::value, T &>::type GetCustom() const {
		...
		if (iter == customs.end()) {
			throw sabot_sql::InternalException("SeserializationData - no stack for %s", type);
		}
		auto &stack = iter->second;
		if (stack.empty()) {
			throw sabot_sql::InternalException("SerializationData - unexpected empty stack for %s", type);
		}
	...
	typename std::enable_if<std::is_base_of<CustomData, T>::value, void>::type SetCustom(T &data) {
		...
		auto iter = customs.find(type);
		if (iter == customs.end()) {
			iter = customs.emplace(type, sabot_sql::stack<sabot_sql::reference<CustomData>> {}).first;
		}
		auto &stack = iter->second;
		stack.push(data);
	}
