...
//                         SabotSQL
//
// sabot_sql/logging/log_storage.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/parallel/thread_context.hpp"
...
class LogStorage {
public:
	SABOT_SQL_API explicit LogStorage() {
	}
	SABOT_SQL_API virtual ~LogStorage() = default;
	...
	//! WRITING
	SABOT_SQL_API virtual void WriteLogEntry(timestamp_t timestamp, LogLevel level, const string &log_type,
	                                      const string &log_message, const RegisteredLoggingContext &context) = 0;
	SABOT_SQL_API virtual void WriteLogEntries(DataChunk &chunk, const RegisteredLoggingContext &context) = 0;
	SABOT_SQL_API virtual void FlushAll() = 0;
	SABOT_SQL_API virtual void Flush(LoggingTargetTable table) = 0;
	SABOT_SQL_API virtual void Truncate();
	SABOT_SQL_API virtual bool IsEnabled(LoggingTargetTable table) = 0;

	//! READING (OPTIONAL)
	SABOT_SQL_API virtual bool CanScan(LoggingTargetTable table) {
		return false;
	}
	// Reading interface 1: basic single-threaded scan
	SABOT_SQL_API virtual unique_ptr<LogStorageScanState> CreateScanState(LoggingTargetTable table) const;
	SABOT_SQL_API virtual bool Scan(LogStorageScanState &state, DataChunk &result) const;
	SABOT_SQL_API virtual void InitializeScan(LogStorageScanState &state) const;

	// Reading interface 2: using bind_replace
	SABOT_SQL_API virtual unique_ptr<TableRef> BindReplace(ClientContext &context, TableFunctionBindInput &input,
	                                                    LoggingTargetTable table);

	//! CONFIGURATION
	SABOT_SQL_API virtual void UpdateConfig(DatabaseInstance &db, case_insensitive_map_t<Value> &config);
};
