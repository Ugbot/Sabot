...
//                         SabotSQL
//
// sabot_sql/main/extension_helper.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/main/extension_entries.hpp"
...
struct ExtensionInitResult {
	...
	// The deserialized install from the `<ext>.sabot_sql_extension.info` file
...
enum class ExtensionUpdateResultTag : uint8_t {
	...
	NOT_INSTALLED = 3,
	// Statically loaded extensions can not be updated; they are baked into the SabotSQL executable
	STATICALLY_LOADED = 4,
...
public:
	static void LoadAllExtensions(SabotSQL &db);

	static ExtensionLoadResult LoadExtension(SabotSQL &db, const std::string &extension);
	...
	SABOT_SQL_API static bool TryAutoLoadExtension(DatabaseInstance &db, const string &extension_name) noexcept;
	SABOT_SQL_API static bool TryAutoLoadExtension(ClientContext &context, const string &extension_name) noexcept;
	...
	//! Default extensions are all extensions that SabotSQL knows and expect to be available (both in-tree and
	//! out-of-tree)
...
private:
	static ExtensionLoadResult LoadExtensionInternal(SabotSQL &db, const std::string &extension, bool initial_load);
};
