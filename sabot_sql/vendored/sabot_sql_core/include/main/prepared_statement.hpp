...
//                         SabotSQL
//
// sabot_sql/main/prepared_statement.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	...
	SABOT_SQL_API PreparedStatement(shared_ptr<ClientContext> context, shared_ptr<PreparedStatementData> data,
	                             string query, case_insensitive_map_t<idx_t> named_param_map);
	//! Create a prepared statement that was not successfully prepared
	SABOT_SQL_API explicit PreparedStatement(ErrorData error);

	SABOT_SQL_API ~PreparedStatement();
...
public:
	...
	SABOT_SQL_API const string &GetError();
	...
	SABOT_SQL_API StatementType GetStatementType();
	//! Returns the underlying statement properties
	SABOT_SQL_API StatementProperties GetStatementProperties();
	//! Returns the result SQL types of the prepared statement
	SABOT_SQL_API const vector<LogicalType> &GetTypes();
	//! Returns the result names of the prepared statement
	SABOT_SQL_API const vector<string> &GetNames();
	//! Returns the map of parameter index to the expected type of parameter
	SABOT_SQL_API case_insensitive_map_t<LogicalType> GetExpectedParameterTypes() const;
	...
	//! Create a pending query result of the prepared statement with the given set of arguments
	SABOT_SQL_API unique_ptr<PendingQueryResult> PendingQuery(vector<Value> &values, bool allow_stream_result = true);

	//! Create a pending query result of the prepared statement with the given set named arguments
	SABOT_SQL_API unique_ptr<PendingQueryResult> PendingQuery(case_insensitive_map_t<BoundParameterData> &named_values,
	                                                       bool allow_stream_result = true);

	//! Execute the prepared statement with the given set of values
	SABOT_SQL_API unique_ptr<QueryResult> Execute(vector<Value> &values, bool allow_stream_result = true);

	//! Execute the prepared statement with the given set of named+unnamed values
	SABOT_SQL_API unique_ptr<QueryResult> Execute(case_insensitive_map_t<BoundParameterData> &named_values,
	                                           bool allow_stream_result = true);
