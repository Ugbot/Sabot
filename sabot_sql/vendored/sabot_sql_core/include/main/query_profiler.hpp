...
//                         SabotSQL
//
// sabot_sql/main/query_profiler.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	SABOT_SQL_API explicit OperatorProfiler(ClientContext &context);
	~OperatorProfiler() {
	}

public:
	SABOT_SQL_API void StartOperator(optional_ptr<const PhysicalOperator> phys_op);
	SABOT_SQL_API void EndOperator(optional_ptr<DataChunk> chunk);
	SABOT_SQL_API void FinishSource(GlobalSourceState &gstate, LocalSourceState &lstate);

	//! Adds the timings in the OperatorProfiler (tree) to the QueryProfiler (tree).
	SABOT_SQL_API void Flush(const PhysicalOperator &phys_op);
	SABOT_SQL_API OperatorInformation &GetOperatorInfo(const PhysicalOperator &phys_op);
	SABOT_SQL_API bool OperatorInfoIsInitialized(const PhysicalOperator &phys_op);
	SABOT_SQL_API void AddExtraInfo(InsertionOrderPreservingMap<string> extra_info);
...
public:
	SABOT_SQL_API explicit QueryProfiler(ClientContext &context);

public:
	//! Propagate save_location, enabled, detailed_enabled and automatic_print_format.
	void Propagate(QueryProfiler &qp);

	SABOT_SQL_API bool IsEnabled() const;
	SABOT_SQL_API bool IsDetailedEnabled() const;
	SABOT_SQL_API ProfilerPrintFormat GetPrintFormat(ExplainFormat format = ExplainFormat::DEFAULT) const;
	SABOT_SQL_API bool PrintOptimizerOutput() const;
	SABOT_SQL_API string GetSaveLocation() const;

	SABOT_SQL_API static QueryProfiler &Get(ClientContext &context);

	SABOT_SQL_API void Start(const string &query);
	SABOT_SQL_API void Reset();
	SABOT_SQL_API void StartQuery(const string &query, bool is_explain_analyze = false, bool start_at_optimizer = false);
	SABOT_SQL_API void EndQuery();

	//! Adds nr_bytes bytes to the total bytes read.
	SABOT_SQL_API void AddBytesRead(const idx_t nr_bytes);
	//! Adds nr_bytes bytes to the total bytes written.
	SABOT_SQL_API void AddBytesWritten(const idx_t nr_bytes);

	SABOT_SQL_API void StartExplainAnalyze();

	//! Adds the timings gathered by an OperatorProfiler to this query profiler
	SABOT_SQL_API void Flush(OperatorProfiler &profiler);
	//! Adds the top level query information to the global profiler.
	SABOT_SQL_API void SetInfo(const double &blocked_thread_time);

	SABOT_SQL_API void StartPhase(MetricsType phase_metric);
	SABOT_SQL_API void EndPhase();

	SABOT_SQL_API void Initialize(const PhysicalOperator &root);

	SABOT_SQL_API string QueryTreeToString() const;
	SABOT_SQL_API void QueryTreeToStream(std::ostream &str) const;
	...
	SABOT_SQL_API string ToString(ExplainFormat format = ExplainFormat::DEFAULT) const;
	SABOT_SQL_API string ToString(ProfilerPrintFormat format) const;
	...
	SABOT_SQL_API string ToJSON() const;
	SABOT_SQL_API void WriteToFile(const char *path, string &info) const;
	...
	SABOT_SQL_API void GetRootUnderLock(const std::function<void(optional_ptr<ProfilingNode>)> &callback) {
