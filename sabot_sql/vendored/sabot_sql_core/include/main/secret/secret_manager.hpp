...
//                         SabotSQL
//
// sabot_sql/main/secret/duck_secret_manager.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/main/secret/secret_storage.hpp"
...
//! The Secret Manager for SabotSQL. Can handle both temporary and persistent secrets
class SecretManager {
...
public:
	...
	SABOT_SQL_API static SecretManager &Get(ClientContext &context);
	SABOT_SQL_API static SecretManager &Get(DatabaseInstance &db);

	// Initialize the secret manager with the DB instance
	SABOT_SQL_API void Initialize(DatabaseInstance &db);
	//! Load a secret storage
	SABOT_SQL_API void LoadSecretStorage(unique_ptr<SecretStorage> storage);

	//! Deserialize a secret by automatically selecting the correct deserializer, secret_path can be set to improve
	//! error hints
	SABOT_SQL_API unique_ptr<BaseSecret> DeserializeSecret(Deserializer &deserializer, const string &secret_path = "");
	//! Register a new SecretType
	SABOT_SQL_API void RegisterSecretType(SecretType &type);
	//! Lookup a SecretType
	SABOT_SQL_API SecretType LookupType(const string &type);
	//! Register a Secret Function i.e. a secret provider for a secret type
	SABOT_SQL_API void RegisterSecretFunction(CreateSecretFunction function, OnCreateConflict on_conflict);
	//! Register a secret by providing a secret manually
	SABOT_SQL_API unique_ptr<SecretEntry> RegisterSecret(CatalogTransaction transaction,
	...
	SABOT_SQL_API unique_ptr<SecretEntry> CreateSecret(ClientContext &context, const CreateSecretInput &info);
	//! The Bind for create secret is done by the secret manager
	SABOT_SQL_API BoundStatement BindCreateSecret(CatalogTransaction transaction, CreateSecretInput &info);
	//! Lookup the best matching secret by matching the secret scopes to the path
	SABOT_SQL_API SecretMatch LookupSecret(CatalogTransaction transaction, const string &path, const string &type);
	//! Get a secret by name, optionally from a specific storage
	SABOT_SQL_API unique_ptr<SecretEntry> GetSecretByName(CatalogTransaction transaction, const string &name,
	                                                   const string &storage = "");
	//! Delete a secret by name, optionally by providing the storage to drop from
	SABOT_SQL_API void DropSecretByName(CatalogTransaction transaction, const string &name,
	                                 OnEntryNotFound on_entry_not_found,
	                                 SecretPersistType persist_type = SecretPersistType::DEFAULT,
	                                 const string &storage = "");
	//! List all secrets from all secret storages
	SABOT_SQL_API vector<SecretEntry> AllSecrets(CatalogTransaction transaction);

	//! List all secret types
	SABOT_SQL_API vector<SecretType> AllSecretTypes();

	//! Secret Manager settings
	SABOT_SQL_API virtual void SetEnablePersistentSecrets(bool enabled);
	SABOT_SQL_API virtual void ResetEnablePersistentSecrets();
	SABOT_SQL_API virtual bool PersistentSecretsEnabled();

	SABOT_SQL_API virtual void SetDefaultStorage(const string &storage);
	SABOT_SQL_API virtual void ResetDefaultStorage();
	SABOT_SQL_API virtual string DefaultStorage();

	SABOT_SQL_API virtual void SetPersistentSecretPath(const string &path);
	SABOT_SQL_API virtual void ResetPersistentSecretPath();
	SABOT_SQL_API virtual string PersistentSecretPath();

	//! Utility functions
	SABOT_SQL_API void DropSecretByName(ClientContext &context, const string &name, OnEntryNotFound on_entry_not_found,
	                                 SecretPersistType persist_type = SecretPersistType::DEFAULT,
	                                 const string &storage = "");
