...
//                         SabotSQL
//
// sabot_sql/main/query_result.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	...
	SABOT_SQL_API BaseQueryResult(QueryResultType type, StatementType statement_type, StatementProperties properties,
	                           vector<LogicalType> types, vector<string> names);
	//! Creates an unsuccessful query result with error condition
	SABOT_SQL_API BaseQueryResult(QueryResultType type, ErrorData error);
	SABOT_SQL_API virtual ~BaseQueryResult();
...
public:
	[[noreturn]] SABOT_SQL_API void ThrowError(const string &prepended_message = "") const;
	SABOT_SQL_API void SetError(ErrorData error);
	SABOT_SQL_API bool HasError() const;
	SABOT_SQL_API const ExceptionType &GetErrorType() const;
	SABOT_SQL_API const std::string &GetError();
	SABOT_SQL_API ErrorData &GetErrorObject();
	SABOT_SQL_API idx_t ColumnCount();
...
public:
	//! Creates a successful query result with the specified names and types
	SABOT_SQL_API QueryResult(QueryResultType type, StatementType statement_type, StatementProperties properties,
	                       vector<LogicalType> types, vector<string> names, ClientProperties client_properties);
	//! Creates an unsuccessful query result with error condition
	SABOT_SQL_API QueryResult(QueryResultType type, ErrorData error);
	SABOT_SQL_API ~QueryResult() override;
...
public:
	...
	SABOT_SQL_API const string &ColumnName(idx_t index) const;
	...
	SABOT_SQL_API virtual unique_ptr<DataChunk> Fetch();
	//! Fetches a DataChunk from the query result. The vectors are not normalized and hence any vector types can be
	//! returned.
	SABOT_SQL_API virtual unique_ptr<DataChunk> FetchRaw() = 0;
	//! Converts the QueryResult to a string
	SABOT_SQL_API virtual string ToString() = 0;
	//! Converts the QueryResult to a box-rendered string
	SABOT_SQL_API virtual string ToBox(ClientContext &context, const BoxRendererConfig &config);
	...
	SABOT_SQL_API bool Equals(QueryResult &other);
...
protected:
	SABOT_SQL_API string HeaderToString();
