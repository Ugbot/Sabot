...
//                         SabotSQL
//
// sabot_sql/main/capi/cast/generic_cast.hpp
//
//
//===----------------------------------------------------------------------===//
...
template <class RESULT_TYPE, class OP = sabot_sql::TryCast>
RESULT_TYPE GetInternalCValue(sabot_sql_result *result, idx_t col, idx_t row) {
	...
	case SABOT_SQL_TYPE_BLOB:
		return TryCastCInternal<sabot_sql_blob, RESULT_TYPE, FromCBlobCastWrapper>(result, col, row);
