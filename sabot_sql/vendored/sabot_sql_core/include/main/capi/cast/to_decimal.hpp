...
struct ToCDecimalCastWrapper {
	...
	static bool Operation(SOURCE_TYPE input, sabot_sql_decimal &result, CastParameters &parameters, uint8_t width,
	                      uint8_t scale) {
...
struct ToCDecimalCastWrapper<hugeint_t> {
	...
	static bool Operation(SOURCE_TYPE input, sabot_sql_decimal &result, CastParameters &parameters, uint8_t width,
		...
		if (!TryCastToDecimal::Operation<SOURCE_TYPE, hugeint_t>(input, intermediate_result, parameters, width,
		                                                         scale)) {
			result = FetchDefaultValue::Operation<sabot_sql_decimal>();
			return false;
		}
		result.scale = scale;
		result.width = width;

		sabot_sql_hugeint hugeint_value;
...
sabot_sql_decimal TryCastToDecimalCInternal(SOURCE_TYPE source, uint8_t width, uint8_t scale) {
...
template <class SOURCE_TYPE, class OP>
sabot_sql_decimal TryCastToDecimalCInternal(sabot_sql_result *result, idx_t col, idx_t row, uint8_t width, uint8_t scale) {
