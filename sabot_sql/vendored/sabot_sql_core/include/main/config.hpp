...
#include "sabot_sql/common/arrow/arrow_type_extension.hpp"
...
#include "sabot_sql/common/enums/access_mode.hpp"
#include "sabot_sql/common/enums/thread_pin_mode.hpp"
#include "sabot_sql/common/enums/compression_type.hpp"
#include "sabot_sql/common/enums/optimizer_type.hpp"
#include "sabot_sql/common/enums/order_type.hpp"
...
#include "sabot_sql/execution/index/index_type_set.hpp"
#include "sabot_sql/function/cast/default_casts.hpp"
#include "sabot_sql/function/replacement_scan.hpp"
#include "sabot_sql/optimizer/optimizer_extension.hpp"
#include "sabot_sql/parser/parsed_data/create_info.hpp"
#include "sabot_sql/parser/parser_extension.hpp"
#include "sabot_sql/planner/operator_extension.hpp"
#include "sabot_sql/storage/compression/bitpacking.hpp"
#include "sabot_sql/function/encoding_function.hpp"
...
struct DBConfigOptions {
...
#ifdef SABOT_SQL_EXTENSION_AUTOLOAD_DEFAULT
	//! Whether known extensions are allowed to be automatically loaded when a query depends on them
	bool autoload_known_extensions = SABOT_SQL_EXTENSION_AUTOLOAD_DEFAULT;
#else
	bool autoload_known_extensions = false;
#endif
#ifdef SABOT_SQL_EXTENSION_AUTOINSTALL_DEFAULT
	//! Whether known extensions are allowed to be automatically installed when a query depends on them
	bool autoinstall_known_extensions = SABOT_SQL_EXTENSION_AUTOINSTALL_DEFAULT;
#else
	...
	//! The number of external threads that work on SabotSQL tasks. Default: 1.
	...
	//! Serialize the metadata on checkpoint with compatibility for a given SabotSQL version.
	SerializationCompatibility serialization_compatibility = SerializationCompatibility::Default();
	//! Initialize the database with the standard set of SabotSQL functions
	...
	//! SabotSQL API surface
	string sabot_sql_api;
	//! Metadata from SabotSQL callers
	string custom_user_agent;
	//! Encrypt the temp files
	bool temp_file_encryption = false;
	//! The default block allocation size for new sabot_sql database files (new as-in, they do not yet exist).
	idx_t default_block_alloc_size = DEFAULT_BLOCK_ALLOC_SIZE;
	//! The default block header size for new sabot_sql database files.
	idx_t default_block_header_size = SABOT_SQL_BLOCK_HEADER_STORAGE_SIZE;
...
public:
	SABOT_SQL_API DBConfig();
	explicit SABOT_SQL_API DBConfig(bool read_only);
	SABOT_SQL_API DBConfig(const case_insensitive_map_t<Value> &config_dict, bool read_only);
	SABOT_SQL_API ~DBConfig();
	...
	case_insensitive_map_t<sabot_sql::unique_ptr<StorageExtension>> storage_extensions;
...
public:
	SABOT_SQL_API static DBConfig &GetConfig(ClientContext &context);
	SABOT_SQL_API static DBConfig &GetConfig(DatabaseInstance &db);
	SABOT_SQL_API static DBConfig &Get(AttachedDatabase &db);
	SABOT_SQL_API static const DBConfig &GetConfig(const ClientContext &context);
	SABOT_SQL_API static const DBConfig &GetConfig(const DatabaseInstance &db);
	SABOT_SQL_API static vector<ConfigurationOption> GetOptions();
	SABOT_SQL_API static vector<ConfigurationAlias> GetAliases();
	SABOT_SQL_API static idx_t GetOptionCount();
	SABOT_SQL_API static idx_t GetAliasCount();
	SABOT_SQL_API static vector<string> GetOptionNames();
	SABOT_SQL_API static bool IsInMemoryDatabase(const char *database_path);

	SABOT_SQL_API void AddExtensionOption(const string &name, string description, LogicalType parameter,
	...
	SABOT_SQL_API static optional_ptr<const ConfigurationOption> GetOptionByIndex(idx_t index);
	//! Fetcha n alias by index, or nullptr if out of range
	SABOT_SQL_API static optional_ptr<const ConfigurationAlias> GetAliasByIndex(idx_t index);
	//! Fetch an option by name. Returns a pointer to the option, or nullptr if none exists.
	SABOT_SQL_API static optional_ptr<const ConfigurationOption> GetOptionByName(const String &name);
	SABOT_SQL_API void SetOption(const ConfigurationOption &option, const Value &value);
	SABOT_SQL_API void SetOption(optional_ptr<DatabaseInstance> db, const ConfigurationOption &option, const Value &value);
	SABOT_SQL_API void SetOptionByName(const string &name, const Value &value);
	SABOT_SQL_API void SetOptionsByName(const case_insensitive_map_t<Value> &values);
	SABOT_SQL_API void ResetOption(optional_ptr<DatabaseInstance> db, const ConfigurationOption &option);
	SABOT_SQL_API void SetOption(const string &name, Value value);
	SABOT_SQL_API void ResetOption(const String &name);
	SABOT_SQL_API void ResetGenericOption(const String &name);
	static LogicalType ParseLogicalType(const string &type);

	SABOT_SQL_API void CheckLock(const String &name);

	SABOT_SQL_API static idx_t ParseMemoryLimit(const string &arg);

	//! Returns the list of possible compression functions for the physical type.
	SABOT_SQL_API vector<reference<CompressionFunction>> GetCompressionFunctions(const PhysicalType physical_type);
	//! Returns the compression function matching the compression and physical type.
	SABOT_SQL_API optional_ptr<CompressionFunction> GetCompressionFunction(CompressionType type,
	                                                                    const PhysicalType physical_type);
	//! Sets the disabled compression methods
	SABOT_SQL_API void SetDisabledCompressionMethods(const vector<CompressionType> &disabled_compression_methods);
	//! Returns a list of disabled compression methods
	SABOT_SQL_API vector<CompressionType> GetDisabledCompressionMethods() const;

	//! Returns the encode function matching the encoding name.
	SABOT_SQL_API optional_ptr<EncodingFunction> GetEncodeFunction(const string &name) const;
	SABOT_SQL_API void RegisterEncodeFunction(const EncodingFunction &function) const;
	//! Returns the encode function names.
	SABOT_SQL_API vector<reference<EncodingFunction>> GetLoadedEncodedFunctions() const;
	//! Returns the encode function matching the encoding name.
	SABOT_SQL_API ArrowTypeExtension GetArrowExtension(ArrowExtensionMetadata info) const;
	SABOT_SQL_API ArrowTypeExtension GetArrowExtension(const LogicalType &type) const;
	SABOT_SQL_API bool HasArrowExtension(const LogicalType &type) const;
	SABOT_SQL_API bool HasArrowExtension(ArrowExtensionMetadata info) const;
	SABOT_SQL_API void RegisterArrowExtension(const ArrowTypeExtension &extension) const;

	bool operator==(const DBConfig &other);
	bool operator!=(const DBConfig &other);

	SABOT_SQL_API CastFunctionSet &GetCastFunctions();
	SABOT_SQL_API CollationBinding &GetCollationBinding();
	SABOT_SQL_API IndexTypeSet &GetIndexTypes();
