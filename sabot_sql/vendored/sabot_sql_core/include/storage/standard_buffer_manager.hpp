...
//                         SabotSQL
//
// sabot_sql/storage/standard_buffer_manager.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/storage/buffer/block_handle.hpp"
#include "sabot_sql/storage/buffer/buffer_pool.hpp"
...
public:
	...
	SABOT_SQL_API shared_ptr<BlockHandle> AllocateTemporaryMemory(MemoryTag tag, idx_t block_size,
	                                                           bool can_destroy = true) final;
	SABOT_SQL_API shared_ptr<BlockHandle> AllocateMemory(MemoryTag tag, BlockManager *block_manager,
	                                                  bool can_destroy = true) final;
	SABOT_SQL_API BufferHandle Allocate(MemoryTag tag, idx_t block_size, bool can_destroy = true) final;
	SABOT_SQL_API BufferHandle Allocate(MemoryTag tag, BlockManager *block_manager, bool can_destroy = true) final;
	...
	SABOT_SQL_API Allocator &GetBufferAllocator() final;
	...
	SABOT_SQL_API void ReserveMemory(idx_t size) final;
	SABOT_SQL_API void FreeReservedMemory(idx_t size) final;
