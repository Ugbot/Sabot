...
//                         SabotSQL
//
// sabot_sql/storage/string_uncompressed.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/storage/checkpoint/string_checkpoint_state.hpp"
#include "sabot_sql/storage/segment/uncompressed.hpp"
...
public:
	...
	static idx_t StringAppendBase(BufferHandle &handle, ColumnSegment &segment, SegmentStatistics &stats,
		...
		for (idx_t i = 0; i < count; i++) {
			...
			if (SABOT_SQL_UNLIKELY(required_space >=
			...
			if (SABOT_SQL_UNLIKELY(required_space > remaining_space)) {
			...
			if (SABOT_SQL_UNLIKELY(use_overflow_block)) {
...
public:
	...
	inline static string_t FetchStringFromDict(ColumnSegment &segment, uint32_t dict_end_offset, Vector &result,
		...
		if (SABOT_SQL_LIKELY(dict_offset >= 0)) {
