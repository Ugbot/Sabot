...
//                         SabotSQL
//
// sabot_sql/storage/caching_file_system.hpp
//
//
//===----------------------------------------------------------------------===//
...
public:
	SABOT_SQL_API CachingFileHandle(QueryContext context, CachingFileSystem &caching_file_system, const OpenFileInfo &path,
	                             FileOpenFlags flags, CachedFile &cached_file);
	SABOT_SQL_API ~CachingFileHandle();

public:
	//! Get the underlying FileHandle
	SABOT_SQL_API FileHandle &GetFileHandle();
	...
	SABOT_SQL_API BufferHandle Read(data_ptr_t &buffer, idx_t nr_bytes, idx_t location);
	//! Read (non-seeking) nr bytes from the file (or cache), same as above, also sets nr_bytes to actually read bytes
	SABOT_SQL_API BufferHandle Read(data_ptr_t &buffer, idx_t &nr_bytes);
	//! Get some properties of the file
	SABOT_SQL_API string GetPath() const;
	SABOT_SQL_API idx_t GetFileSize();
	SABOT_SQL_API timestamp_t GetLastModifiedTime();
	SABOT_SQL_API string GetVersionTag();
	SABOT_SQL_API bool Validate() const;
	SABOT_SQL_API bool CanSeek();
	SABOT_SQL_API bool IsRemoteFile() const;
...
public:
	SABOT_SQL_API CachingFileSystem(FileSystem &file_system, DatabaseInstance &db);
	SABOT_SQL_API ~CachingFileSystem();

public:
	SABOT_SQL_API static CachingFileSystem Get(ClientContext &context);

	SABOT_SQL_API unique_ptr<CachingFileHandle> OpenFile(const OpenFileInfo &path, FileOpenFlags flags);
	SABOT_SQL_API unique_ptr<CachingFileHandle> OpenFile(QueryContext context, const OpenFileInfo &path,
	                                                  FileOpenFlags flags);
