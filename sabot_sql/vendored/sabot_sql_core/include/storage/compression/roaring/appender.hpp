...
//                         SabotSQL
//
// sabot_sql/storage/compression/roaring/appender.hpp
//
//
//===----------------------------------------------------------------------===//
...
#include "sabot_sql/storage/compression/roaring/roaring.hpp"
...
public:
	static void AppendBytes(STATE_TYPE &state, validity_t entry, idx_t bits) {
		...
		if (SABOT_SQL_UNLIKELY(last_bits != 0)) {
	...
	static void AppendVector(STATE_TYPE &state, Vector &input, idx_t input_size) {
		...
		if (validity.AllValid()) {
			...
			while (appended < input_size) {
				...
				if (SABOT_SQL_UNLIKELY(previous_entry_remainder != 0)) {
				...
				if (SABOT_SQL_UNLIKELY(remainder != 0)) {
