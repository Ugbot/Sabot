#include "sabot_sql/common/adbc/adbc.hpp"
#include "sabot_sql/common/adbc/adbc-init.hpp"
...
#include "sabot_sql/common/arrow/nanoarrow/nanoarrow.hpp"

#include "sabot_sql/main/connection.hpp"
#include "sabot_sql/common/adbc/options.h"
#include "sabot_sql/common/adbc/single_batch_array_stream.hpp"
#include "sabot_sql/function/table/arrow.hpp"
#include "sabot_sql/common/adbc/wrappers.hpp"
#include <stdlib.h>
#include <string.h>
...
AdbcStatusCode sabot_sql_adbc_init(int version, void *driver, struct AdbcError *error) {
	if (!driver) {
		return ADBC_STATUS_INVALID_ARGUMENT;
	}
	auto adbc_driver = static_cast<AdbcDriver *>(driver);

	adbc_driver->DatabaseNew = sabot_sql_adbc::DatabaseNew;
	adbc_driver->DatabaseSetOption = sabot_sql_adbc::DatabaseSetOption;
	adbc_driver->DatabaseInit = sabot_sql_adbc::DatabaseInit;
	adbc_driver->DatabaseRelease = sabot_sql_adbc::DatabaseRelease;
	adbc_driver->ConnectionNew = sabot_sql_adbc::ConnectionNew;
	adbc_driver->ConnectionSetOption = sabot_sql_adbc::ConnectionSetOption;
	adbc_driver->ConnectionInit = sabot_sql_adbc::ConnectionInit;
	adbc_driver->ConnectionRelease = sabot_sql_adbc::ConnectionRelease;
	adbc_driver->ConnectionGetTableTypes = sabot_sql_adbc::ConnectionGetTableTypes;
	adbc_driver->StatementNew = sabot_sql_adbc::StatementNew;
	adbc_driver->StatementRelease = sabot_sql_adbc::StatementRelease;
	adbc_driver->StatementBind = sabot_sql_adbc::StatementBind;
	adbc_driver->StatementBindStream = sabot_sql_adbc::StatementBindStream;
	adbc_driver->StatementExecuteQuery = sabot_sql_adbc::StatementExecuteQuery;
	adbc_driver->StatementPrepare = sabot_sql_adbc::StatementPrepare;
	adbc_driver->StatementSetOption = sabot_sql_adbc::StatementSetOption;
	adbc_driver->StatementSetSqlQuery = sabot_sql_adbc::StatementSetSqlQuery;
	adbc_driver->ConnectionGetObjects = sabot_sql_adbc::ConnectionGetObjects;
	adbc_driver->ConnectionCommit = sabot_sql_adbc::ConnectionCommit;
	adbc_driver->ConnectionRollback = sabot_sql_adbc::ConnectionRollback;
	adbc_driver->ConnectionReadPartition = sabot_sql_adbc::ConnectionReadPartition;
	adbc_driver->StatementExecutePartitions = sabot_sql_adbc::StatementExecutePartitions;
	adbc_driver->ConnectionGetInfo = sabot_sql_adbc::ConnectionGetInfo;
	adbc_driver->StatementGetParameterSchema = sabot_sql_adbc::StatementGetParameterSchema;
	adbc_driver->ConnectionGetTableSchema = sabot_sql_adbc::ConnectionGetTableSchema;
...
struct SabotSQLAdbcStatementWrapper {
	sabot_sql_connection connection;
	sabot_sql_prepared_statement statement;
...
struct SabotSQLAdbcStreamWrapper {
	sabot_sql_result result;
};
...
struct SabotSQLAdbcDatabaseWrapper {
	//! The SabotSQL Database Configuration
	sabot_sql_config config = nullptr;
	//! The SabotSQL Database
	sabot_sql_database database = nullptr;
...
AdbcStatusCode CheckResult(const sabot_sql_state &res, AdbcError *error, const char *error_msg) {
	if (!error) {
		// Error should be a non-null pointer
		return ADBC_STATUS_INVALID_ARGUMENT;
	}
	if (res != SabotSQLSuccess) {
...
AdbcStatusCode DatabaseNew(struct AdbcDatabase *database, struct AdbcError *error) {
	...
	auto wrapper = new (std::nothrow) SabotSQLAdbcDatabaseWrapper;
	if (!wrapper) {
		SetError(error, "Allocation error");
		return ADBC_STATUS_INVALID_ARGUMENT;
	}
	database->private_data = wrapper;
	auto res = sabot_sql_create_config(&wrapper->config);
...
AdbcStatusCode DatabaseSetOption(struct AdbcDatabase *database, const char *key, const char *value,
	...
	auto wrapper = static_cast<SabotSQLAdbcDatabaseWrapper *>(database->private_data);
	if (strcmp(key, "path") == 0) {
		wrapper->path = value;
		return ADBC_STATUS_OK;
	}
	auto res = sabot_sql_set_config(wrapper->config, key, value);
...
AdbcStatusCode DatabaseInit(struct AdbcDatabase *database, struct AdbcError *error) {
	...
	auto wrapper = static_cast<SabotSQLAdbcDatabaseWrapper *>(database->private_data);
	auto res = sabot_sql_open_ext(wrapper->path.c_str(), &wrapper->database, wrapper->config, &errormsg);
...
AdbcStatusCode DatabaseRelease(struct AdbcDatabase *database, struct AdbcError *error) {
	...
	if (database && database->private_data) {
		...
		sabot_sql_close(&wrapper->database);
		sabot_sql_destroy_config(&wrapper->config);
		delete wrapper;
		database->private_data = nullptr;
	}
	return ADBC_STATUS_OK;
...
AdbcStatusCode ConnectionGetTableSchema(struct AdbcConnection *connection, const char *catalog, const char *db_schema,
	...
	if (catalog != nullptr && strlen(catalog) > 0) {
		query += sabot_sql::KeywordHelper::WriteOptionallyQuoted(catalog) + ".";
	}
	query += sabot_sql::KeywordHelper::WriteOptionallyQuoted(db_schema) + ".";
	query += sabot_sql::KeywordHelper::WriteOptionallyQuoted(table_name) + " LIMIT 0;";
...
AdbcStatusCode ConnectionNew(struct AdbcConnection *connection, struct AdbcError *error) {
	...
	auto connection_wrapper = new sabot_sql::SabotSQLAdbcConnectionWrapper();
	connection_wrapper->connection = nullptr;
	connection->private_data = connection_wrapper;
	return ADBC_STATUS_OK;
}

AdbcStatusCode ExecuteQuery(sabot_sql::Connection *conn, const char *query, struct AdbcError *error) {
...
AdbcStatusCode InternalSetOption(sabot_sql::Connection &conn, std::unordered_map<std::string, std::string> &options,
                                 struct AdbcError *error) {
...
AdbcStatusCode ConnectionSetOption(struct AdbcConnection *connection, const char *key, const char *value,
	...
	auto conn_wrapper = static_cast<sabot_sql::SabotSQLAdbcConnectionWrapper *>(connection->private_data);
	if (strcmp(key, ADBC_CONNECTION_OPTION_AUTOCOMMIT) == 0) {
		...
		// This is an unknown option to the SabotSQL driver
		auto error_message =
	...
	auto conn = reinterpret_cast<sabot_sql::Connection *>(conn_wrapper->connection);
...
AdbcStatusCode ConnectionReadPartition(struct AdbcConnection *connection, const uint8_t *serialized_partition,
                                       ...
                                       struct AdbcError *error) {
	SetError(error, "Read Partitions are not supported in SabotSQL");
	return ADBC_STATUS_NOT_IMPLEMENTED;
}
...
AdbcStatusCode StatementExecutePartitions(struct AdbcStatement *statement, struct ArrowSchema *schema,
                                          ...
                                          struct AdbcError *error) {
	SetError(error, "Execute Partitions are not supported in SabotSQL");
	return ADBC_STATUS_NOT_IMPLEMENTED;
}
...
AdbcStatusCode ConnectionGetInfo(struct AdbcConnection *connection, const uint32_t *info_codes,
	...
	sabot_sql::string results = "";

	for (size_t i = 0; i < length; i++) {
		auto code = sabot_sql::NumericCast<uint32_t>(info_codes ? info_codes[i] : i);
		...
		case AdbcInfoCode::VENDOR_VERSION: {
			results += sabot_sql::StringUtil::Format("(1, '%s'),", sabot_sql_library_version());
			break;
		}
		case AdbcInfoCode::DRIVER_NAME: {
			results += "(2, 'ADBC SabotSQL Driver'),";
			break;
		}
...
AdbcStatusCode ConnectionInit(struct AdbcConnection *connection, struct AdbcDatabase *database,
	...
	auto database_wrapper = static_cast<SabotSQLAdbcDatabaseWrapper *>(database->private_data);
	auto conn_wrapper = static_cast<sabot_sql::SabotSQLAdbcConnectionWrapper *>(connection->private_data);
	conn_wrapper->connection = nullptr;

	auto res = sabot_sql_connect(database_wrapper->database, &conn_wrapper->connection);
...
AdbcStatusCode ConnectionRelease(struct AdbcConnection *connection, struct AdbcError *error) {
	if (connection && connection->private_data) {
		auto conn_wrapper = static_cast<sabot_sql::SabotSQLAdbcConnectionWrapper *>(connection->private_data);
		auto conn = reinterpret_cast<sabot_sql::Connection *>(conn_wrapper->connection);
		sabot_sql_disconnect(reinterpret_cast<sabot_sql_connection *>(&conn));
		delete conn_wrapper;
...
static int get_schema(struct ArrowArrayStream *stream, struct ArrowSchema *out) {
	...
	auto result_wrapper = static_cast<SabotSQLAdbcStreamWrapper *>(stream->private_data);
	auto count = sabot_sql_column_count(&result_wrapper->result);
	std::vector<sabot_sql_logical_type> types(count);

	std::vector<std::string> owned_names(count);
	sabot_sql::vector<const char *> names(count);
	for (idx_t i = 0; i < count; i++) {
		types[i] = sabot_sql_column_logical_type(&result_wrapper->result, i);
		auto column_name = sabot_sql_column_name(&result_wrapper->result, i);
		owned_names.emplace_back(column_name);
		names[i] = owned_names.back().c_str();
	}

	auto arrow_options = sabot_sql_result_get_arrow_options(&result_wrapper->result);

	auto res = sabot_sql_to_arrow_schema(arrow_options, &types[0], names.data(), count, out);
	sabot_sql_destroy_arrow_options(&arrow_options);
	for (auto &type : types) {
		sabot_sql_destroy_logical_type(&type);
	}
	if (res) {
		sabot_sql_destroy_error_data(&res);
		return SabotSQLError;
	}
...
static int get_next(struct ArrowArrayStream *stream, struct ArrowArray *out) {
	...
	out->release = nullptr;
	auto result_wrapper = static_cast<SabotSQLAdbcStreamWrapper *>(stream->private_data);
	auto sabot_sql_chunk = sabot_sql_fetch_chunk(result_wrapper->result);
	if (!sabot_sql_chunk) {
		return SabotSQLSuccess;
	}
	auto arrow_options = sabot_sql_result_get_arrow_options(&result_wrapper->result);

	auto conversion_success = sabot_sql_data_chunk_to_arrow(arrow_options, sabot_sql_chunk, out);
	sabot_sql_destroy_arrow_options(&arrow_options);
	sabot_sql_destroy_data_chunk(&sabot_sql_chunk);

	if (conversion_success) {
		sabot_sql_destroy_error_data(&conversion_success);
		return SabotSQLError;
	}
	return SabotSQLSuccess;
}
...
void release(struct ArrowArrayStream *stream) {
	...
	auto result_wrapper = reinterpret_cast<SabotSQLAdbcStreamWrapper *>(stream->private_data);
	if (result_wrapper) {
		sabot_sql_destroy_result(&result_wrapper->result);
	}
...
sabot_sql::unique_ptr<sabot_sql::ArrowArrayStreamWrapper> stream_produce(uintptr_t factory_ptr,
	...
	auto res = sabot_sql::make_uniq<sabot_sql::ArrowArrayStreamWrapper>();
...
AdbcStatusCode Ingest(sabot_sql_connection connection, const char *table_name, const char *schema,
	...
	sabot_sql::ArrowSchemaWrapper arrow_schema_wrapper;
	ConvertedSchemaWrapper out_types;

	input->get_schema(input, &arrow_schema_wrapper.arrow_schema);
	auto res = sabot_sql_schema_from_arrow(connection, &arrow_schema_wrapper.arrow_schema, out_types.GetPtr());
	if (res) {
		SetError(error, sabot_sql_error_data_message(res));
		sabot_sql_destroy_error_data(&res);
		return ADBC_STATUS_INTERNAL;
	}

	auto &d_converted_schema = *reinterpret_cast<sabot_sql::ArrowTableSchema *>(out_types.Get());
	...
	if (ingestion_mode == IngestionMode::CREATE) {
		...
		create_table << ");";
		sabot_sql_result result;
		if (sabot_sql_query(connection, create_table.str().c_str(), &result) == SabotSQLError) {
			SetError(error, sabot_sql_result_error(&result));
			sabot_sql_destroy_result(&result);
			return ADBC_STATUS_INTERNAL;
		}
		sabot_sql_destroy_result(&result);
	}
	...
	sabot_sql::ArrowArrayWrapper arrow_array_wrapper;
	...
	while (arrow_array_wrapper.arrow_array.release) {
		...
		auto res = sabot_sql_data_chunk_from_arrow(connection, &arrow_array_wrapper.arrow_array, out_types.Get(),
		                                        &out_chunk.chunk);
		if (res) {
			SetError(error, sabot_sql_error_data_message(res));
			sabot_sql_destroy_error_data(&res);
		}
		if (sabot_sql_append_data_chunk(appender.Get(), out_chunk.chunk) != SabotSQLSuccess) {
			return ADBC_STATUS_INTERNAL;
		}
		arrow_array_wrapper = sabot_sql::ArrowArrayWrapper();
		input->get_next(input, &arrow_array_wrapper.arrow_array);
	}
	return ADBC_STATUS_OK;
}
...
AdbcStatusCode StatementNew(struct AdbcConnection *connection, struct AdbcStatement *statement,
	...
	auto statement_wrapper = static_cast<SabotSQLAdbcStatementWrapper *>(malloc(sizeof(SabotSQLAdbcStatementWrapper)));
...
AdbcStatusCode StatementRelease(struct AdbcStatement *statement, struct AdbcError *error) {
	...
	auto wrapper = static_cast<SabotSQLAdbcStatementWrapper *>(statement->private_data);
	if (wrapper->statement) {
		sabot_sql_destroy_prepare(&wrapper->statement);
...
AdbcStatusCode StatementGetParameterSchema(struct AdbcStatement *statement, struct ArrowSchema *schema,
	...
	auto prepared_wrapper = reinterpret_cast<sabot_sql::PreparedStatementWrapper *>(wrapper->statement);
	...
	for (idx_t i = 0; i < count; i++) {
		...
		types[i] = sabot_sql_create_logical_type(SABOT_SQL_TYPE_SQLNULL);
		auto column_name = std::to_string(i);
		owned_names.emplace_back(column_name);
		names[i] = owned_names.back().c_str();
	}

	sabot_sql_arrow_options arrow_options;
	sabot_sql_connection_get_arrow_options(wrapper->connection, &arrow_options);

	auto res = sabot_sql_to_arrow_schema(arrow_options, &types[0], names.data(), count, schema);
...
static AdbcStatusCode IngestToTableFromBoundStream(SabotSQLAdbcStatementWrapper *statement, AdbcError *error) {
...
AdbcStatusCode StatementExecuteQuery(struct AdbcStatement *statement, struct ArrowArrayStream *out,
	...
	auto stream_wrapper = static_cast<SabotSQLAdbcStreamWrapper *>(malloc(sizeof(SabotSQLAdbcStreamWrapper)));
	if (has_stream) {
		...
		try {
			auto res =
			    sabot_sql_schema_from_arrow(wrapper->connection, &arrow_schema_wrapper.arrow_schema, out_types.GetPtr());
			if (res) {
				SetError(error, sabot_sql_error_data_message(res));
				sabot_sql_destroy_error_data(&res);
			}
		...
		auto prepared_statement_params =
		    reinterpret_cast<sabot_sql::PreparedStatementWrapper *>(wrapper->statement)->statement->named_param_map.size();
		...
		while (arrow_array_wrapper.arrow_array.release) {
			...
			DataChunkWrapper out_chunk;
			auto res_conv = sabot_sql_data_chunk_from_arrow(wrapper->connection, &arrow_array_wrapper.arrow_array,
			                                             out_types.Get(), &out_chunk.chunk);
			if (res_conv) {
				SetError(error, sabot_sql_error_data_message(res_conv));
				sabot_sql_destroy_error_data(&res_conv);
				return ADBC_STATUS_INVALID_ARGUMENT;
			}
			if (!out_chunk.chunk) {
			...
			auto chunk = reinterpret_cast<sabot_sql::DataChunk *>(out_chunk.chunk);
			if (chunk->size() == 0) {
				SetError(error, "Please provide a non-empty chunk to be bound");
				free(stream_wrapper);
				return ADBC_STATUS_INVALID_ARGUMENT;
			...
			sabot_sql_clear_bindings(wrapper->statement);
			for (idx_t col_idx = 0; col_idx < chunk->ColumnCount(); col_idx++) {
				auto val = chunk->GetValue(col_idx, 0);
				auto duck_val = reinterpret_cast<sabot_sql_value>(&val);
				auto res = sabot_sql_bind_value(wrapper->statement, 1 + col_idx, duck_val);
				if (res != SabotSQLSuccess) {
					SetError(error, sabot_sql_prepare_error(wrapper->statement));
					free(stream_wrapper);
					return ADBC_STATUS_INVALID_ARGUMENT;
				}
			}
			auto res = sabot_sql_execute_prepared(wrapper->statement, &stream_wrapper->result);
			if (res != SabotSQLSuccess) {
				SetError(error, sabot_sql_result_error(&stream_wrapper->result));
				free(stream_wrapper);
				return ADBC_STATUS_INVALID_ARGUMENT;
			}
...
AdbcStatusCode StatementSetSqlQuery(struct AdbcStatement *statement, const char *query, struct AdbcError *error) {
	...
	sabot_sql_extracted_statements extracted_statements = nullptr;
	auto extract_statements_size = sabot_sql_extract_statements(wrapper->connection, query, &extracted_statements);
	auto error_msg_extract_statements = sabot_sql_extract_statements_error(extracted_statements);
	if (error_msg_extract_statements != nullptr) {
		...
		sabot_sql_destroy_extracted(&extracted_statements);
		return ADBC_STATUS_INTERNAL;
	}
	// Now lets loop over the statements, and execute every one
	for (idx_t i = 0; i < extract_statements_size - 1; i++) {
		sabot_sql_prepared_statement statement_internal = nullptr;
		auto res =
		    sabot_sql_prepare_extracted_statement(wrapper->connection, extracted_statements, i, &statement_internal);
		auto error_msg = sabot_sql_prepare_error(statement_internal);
		auto adbc_status = CheckResult(res, error, error_msg);
		if (adbc_status != ADBC_STATUS_OK) {
			sabot_sql_destroy_prepare(&statement_internal);
			sabot_sql_destroy_extracted(&extracted_statements);
			return adbc_status;
		}
		// Execute
		sabot_sql_arrow out_result;
		res = sabot_sql_execute_prepared_arrow(statement_internal, &out_result);
		if (res != SabotSQLSuccess) {
			SetError(error, sabot_sql_query_arrow_error(out_result));
			sabot_sql_destroy_arrow(&out_result);
			sabot_sql_destroy_prepare(&statement_internal);
			sabot_sql_destroy_extracted(&extracted_statements);
			return ADBC_STATUS_INVALID_ARGUMENT;
		}
	...
	auto res = sabot_sql_prepare_extracted_statement(wrapper->connection, extracted_statements,
	                                              extract_statements_size - 1, &wrapper->statement);
	auto error_msg = sabot_sql_prepare_error(wrapper->statement);
	sabot_sql_destroy_extracted(&extracted_statements);
...
AdbcStatusCode StatementSetOption(struct AdbcStatement *statement, const char *key, const char *value,
	...
	ss << "Statement Set Option " << key << " is not yet accepted by SabotSQL";
...
AdbcStatusCode ConnectionGetObjects(struct AdbcConnection *connection, int depth, const char *catalog,
	...
	case ADBC_OBJECT_DEPTH_CATALOGS:
		...
		query = sabot_sql::StringUtil::Format(R"(
				SELECT
					catalog_name,
					[]::STRUCT(
