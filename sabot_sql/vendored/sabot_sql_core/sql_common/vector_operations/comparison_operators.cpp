...
#include "sabot_sql/common/vector_operations/binary_executor.hpp"
...
static bool EqualsFloat(T left, T right) {
	if (SABOT_SQL_UNLIKELY(Value::IsNan(left) && Value::IsNan(right))) {
		return true;
	}
	return left == right;
}
...
static bool GreaterThanFloat(T left, T right) {
	...
	if (SABOT_SQL_UNLIKELY(right_is_nan)) {
		return false;
	}
	// if left is nan, but right is not, left is always bigger
	if (SABOT_SQL_UNLIKELY(left_is_nan)) {
		return true;
	}
	return left > right;
}
...
template <>
inline idx_t ComparisonSelector::Select<sabot_sql::Equals>(Vector &left, Vector &right, const SelectionVector *sel,
...
template <>
inline idx_t ComparisonSelector::Select<sabot_sql::NotEquals>(Vector &left, Vector &right, const SelectionVector *sel,
                                                           idx_t count, SelectionVector *true_sel,
                                                           SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t ComparisonSelector::Select<sabot_sql::GreaterThan>(Vector &left, Vector &right, const SelectionVector *sel,
                                                             idx_t count, SelectionVector *true_sel,
                                                             SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t
ComparisonSelector::Select<sabot_sql::GreaterThanEquals>(Vector &left, Vector &right, const SelectionVector *sel,
                                                      idx_t count, SelectionVector *true_sel,
                                                      SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t ComparisonSelector::Select<sabot_sql::LessThan>(Vector &left, Vector &right, const SelectionVector *sel,
                                                          idx_t count, SelectionVector *true_sel,
                                                          SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t ComparisonSelector::Select<sabot_sql::LessThanEquals>(Vector &left, Vector &right, const SelectionVector *sel,
                                                                idx_t count, SelectionVector *true_sel,
                                                                SelectionVector *false_sel, ValidityMask &null_mask) {
...
void VectorOperations::Equals(Vector &left, Vector &right, Vector &result, idx_t count) {
	ComparisonExecutor::Execute<sabot_sql::Equals>(left, right, result, count);
}

void VectorOperations::NotEquals(Vector &left, Vector &right, Vector &result, idx_t count) {
	ComparisonExecutor::Execute<sabot_sql::NotEquals>(left, right, result, count);
}

void VectorOperations::GreaterThanEquals(Vector &left, Vector &right, Vector &result, idx_t count) {
	ComparisonExecutor::Execute<sabot_sql::GreaterThanEquals>(left, right, result, count);
}

void VectorOperations::LessThanEquals(Vector &left, Vector &right, Vector &result, idx_t count) {
	ComparisonExecutor::Execute<sabot_sql::GreaterThanEquals>(right, left, result, count);
}

void VectorOperations::GreaterThan(Vector &left, Vector &right, Vector &result, idx_t count) {
	ComparisonExecutor::Execute<sabot_sql::GreaterThan>(left, right, result, count);
}

void VectorOperations::LessThan(Vector &left, Vector &right, Vector &result, idx_t count) {
	ComparisonExecutor::Execute<sabot_sql::GreaterThan>(right, left, result, count);
}
