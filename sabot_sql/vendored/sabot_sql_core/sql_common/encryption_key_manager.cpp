#include "sabot_sql/common/encryption_key_manager.hpp"
...
string EncryptionKeyManager::GenerateRandomKeyID() {
	...
	sabot_sql_mbedtls::MbedTlsWrapper::AESStateMBEDTLS::GenerateRandomDataStatic(key_id, KEY_ID_BYTES);
...
string EncryptionKeyManager::Base64Decode(const string &key) {
	...
	auto output = sabot_sql::unique_ptr<unsigned char[]>(new unsigned char[result_size]);
