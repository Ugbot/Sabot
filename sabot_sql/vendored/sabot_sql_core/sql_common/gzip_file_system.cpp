#include "sabot_sql/common/gzip_file_system.hpp"
...
struct MiniZStreamWrapper : public StreamWrapper {
	...
	unique_ptr<sabot_sql_miniz::mz_stream> mz_stream_ptr;
	bool writing = false;
	sabot_sql_miniz::mz_ulong crc;
	idx_t total_size;
...
void MiniZStreamWrapper::Initialize(QueryContext context, CompressedFile &file, bool write) {
	Close();
	this->file = &file;
	mz_stream_ptr = make_uniq<sabot_sql_miniz::mz_stream>();
	memset(mz_stream_ptr.get(), 0, sizeof(sabot_sql_miniz::mz_stream));
	this->writing = write;
	...
	if (write) {
		...
		auto ret = mz_deflateInit2(mz_stream_ptr.get(), sabot_sql_miniz::MZ_DEFAULT_LEVEL, MZ_DEFLATED,
		                           -MZ_DEFAULT_WINDOW_BITS, 1, 0);
		if (ret != sabot_sql_miniz::MZ_OK) {
		...
		auto ret = sabot_sql_miniz::mz_inflateInit2(mz_stream_ptr.get(), -MZ_DEFAULT_WINDOW_BITS);
		if (ret != sabot_sql_miniz::MZ_OK) {
			throw InternalException("Failed to initialize miniz");
		}
	}
...
bool MiniZStreamWrapper::Read(StreamData &sd) {
	...
	if (sd.refresh) {
		...
		sabot_sql_miniz::mz_inflateEnd(mz_stream_ptr.get());
		auto sta = sabot_sql_miniz::mz_inflateInit2(mz_stream_ptr.get(), -MZ_DEFAULT_WINDOW_BITS);
		if (sta != sabot_sql_miniz::MZ_OK) {
			throw InternalException("Failed to initialize miniz");
		}
	}
	...
	auto ret = sabot_sql_miniz::mz_inflate(mz_stream_ptr.get(), sabot_sql_miniz::MZ_NO_FLUSH);
	if (ret != sabot_sql_miniz::MZ_OK && ret != sabot_sql_miniz::MZ_STREAM_END) {
		throw IOException("Failed to decode gzip stream: %s", sabot_sql_miniz::mz_error(ret));
	}
	...
	if (ret == sabot_sql_miniz::MZ_STREAM_END) {
...
void MiniZStreamWrapper::Write(CompressedFile &file, StreamData &sd, data_ptr_t uncompressed_data,
	...
	crc = sabot_sql_miniz::mz_crc32(crc, reinterpret_cast<const unsigned char *>(uncompressed_data),
	...
	while (remaining > 0) {
		...
		auto res = mz_deflate(mz_stream_ptr.get(), sabot_sql_miniz::MZ_NO_FLUSH);
		if (res != sabot_sql_miniz::MZ_OK) {
			D_ASSERT(res != sabot_sql_miniz::MZ_STREAM_END);
...
void MiniZStreamWrapper::FlushStream() const {
	...
	while (true) {
		...
		auto res = mz_deflate(mz_stream_ptr.get(), sabot_sql_miniz::MZ_FINISH);
		sd.out_buff_start += (output_remaining - mz_stream_ptr->avail_out);
		...
		if (res == sabot_sql_miniz::MZ_STREAM_END) {
			break;
		}
		if (res != sabot_sql_miniz::MZ_OK) {
			throw InternalException("Failed to compress GZIP block");
...
void MiniZStreamWrapper::Close() {
	...
	if (writing) {
		...
		sabot_sql_miniz::mz_deflateEnd(mz_stream_ptr.get());
	} else {
		sabot_sql_miniz::mz_inflateEnd(mz_stream_ptr.get());
	}
...
bool GZipFileSystem::CheckIsZip(const char *data, sabot_sql::idx_t size) {
...
string GZipFileSystem::UncompressGZIPString(const char *data, idx_t size) {
	...
	auto mz_stream_ptr = make_uniq<sabot_sql_miniz::mz_stream>();
	memset(mz_stream_ptr.get(), 0, sizeof(sabot_sql_miniz::mz_stream));
	...
	// stream is now set to beginning of payload data
	auto status = sabot_sql_miniz::mz_inflateInit2(mz_stream_ptr.get(), -MZ_DEFAULT_WINDOW_BITS);
	if (status != sabot_sql_miniz::MZ_OK) {
		throw InternalException("Failed to initialize miniz");
	}
	...
	while (status == sabot_sql_miniz::MZ_OK) {
		...
		status = mz_inflate(mz_stream_ptr.get(), sabot_sql_miniz::MZ_NO_FLUSH);
		if (status != sabot_sql_miniz::MZ_STREAM_END && status != sabot_sql_miniz::MZ_OK) {
