...
Regex::Regex(const std::string &pattern, RegexOptions options) {
	...
	regex = sabot_sql::make_shared_ptr<sabot_sql_re2::RE2>(StringPiece(pattern), o);
}

bool RegexSearchInternal(const char *input_data, size_t input_size, Match &match, const RE2 &regex, RE2::Anchor anchor,
                         size_t start, size_t end) {
	sabot_sql::vector<StringPiece> target_groups;
	auto group_count = sabot_sql::UnsafeNumericCast<size_t>(regex.NumberOfCapturingGroups() + 1);
	target_groups.resize(group_count);
	match.groups.clear();
	if (!regex.Match(StringPiece(input_data, input_size), start, end, anchor, target_groups.data(),
	                 sabot_sql::UnsafeNumericCast<int>(group_count))) {
		return false;
	}
	for (auto &group : target_groups) {
		GroupMatch group_match;
		group_match.text = group.ToString();
		group_match.position = group.data() != nullptr ? sabot_sql::NumericCast<uint32_t>(group.data() - input_data) : 0;
...
bool RegexMatch(const char *start, const char *end, Match &match, const Regex &regex) {
	auto sz = sabot_sql::UnsafeNumericCast<size_t>(end - start);
...
sabot_sql::vector<Match> RegexFindAll(const std::string &input, const Regex &regex) {
	return RegexFindAll(input.c_str(), input.size(), regex.GetRegex());
}

sabot_sql::vector<Match> RegexFindAll(const char *input_data, size_t input_size, const RE2 &regex) {
	sabot_sql::vector<Match> matches;
	size_t position = 0;
	Match match;
	while (RegexSearchInternal(input_data, input_size, match, regex, RE2::UNANCHORED, position, input_size)) {
		if (match.length(0)) {
			...
			if (!next_char_length) {
				throw sabot_sql::InvalidInputException("Invalid UTF-8 leading byte at position " +
