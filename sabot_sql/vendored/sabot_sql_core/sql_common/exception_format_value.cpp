...
#include "sabot_sql/common/helper.hpp" // defines SABOT_SQL_EXPLICIT_FALLTHROUGH which fmt will use to annotate
...
string ExceptionFormatValue::Format(const string &msg, std::vector<ExceptionFormatValue> &values) {
	try {
		std::vector<sabot_sql_fmt::basic_format_arg<sabot_sql_fmt::printf_context>> format_args;
		for (auto &val : values) {
			switch (val.type) {
			case ExceptionFormatValueType::FORMAT_VALUE_TYPE_DOUBLE:
				format_args.push_back(sabot_sql_fmt::internal::make_arg<sabot_sql_fmt::printf_context>(val.dbl_val));
				break;
			case ExceptionFormatValueType::FORMAT_VALUE_TYPE_INTEGER:
				format_args.push_back(sabot_sql_fmt::internal::make_arg<sabot_sql_fmt::printf_context>(val.int_val));
				break;
			case ExceptionFormatValueType::FORMAT_VALUE_TYPE_STRING:
				format_args.push_back(sabot_sql_fmt::internal::make_arg<sabot_sql_fmt::printf_context>(val.str_val));
				break;
			}
		}
		return sabot_sql_fmt::vsprintf(msg, sabot_sql_fmt::basic_format_args<sabot_sql_fmt::printf_context>(
