...
#include "sabot_sql/common/operator/add.hpp"
...
#include "sabot_sql/common/operator/double_cast_operator.hpp"
...
string_t CastFromPointer::Operation(uintptr_t input, Vector &vector) {
	std::string s = sabot_sql_fmt::format("0x{:x}", input);
...
struct HugeIntegerCastOperation {
	...
	static bool HandleDigit(T &state, uint8_t digit) {
		if (NEGATIVE) {
			if (SABOT_SQL_UNLIKELY(state.intermediate <
	...
	static bool HandleDecimal(T &state, uint8_t digit) {
		...
		if (SABOT_SQL_UNLIKELY(state.decimal_intermediate > (NumericLimits<int64_t>::Maximum() - digit) / 10)) {
			if (!state.FlushDecimal()) {
				return false;
			}
		}
