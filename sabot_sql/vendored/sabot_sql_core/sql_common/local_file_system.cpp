...
#include "sabot_sql/common/checksum.hpp"
...
#include "sabot_sql/function/scalar/string_common.hpp"
#include "sabot_sql/main/client_context.hpp"
#include "sabot_sql/main/database.hpp"
#include "sabot_sql/logging/file_system_logger.hpp"
...
#include <unistd.h>
#else
#include "sabot_sql/common/windows_util.hpp"
...
public:
	void Close() override {
		if (fd != -1) {
			close(fd);
			fd = -1;
			SABOT_SQL_LOG_FILE_SYSTEM_CLOSE((*this));
		}
	};
};
...
static string AdditionalProcessInfo(FileSystem &fs, pid_t pid) {
	if (pid == getpid()) {
		return "Lock is already held in current process, likely another SabotSQL instance";
	}
...
unique_ptr<FileHandle> LocalFileSystem::OpenFile(const string &path_p, FileOpenFlags flags,
	...
	if (flags.Lock() != FileLockType::NO_LOCK) {
		...
		if (file_type != FileType::FILE_TYPE_FIFO && file_type != FileType::FILE_TYPE_SOCKET) {
			...
			if (has_error) {
				...
				extended_error += ". See also https://sabot_sql.org/docs/stable/connect/concurrency";
	...
	if (opener) {
		...
		SABOT_SQL_LOG_FILE_SYSTEM_OPEN((*file_handle));
	}
...
void LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	SABOT_SQL_LOG_FILE_SYSTEM_READ(handle, bytes_to_read, location - UnsafeNumericCast<idx_t>(bytes_to_read));
}
...
int64_t LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	SABOT_SQL_LOG_FILE_SYSTEM_READ(handle, bytes_read, unix_handle.current_pos);
...
void LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	SABOT_SQL_LOG_FILE_SYSTEM_WRITE(handle, nr_bytes, location);
}
...
int64_t LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	SABOT_SQL_LOG_FILE_SYSTEM_WRITE(handle, nr_bytes, unix_handle.current_pos);
...
bool LocalFileSystem::ListFilesExtended(const string &directory,
	...
	sabot_sql::unique_ptr<DIR, std::function<void(DIR *)>> dir_unique_ptr(dir, [](DIR *d) { closedir(d); });
...
static string AdditionalLockInfo(const std::wstring path) {
	...
	// allocate
	auto process_info_buffer = sabot_sql::unique_ptr<RM_PROCESS_INFO[]>(new RM_PROCESS_INFO[process_info_size_needed]);
...
unique_ptr<FileHandle> LocalFileSystem::OpenFile(const string &path_p, FileOpenFlags flags,
	...
	if (opener) {
		...
		SABOT_SQL_LOG_FILE_SYSTEM_OPEN((*handle));
	}
...
void LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	SABOT_SQL_LOG_FILE_SYSTEM_READ(handle, bytes_read, location);
}
...
int64_t LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	SABOT_SQL_LOG_FILE_SYSTEM_READ(handle, bytes_read, pos - bytes_read);
	return bytes_read;
}
...
void LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	SABOT_SQL_LOG_FILE_SYSTEM_WRITE(handle, bytes_written, location);
}
...
int64_t LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	pos += bytes_written;
	SABOT_SQL_LOG_FILE_SYSTEM_WRITE(handle, bytes_written, pos - bytes_written);
	return bytes_written;
}
