...
#include "sabot_sql/common/types/sel_cache.hpp"
#include "sabot_sql/common/types/vector_cache.hpp"
...
static void ReleaseSabotSQLArrowSchema(ArrowSchema *schema) {
	if (!schema || !schema->release) {
		return;
	}
	schema->release = nullptr;
	auto holder = static_cast<SabotSQLArrowSchemaHolder *>(schema->private_data);
	schema->private_data = nullptr;

	delete holder;
}

void InitializeChild(ArrowSchema &child, SabotSQLArrowSchemaHolder &root_holder, const string &name = "") {
	//! Child is cleaned up by parent
	child.private_data = nullptr;
	child.release = ReleaseSabotSQLArrowSchema;
...
void SetArrowFormat(SabotSQLArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
                    ClientProperties &options, ClientContext &context);

void SetArrowStructFormat(SabotSQLArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
...
void SetArrowMapFormat(SabotSQLArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
                       ClientProperties &options, ClientContext &context) {
	child.format = "+m";
...
bool SetArrowExtension(SabotSQLArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
	...
	case LogicalTypeId::DECIMAL: {
		...
		if (options.arrow_output_version <= ArrowFormatVersion::V1_4) {
			...
			case PhysicalType::INT128:
				...
				break;
			default:
				throw NotImplementedException("Unsupported internal type For SABOT_SQL Decimal -> Arrow ");
			}
		}
...
void ArrowConverter::ToArrowSchema(ArrowSchema *out_schema, const vector<LogicalType> &types,
	...
	auto root_holder = make_uniq<SabotSQLArrowSchemaHolder>();
	...
	out_schema->name = "sabot_sql_query_result";
	...
	out_schema->release = ReleaseSabotSQLArrowSchema;
}
