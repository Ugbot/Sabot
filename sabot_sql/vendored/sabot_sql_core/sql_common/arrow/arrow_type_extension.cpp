...
#include "sabot_sql/common/arrow/schema_metadata.hpp"
...
ArrowTypeExtension::ArrowTypeExtension(string vendor_name, string type_name,
                                       ...
                                       shared_ptr<ArrowTypeExtensionData> type, cast_arrow_duck_t arrow_to_sabot_sql,
                                       cast_duck_arrow_t sabot_sql_to_arrow)
    : populate_arrow_schema(populate_arrow_schema), get_type(get_type),
      extension_metadata(ArrowExtensionMetadata::ARROW_EXTENSION_NON_CANONICAL, std::move(vendor_name),
                         std::move(type_name), {}),
      type_extension(std::move(type)) {
	type_extension->arrow_to_sabot_sql = arrow_to_sabot_sql;
	type_extension->sabot_sql_to_arrow = sabot_sql_to_arrow;
}
...
unique_ptr<ArrowType> ArrowTypeExtension::GetType(const ArrowSchema &schema,
	...
	auto sabot_sql_type = type_extension->GetSabotSQLType();
	return make_uniq<ArrowType>(sabot_sql_type);
}
...
LogicalTypeId ArrowTypeExtension::GetLogicalTypeId() const {
	return type_extension->GetSabotSQLType().id();
}

LogicalType ArrowTypeExtension::GetLogicalType() const {
	return type_extension->GetSabotSQLType();
}
...
void ArrowTypeExtension::PopulateArrowSchema(SabotSQLArrowSchemaHolder &root_holder, ArrowSchema &child,
                                             const LogicalType &sabot_sql_type, ClientContext &context,
                                             const ArrowTypeExtension &extension) {
	if (extension.populate_arrow_schema) {
		extension.populate_arrow_schema(root_holder, child, sabot_sql_type, context, extension);
		return;
	}
...
struct ArrowJson {
	...
	static void PopulateSchema(SabotSQLArrowSchemaHolder &root_holder, ArrowSchema &schema, const LogicalType &type,
...
void ArrowTypeExtensionSet::Initialize(const DBConfig &config) {
	...
	config.RegisterArrowExtension(
	    {"SabotSQL", "hugeint", "w:16", make_shared_ptr<ArrowTypeExtensionData>(LogicalType::HUGEINT)});
	config.RegisterArrowExtension(
	    {"SabotSQL", "uhugeint", "w:16", make_shared_ptr<ArrowTypeExtensionData>(LogicalType::UHUGEINT)});
	config.RegisterArrowExtension(
	    {"SabotSQL", "time_tz", "w:8", make_shared_ptr<ArrowTypeExtensionData>(LogicalType::TIME_TZ)});
	...
	config.RegisterArrowExtension({"SabotSQL", "bit", &ArrowBit::PopulateSchema, &ArrowBit::GetType,
	                               make_shared_ptr<ArrowTypeExtensionData>(LogicalType::BIT), nullptr, nullptr});

	config.RegisterArrowExtension({"SabotSQL", "bignum", &ArrowBignum::PopulateSchema, &ArrowBignum::GetType,
