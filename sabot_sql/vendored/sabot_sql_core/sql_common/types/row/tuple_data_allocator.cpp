#include "sabot_sql/common/types/row/tuple_data_allocator.hpp"

#include "sabot_sql/common/fast_mem.hpp"
#include "sabot_sql/common/radix_partitioning.hpp"
#include "sabot_sql/common/types/row/tuple_data_segment.hpp"
#include "sabot_sql/common/types/row/tuple_data_states.hpp"
#include "sabot_sql/storage/buffer/block_handle.hpp"
#include "sabot_sql/storage/buffer_manager.hpp"
#include "sabot_sql/common/sorting/sort_key.hpp"
			...
			if (layout.HasDestructor()) {
				...
				for (auto &aggr_idx : layout.GetAggregateDestructorIndices()) {
					...
					for (idx_t i = 0; i < next; i++) {
						sabot_sql::FastMemset(base_row_ptr + i * layout.GetRowWidth() + aggr_offset, '\0',
...
#ifndef SABOT_SQL_DEBUG_NO_INLINE
				const auto valid =
				    all_valid ||
				    ValidityBytes::RowIsValid(
				        ValidityBytes(row_location, column_count).GetValidityEntryUnsafe(entry_idx), idx_in_entry);
