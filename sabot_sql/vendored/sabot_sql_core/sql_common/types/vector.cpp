...
#include "sabot_sql/common/types/value_map.hpp"
#include "sabot_sql/common/types/bignum.hpp"
#include "sabot_sql/function/scalar/variant_utils.hpp"
...
#include "sabot_sql/storage/string_uncompressed.hpp"
...
Value Vector::GetValueInternal(const Vector &v_p, idx_t index_p) {
	...
	case LogicalTypeId::MAP: {
		...
		sabot_sql::vector<Value> children;
...
void FSSTVector::RegisterDecoder(Vector &vector, buffer_ptr<void> &sabot_sql_fsst_decoder,
	...
	auto &fsst_string_buffer = vector.auxiliary->Cast<VectorFSSTStringBuffer>();
	fsst_string_buffer.AddDecoder(sabot_sql_fsst_decoder, string_block_limit);
}
...
UnionInvalidReason UnionVector::CheckUnionValidity(Vector &vector_p, idx_t count, const SelectionVector &sel_p) {
	...
	sabot_sql::vector<UnifiedVectorFormat> child_vdata(entries.size());
