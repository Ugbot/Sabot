...
hash_t FunctionExpression::Hash() const {
	...
	result = CombineHash(result, sabot_sql::Hash<const char *>(schema.c_str()));
	result = CombineHash(result, sabot_sql::Hash<const char *>(function_name.c_str()));
	result = CombineHash(result, sabot_sql::Hash<bool>(distinct));
	result = CombineHash(result, sabot_sql::Hash<bool>(export_state));
	return result;
}
