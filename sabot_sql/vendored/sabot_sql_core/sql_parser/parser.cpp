...
void Parser::ParseQuery(const string &query) {
	...
	}
	{
		...
		// If SabotSQL fails to parse the entire sql string, break the string down into individual statements
		...
		if (parsing_succeed) {
			...
			for (auto const &query_statement : queries) {
				...
				// Creating a new scope to allow extensions to use PostgresParser, which is not reentrant
				{
					...
					// LCOV_EXCL_START
					// first see if SabotSQL can parse this individual query statement
				...
				// LCOV_EXCL_START
				// let extensions parse the statement which SabotSQL failed to parse
...
vector<SimplifiedToken> Parser::Tokenize(const string &query) {
	...
	for (auto &pg_token : pg_tokens) {
		...
		case sabot_sql_libpgquery::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_IDENTIFIER:
			token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_IDENTIFIER;
			break;
		case sabot_sql_libpgquery::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_NUMERIC_CONSTANT:
			token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_NUMERIC_CONSTANT;
			break;
		case sabot_sql_libpgquery::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_STRING_CONSTANT:
			token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_STRING_CONSTANT;
			break;
		case sabot_sql_libpgquery::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_OPERATOR:
			token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_OPERATOR;
			break;
		case sabot_sql_libpgquery::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_KEYWORD:
			token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_KEYWORD;
			break;
		// comments are not supported by our tokenizer right now
		case sabot_sql_libpgquery::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_COMMENT: // LCOV_EXCL_START
...
KeywordCategory ToKeywordCategory(sabot_sql_libpgquery::PGKeywordCategory type) {
	switch (type) {
	case sabot_sql_libpgquery::PGKeywordCategory::PG_KEYWORD_RESERVED:
		return KeywordCategory::KEYWORD_RESERVED;
	case sabot_sql_libpgquery::PGKeywordCategory::PG_KEYWORD_UNRESERVED:
		return KeywordCategory::KEYWORD_UNRESERVED;
	case sabot_sql_libpgquery::PGKeywordCategory::PG_KEYWORD_TYPE_FUNC:
		return KeywordCategory::KEYWORD_TYPE_FUNC;
	case sabot_sql_libpgquery::PGKeywordCategory::PG_KEYWORD_COL_NAME:
		return KeywordCategory::KEYWORD_COL_NAME;
	case sabot_sql_libpgquery::PGKeywordCategory::PG_KEYWORD_NONE:
