...
void Transformer::TransformWindowDef(sabot_sql_libpgquery::PGWindowDef &window_spec, WindowExpression &expr,
...
void Transformer::TransformWindowFrame(sabot_sql_libpgquery::PGWindowDef &window_spec, WindowExpression &expr) {
...
unique_ptr<ParsedExpression> Transformer::TransformFuncCall(sabot_sql_libpgquery::PGFuncCall &root) {
	auto name = root.funcname;
	string catalog, schema, function_name;
	if (name->length == 3) {
		// catalog + schema + name
		catalog = PGPointerCast<sabot_sql_libpgquery::PGValue>(name->head->data.ptr_value)->val.str;
		schema = PGPointerCast<sabot_sql_libpgquery::PGValue>(name->head->next->data.ptr_value)->val.str;
		function_name = PGPointerCast<sabot_sql_libpgquery::PGValue>(name->head->next->next->data.ptr_value)->val.str;
	} else if (name->length == 2) {
		// schema + name
		catalog = INVALID_CATALOG;
		schema = PGPointerCast<sabot_sql_libpgquery::PGValue>(name->head->data.ptr_value)->val.str;
		function_name = PGPointerCast<sabot_sql_libpgquery::PGValue>(name->head->next->data.ptr_value)->val.str;
	} else if (name->length == 1) {
		// unqualified name
		catalog = INVALID_CATALOG;
		schema = INVALID_SCHEMA;
		function_name = PGPointerCast<sabot_sql_libpgquery::PGValue>(name->head->data.ptr_value)->val.str;
	} else {
	...
	if (root.over) {
		...
		if (win_fun_type == ExpressionType::WINDOW_AGGREGATE &&
		    root.agg_ignore_nulls != sabot_sql_libpgquery::PG_DEFAULT_NULLS) {
		...
		expr->ignore_nulls = (root.agg_ignore_nulls == sabot_sql_libpgquery::PG_IGNORE_NULLS);
		...
		auto window_spec = PGPointerCast<sabot_sql_libpgquery::PGWindowDef>(root.over);
		if (window_spec->name) {
	...
	if (root.agg_ignore_nulls != sabot_sql_libpgquery::PG_DEFAULT_NULLS) {
...
unique_ptr<ParsedExpression> Transformer::TransformSQLValueFunction(sabot_sql_libpgquery::PGSQLValueFunction &node) {
