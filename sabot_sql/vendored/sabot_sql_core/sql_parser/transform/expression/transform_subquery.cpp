...
unique_ptr<ParsedExpression> Transformer::TransformSubquery(sabot_sql_libpgquery::PGSubLink &root) {
	...
	case sabot_sql_libpgquery::PG_EXISTS_SUBLINK: {
		subquery_expr->subquery_type = SubqueryType::EXISTS;
		break;
	}
	case sabot_sql_libpgquery::PG_ANY_SUBLINK:
	case sabot_sql_libpgquery::PG_ALL_SUBLINK: {
		...
		if (!root.operName) {
			...
			auto operator_name =
			    string((PGPointerCast<sabot_sql_libpgquery::PGValue>(root.operName->head->data.ptr_value))->val.str);
		...
		if (root.subLinkType == sabot_sql_libpgquery::PG_ALL_SUBLINK) {
	...
	case sabot_sql_libpgquery::PG_EXPR_SUBLINK: {
	...
	case sabot_sql_libpgquery::PG_ARRAY_SUBLINK: {
		// ARRAY expression
		// wrap subquery into
