...
unique_ptr<ParsedExpression> Transformer::TransformInterval(sabot_sql_libpgquery::PGIntervalConstant &node) {
	...
	int32_t mask = NumericCast<int32_t>(
	    PGPointerCast<sabot_sql_libpgquery::PGAConst>(node.typmods->head->data.ptr_value)->val.val.ival);
