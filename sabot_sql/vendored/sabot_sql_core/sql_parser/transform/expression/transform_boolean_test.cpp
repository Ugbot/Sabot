...
unique_ptr<ParsedExpression> Transformer::TransformBooleanTest(sabot_sql_libpgquery::PGBooleanTest &node) {
	auto argument = TransformExpression(PGPointerCast<sabot_sql_libpgquery::PGNode>(node.arg));

	switch (node.booltesttype) {
	case sabot_sql_libpgquery::PGBoolTestType::PG_IS_TRUE:
		return TransformBooleanTestInternal(std::move(argument), ExpressionType::COMPARE_NOT_DISTINCT_FROM, true,
		                                    node.location);
	case sabot_sql_libpgquery::PGBoolTestType::IS_NOT_TRUE:
		return TransformBooleanTestInternal(std::move(argument), ExpressionType::COMPARE_DISTINCT_FROM, true,
		                                    node.location);
	case sabot_sql_libpgquery::PGBoolTestType::IS_FALSE:
		return TransformBooleanTestInternal(std::move(argument), ExpressionType::COMPARE_NOT_DISTINCT_FROM, false,
		                                    node.location);
	case sabot_sql_libpgquery::PGBoolTestType::IS_NOT_FALSE:
		return TransformBooleanTestInternal(std::move(argument), ExpressionType::COMPARE_DISTINCT_FROM, false,
		                                    node.location);
	case sabot_sql_libpgquery::PGBoolTestType::IS_UNKNOWN: // IS NULL
		return TransformBooleanTestIsNull(std::move(argument), ExpressionType::OPERATOR_IS_NULL, node.location);
	case sabot_sql_libpgquery::PGBoolTestType::IS_NOT_UNKNOWN: // IS NOT NULL
