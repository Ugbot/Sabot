...
unique_ptr<ParsedExpression> Transformer::TransformBoolExpr(sabot_sql_libpgquery::PGBoolExpr &root) {
	unique_ptr<ParsedExpression> result;
	for (auto node = root.args->head; node != nullptr; node = node->next) {
		auto next = TransformExpression(PGPointerCast<sabot_sql_libpgquery::PGNode>(node->data.ptr_value));

		switch (root.boolop) {
		case sabot_sql_libpgquery::PG_AND_EXPR: {
			if (!result) {
		...
		case sabot_sql_libpgquery::PG_OR_EXPR: {
			if (!result) {
				result = std::move(next);
			} else {
		...
		case sabot_sql_libpgquery::PG_NOT_EXPR: {
