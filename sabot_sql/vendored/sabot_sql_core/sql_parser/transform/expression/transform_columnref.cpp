...
QualifiedColumnName TransformQualifiedColumnName(sabot_sql_libpgquery::PGList &list) {
...
unique_ptr<ParsedExpression> Transformer::TransformStarExpression(sabot_sql_libpgquery::PGAStar &star) {
	...
	if (star.except_list) {
		for (auto head = star.except_list->head; head; head = head->next) {
			auto exclude_column_list = PGPointerCast<sabot_sql_libpgquery::PGList>(head->data.ptr_value);
	...
	if (star.replace_list) {
		for (auto head = star.replace_list->head; head; head = head->next) {
			auto list = PGPointerCast<sabot_sql_libpgquery::PGList>(head->data.ptr_value);
			D_ASSERT(list->length == 2);
			auto replace_expression =
			    TransformExpression(PGPointerCast<sabot_sql_libpgquery::PGNode>(list->head->data.ptr_value));
			auto value = PGPointerCast<sabot_sql_libpgquery::PGValue>(list->tail->data.ptr_value);
			D_ASSERT(value->type == sabot_sql_libpgquery::T_PGString);
	...
	if (star.rename_list) {
		for (auto head = star.rename_list->head; head; head = head->next) {
			auto list = PGPointerCast<sabot_sql_libpgquery::PGList>(head->data.ptr_value);
			D_ASSERT(list->length == 2);
			auto rename_column_list = PGPointerCast<sabot_sql_libpgquery::PGList>(list->head->data.ptr_value);
...
unique_ptr<ParsedExpression> Transformer::TransformColumnRef(sabot_sql_libpgquery::PGColumnRef &root) {
	auto fields = root.fields;
	auto head_node = PGPointerCast<sabot_sql_libpgquery::PGNode>(fields->head->data.ptr_value);
	...
	case sabot_sql_libpgquery::T_PGString: {
		...
		for (auto node = fields->head; node; node = node->next) {
			column_names.emplace_back(PGPointerCast<sabot_sql_libpgquery::PGValue>(node->data.ptr_value)->val.str);
		}
	...
	case sabot_sql_libpgquery::T_PGAStar: {
		return TransformStarExpression(PGCast<sabot_sql_libpgquery::PGAStar>(*head_node));
	}
	default:
