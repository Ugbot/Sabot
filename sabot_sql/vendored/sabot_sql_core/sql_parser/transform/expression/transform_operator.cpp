...
unique_ptr<ParsedExpression> Transformer::TransformInExpression(const string &name, sabot_sql_libpgquery::PGAExpr &root) {
	...
	if (root.rexpr->type == sabot_sql_libpgquery::T_PGList) {
		auto result = make_uniq<OperatorExpression>(operator_type, std::move(left_expr));
		TransformExpressionList(*PGPointerCast<sabot_sql_libpgquery::PGList>(root.rexpr), result->children);
		return std::move(result);
	}
...
unique_ptr<ParsedExpression> Transformer::TransformAExprInternal(sabot_sql_libpgquery::PGAExpr &root) {
	auto name = string(PGPointerCast<sabot_sql_libpgquery::PGValue>(root.name->head->data.ptr_value)->val.str);

	switch (root.kind) {
	case sabot_sql_libpgquery::PG_AEXPR_OP_ALL:
	case sabot_sql_libpgquery::PG_AEXPR_OP_ANY: {
		// left=ANY(right)
		...
		if (root.kind == sabot_sql_libpgquery::PG_AEXPR_OP_ALL) {
	...
	case sabot_sql_libpgquery::PG_AEXPR_BETWEEN:
	case sabot_sql_libpgquery::PG_AEXPR_NOT_BETWEEN: {
		auto between_args = PGPointerCast<sabot_sql_libpgquery::PGList>(root.rexpr);
		...
		auto between_left =
		    TransformExpression(PGPointerCast<sabot_sql_libpgquery::PGNode>(between_args->head->data.ptr_value));
		auto between_right =
		    TransformExpression(PGPointerCast<sabot_sql_libpgquery::PGNode>(between_args->tail->data.ptr_value));

		auto compare_between =
		    make_uniq<BetweenExpression>(std::move(input), std::move(between_left), std::move(between_right));
		if (root.kind == sabot_sql_libpgquery::PG_AEXPR_BETWEEN) {
	...
	case sabot_sql_libpgquery::PG_AEXPR_SIMILAR: {
		auto left_expr = TransformExpression(root.lexpr);
		auto right_expr = TransformExpression(root.rexpr);
...
unique_ptr<ParsedExpression> Transformer::TransformAExpr(sabot_sql_libpgquery::PGAExpr &root) {
