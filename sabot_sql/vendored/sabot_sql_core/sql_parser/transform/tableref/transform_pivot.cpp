...
PivotColumn Transformer::TransformPivotColumn(sabot_sql_libpgquery::PGPivot &pivot, bool is_pivot) {
	PivotColumn col;
...
vector<PivotColumn> Transformer::TransformPivotList(sabot_sql_libpgquery::PGList &list, bool is_pivot) {
	vector<PivotColumn> result;
	for (auto node = list.head; node != nullptr; node = node->next) {
		auto pivot = PGPointerCast<sabot_sql_libpgquery::PGPivot>(node->data.ptr_value);
		result.push_back(TransformPivotColumn(*pivot, is_pivot));
	}
	return result;
}

unique_ptr<TableRef> Transformer::TransformPivot(sabot_sql_libpgquery::PGPivotExpr &root) {
