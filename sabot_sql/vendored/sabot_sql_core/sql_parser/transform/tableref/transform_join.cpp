...
vector<string> Transformer::TransformUsingClause(sabot_sql_libpgquery::PGList &usingClause) {
	...
	for (auto node = usingClause.head; node != nullptr; node = node->next) {
		...
		D_ASSERT(target->type == sabot_sql_libpgquery::T_PGString);
...
unique_ptr<TableRef> Transformer::TransformJoin(sabot_sql_libpgquery::PGJoinExpr &root) {
	auto result = make_uniq<JoinRef>(JoinRefType::REGULAR);
	switch (root.jointype) {
	case sabot_sql_libpgquery::PG_JOIN_INNER: {
		result->type = JoinType::INNER;
		break;
	}
	case sabot_sql_libpgquery::PG_JOIN_LEFT: {
		result->type = JoinType::LEFT;
		break;
	}
	case sabot_sql_libpgquery::PG_JOIN_FULL: {
		result->type = JoinType::OUTER;
		break;
	}
	case sabot_sql_libpgquery::PG_JOIN_RIGHT: {
		result->type = JoinType::RIGHT;
		break;
	}
	case sabot_sql_libpgquery::PG_JOIN_SEMI: {
		result->type = JoinType::SEMI;
		break;
	}
	case sabot_sql_libpgquery::PG_JOIN_ANTI: {
		result->type = JoinType::ANTI;
		break;
	}
	case sabot_sql_libpgquery::PG_JOIN_POSITION: {
	...
	case sabot_sql_libpgquery::PG_JOIN_NATURAL:
		result->ref_type = JoinRefType::NATURAL;
		break;
	case sabot_sql_libpgquery::PG_JOIN_ASOF:
