...
unique_ptr<ExplainStatement> Transformer::TransformExplain(sabot_sql_libpgquery::PGExplainStmt &stmt) {
	...
	if (stmt.options) {
		for (auto n = stmt.options->head; n; n = n->next) {
			auto def_elem = PGPointerCast<sabot_sql_libpgquery::PGDefElem>(n->data.ptr_value);
			...
			if (elem == "analyze") {
				...
				if (def_elem->arg) {
					...
					auto val = TransformValue(*PGPointerCast<sabot_sql_libpgquery::PGValue>(def_elem->arg))->value;
					format_is_set = true;
