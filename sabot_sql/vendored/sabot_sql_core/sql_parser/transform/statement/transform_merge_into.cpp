...
unique_ptr<MergeIntoAction> Transformer::TransformMergeIntoAction(sabot_sql_libpgquery::PGMatchAction &action) {
	auto result = make_uniq<MergeIntoAction>();
	if (action.andClause) {
		result->condition = TransformExpression(action.andClause);
	}
	switch (action.actionType) {
	case sabot_sql_libpgquery::MERGE_ACTION_TYPE_UPDATE:
		...
		break;
	case sabot_sql_libpgquery::MERGE_ACTION_TYPE_DELETE:
		result->action_type = MergeActionType::MERGE_DELETE;
		break;
	case sabot_sql_libpgquery::MERGE_ACTION_TYPE_INSERT:
		...
		break;
	case sabot_sql_libpgquery::MERGE_ACTION_TYPE_DO_NOTHING:
		result->action_type = MergeActionType::MERGE_DO_NOTHING;
		break;
	case sabot_sql_libpgquery::MERGE_ACTION_TYPE_ERROR:
...
unique_ptr<SQLStatement> Transformer::TransformMergeInto(sabot_sql_libpgquery::PGMergeIntoStmt &stmt) {
	...
	for (auto cell = stmt.matchActions->head; cell; cell = cell->next) {
		auto match_action = PGPointerCast<sabot_sql_libpgquery::PGMatchAction>(cell->data.ptr_value);
		auto action = TransformMergeIntoAction(*match_action);
		MergeActionCondition action_condition;
		switch (match_action->when) {
		case sabot_sql_libpgquery::MERGE_ACTION_WHEN_MATCHED:
			action_condition = MergeActionCondition::WHEN_MATCHED;
			break;
		case sabot_sql_libpgquery::MERGE_ACTION_WHEN_NOT_MATCHED_BY_SOURCE:
			action_condition = MergeActionCondition::WHEN_NOT_MATCHED_BY_SOURCE;
			break;
		case sabot_sql_libpgquery::MERGE_ACTION_WHEN_NOT_MATCHED_BY_TARGET:
