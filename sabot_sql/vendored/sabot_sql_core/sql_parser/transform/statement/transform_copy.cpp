...
void Transformer::TransformCopyOptions(CopyInfo &info, optional_ptr<sabot_sql_libpgquery::PGList> options) {
	if (!options) {
		return;
	}
...
unique_ptr<CopyStatement> Transformer::TransformCopy(sabot_sql_libpgquery::PGCopyStmt &stmt) {
	...
	if (stmt.attlist) {
		for (auto n = stmt.attlist->head; n != nullptr; n = n->next) {
			auto target = PGPointerCast<sabot_sql_libpgquery::PGResTarget>(n->data.ptr_value);
			if (target->name) {
