...
unique_ptr<QueryNode> Transformer::TransformPivotStatement(sabot_sql_libpgquery::PGSelectStmt &select) {
	...
	if (select.withClause) {
		TransformCTE(*PGPointerCast<sabot_sql_libpgquery::PGWithClause>(select.withClause), select_node->cte_map);
	}
	if (!pivot->columns) {
