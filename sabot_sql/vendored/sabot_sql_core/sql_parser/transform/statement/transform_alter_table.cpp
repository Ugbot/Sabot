...
vector<string> Transformer::TransformNameList(sabot_sql_libpgquery::PGList &list) {
...
unique_ptr<AlterStatement> Transformer::TransformAlter(sabot_sql_libpgquery::PGAlterTableStmt &stmt) {
	...
	for (auto c = stmt.cmds->head; c != nullptr; c = c->next) {
		...
		auto command = PGPointerCast<sabot_sql_libpgquery::PGAlterTableCmd>(c->data.ptr_value);
		...
		case sabot_sql_libpgquery::PG_AT_AddColumn: {
			auto column_name_list = PGPointerCast<sabot_sql_libpgquery::PGList>(command->def_list->head->data.ptr_value);
			auto column_def = PGPointerCast<sabot_sql_libpgquery::PGColumnDef>(command->def_list->tail->data.ptr_value);
			if (stmt.relkind != sabot_sql_libpgquery::PG_OBJECT_TABLE) {
				throw ParserException("Adding columns is only supported for tables");
			}
			if (column_def->category == sabot_sql_libpgquery::COL_GENERATED) {
		...
		case sabot_sql_libpgquery::PG_AT_DropColumn: {
			auto cascade = command->behavior == sabot_sql_libpgquery::PG_DROP_CASCADE;
			if (stmt.relkind != sabot_sql_libpgquery::PG_OBJECT_TABLE) {
		...
		case sabot_sql_libpgquery::PG_AT_ColumnDefault: {
		...
		case sabot_sql_libpgquery::PG_AT_AlterColumnType: {
			auto column_def = PGPointerCast<sabot_sql_libpgquery::PGColumnDef>(command->def);
			auto column_entry = TransformColumnDefinition(*column_def);
		...
		case sabot_sql_libpgquery::PG_AT_SetNotNull: {
			result->info = make_uniq<SetNotNullInfo>(std::move(data), command->name);
			break;
		}
		case sabot_sql_libpgquery::PG_AT_DropNotNull: {
			result->info = make_uniq<DropNotNullInfo>(std::move(data), command->name);
			break;
		}
		case sabot_sql_libpgquery::PG_AT_AddConstraint: {
			auto pg_constraint = PGCast<sabot_sql_libpgquery::PGConstraint>(*command->def);
			if (pg_constraint.contype != sabot_sql_libpgquery::PGConstrType::PG_CONSTR_PRIMARY) {
		...
		case sabot_sql_libpgquery::PG_AT_SetPartitionedBy: {
		...
		case sabot_sql_libpgquery::PG_AT_SetSortedBy: {
