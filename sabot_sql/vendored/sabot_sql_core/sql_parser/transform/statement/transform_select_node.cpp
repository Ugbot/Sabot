...
#include "sabot_sql/parser/query_node/set_operation_node.hpp"
...
void Transformer::TransformModifiers(sabot_sql_libpgquery::PGSelectStmt &stmt, QueryNode &node) {
	...
	if (stmt.limitCount || stmt.limitOffset) {
		if (stmt.limitCount && stmt.limitCount->type == sabot_sql_libpgquery::T_PGLimitPercent) {
			auto limit_percent_modifier = make_uniq<LimitPercentModifier>();
			auto expr_node = PGPointerCast<sabot_sql_libpgquery::PGLimitPercent>(stmt.limitCount)->limit_percent;
...
bool Transformer::SetOperationsMatch(sabot_sql_libpgquery::PGSelectStmt &root, sabot_sql_libpgquery::PGNode &node) {
	if (node.type != sabot_sql_libpgquery::T_PGSelectStmt) {
		// not a select or set-op - set operations cannot match
		return false;
	}
	auto &stmt = PGCast<sabot_sql_libpgquery::PGSelectStmt>(node);
	if (root.op != stmt.op || root.all != stmt.all) {
		// set operation type does not match
		return false;
	}
	if (root.op != sabot_sql_libpgquery::PG_SETOP_UNION && root.op != sabot_sql_libpgquery::PG_SETOP_UNION_BY_NAME) {
...
void Transformer::TransformSetOperationChildren(sabot_sql_libpgquery::PGSelectStmt &stmt, SetOperationNode &result) {
	D_ASSERT(stmt.larg && stmt.rarg);
	vector<reference<sabot_sql_libpgquery::PGNode>> set_operations;
	...
	for (idx_t i = 0; i < set_operations.size(); i++) {
		...
		if (!SetOperationsMatch(stmt, node)) {
			...
			auto &select = PGCast<sabot_sql_libpgquery::PGSelectStmt>(node);
...
unique_ptr<QueryNode> Transformer::TransformSelectInternal(sabot_sql_libpgquery::PGSelectStmt &stmt) {
	D_ASSERT(stmt.type == sabot_sql_libpgquery::T_PGSelectStmt);
	auto stack_checker = StackCheck();

	unique_ptr<QueryNode> node;

	switch (stmt.op) {
	case sabot_sql_libpgquery::PG_SETOP_NONE: {
		node = make_uniq<SelectNode>();
		auto &result = node->Cast<SelectNode>();
		if (stmt.withClause) {
			TransformCTE(*PGPointerCast<sabot_sql_libpgquery::PGWithClause>(stmt.withClause), node->cte_map);
		}
		if (stmt.windowClause) {
			for (auto window_ele = stmt.windowClause->head; window_ele != nullptr; window_ele = window_ele->next) {
				auto window_def = PGPointerCast<sabot_sql_libpgquery::PGWindowDef>(window_ele->data.ptr_value);
				D_ASSERT(window_def);
		...
		if (stmt.distinctClause != nullptr) {
			...
			auto target = PGPointerCast<sabot_sql_libpgquery::PGNode>(stmt.distinctClause->head->data.ptr_value);
			if (target) {
	...
	case sabot_sql_libpgquery::PG_SETOP_UNION_BY_NAME: {
