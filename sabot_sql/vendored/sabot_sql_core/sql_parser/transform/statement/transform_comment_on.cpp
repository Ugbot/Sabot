...
unique_ptr<AlterStatement> Transformer::TransformCommentOn(sabot_sql_libpgquery::PGCommentOnStmt &stmt) {
	QualifiedName qualified_name;
	string column_name;

	if (stmt.object_type != sabot_sql_libpgquery::PG_OBJECT_COLUMN) {
	...
	} else if (stmt.object_type == sabot_sql_libpgquery::PG_OBJECT_COLUMN) {
	...
	} else if (stmt.object_type == sabot_sql_libpgquery::PG_OBJECT_DATABASE) {
		throw NotImplementedException("Adding comments to databases is not implemented");
	} else if (stmt.object_type == sabot_sql_libpgquery::PG_OBJECT_SCHEMA) {
