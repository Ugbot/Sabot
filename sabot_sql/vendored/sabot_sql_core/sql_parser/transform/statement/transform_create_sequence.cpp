...
unique_ptr<CreateStatement> Transformer::TransformCreateSequence(sabot_sql_libpgquery::PGCreateSeqStmt &stmt) {
	...
	if (stmt.options) {
		...
		sabot_sql_libpgquery::PGListCell *cell = nullptr;
		for_each_cell(cell, stmt.options->head) {
			...
			auto val = PGPointerCast<sabot_sql_libpgquery::PGValue>(def_elem->arg);
			bool nodef = def_elem->defaction == sabot_sql_libpgquery::PG_DEFELEM_UNSPEC && !val; // e.g. NO MINVALUE
			int64_t opt_value = 0;

			if (val) {
				if (val->type == sabot_sql_libpgquery::T_PGInteger) {
					opt_value = val->val.ival;
				} else if (val->type == sabot_sql_libpgquery::T_PGFloat) {
