...
vector<unique_ptr<ParsedExpression>> Transformer::TransformIndexParameters(sabot_sql_libpgquery::PGList &list,
...
unique_ptr<CreateStatement> Transformer::TransformCreateIndex(sabot_sql_libpgquery::PGIndexStmt &stmt) {
	...
	if (stmt.options) {
		...
		for_each_cell(cell, stmt.options->head) {
			...
			Value val;
			if (def_elem->arg) {
				val = TransformValue(*PGPointerCast<sabot_sql_libpgquery::PGValue>(def_elem->arg))->value;
			} else {
