...
unique_ptr<SampleOptions> Transformer::TransformSampleOptions(optional_ptr<sabot_sql_libpgquery::PGNode> options) {
	if (!options) {
		return nullptr;
	}
	auto result = make_uniq<SampleOptions>();
	auto &sample_options = PGCast<sabot_sql_libpgquery::PGSampleOptions>(*options);
	auto &sample_size = *PGPointerCast<sabot_sql_libpgquery::PGSampleSize>(sample_options.sample_size);
