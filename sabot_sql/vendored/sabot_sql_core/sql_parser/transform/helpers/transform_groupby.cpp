...
void Transformer::TransformGroupByExpression(sabot_sql_libpgquery::PGNode &n, GroupingExpressionMap &map,
...
void Transformer::TransformGroupByNode(sabot_sql_libpgquery::PGNode &n, GroupingExpressionMap &map, SelectNode &result,
                                       vector<GroupingSet> &result_sets) {
	if (n.type == sabot_sql_libpgquery::T_PGGroupingSet) {
		auto &grouping_set = PGCast<sabot_sql_libpgquery::PGGroupingSet>(n);
		switch (grouping_set.kind) {
		case sabot_sql_libpgquery::GROUPING_SET_EMPTY:
			result_sets.emplace_back();
			break;
		case sabot_sql_libpgquery::GROUPING_SET_ALL: {
			result.aggregate_handling = AggregateHandling::FORCE_AGGREGATES;
			break;
		}
		case sabot_sql_libpgquery::GROUPING_SET_SETS: {
			for (auto node = grouping_set.content->head; node; node = node->next) {
				auto pg_node = PGPointerCast<sabot_sql_libpgquery::PGNode>(node->data.ptr_value);
				TransformGroupByNode(*pg_node, map, result, result_sets);
			}
			break;
		}
		case sabot_sql_libpgquery::GROUPING_SET_ROLLUP: {
		...
		case sabot_sql_libpgquery::GROUPING_SET_CUBE: {
...
bool Transformer::TransformGroupBy(optional_ptr<sabot_sql_libpgquery::PGList> group, SelectNode &select_node) {
	if (!group) {
		return false;
	}
