...
static void ParseSchemaTableNameFK(sabot_sql_libpgquery::PGRangeVar &input, ForeignKeyInfo &fk_info) {
...
TransformForeignKeyConstraint(sabot_sql_libpgquery::PGConstraint &constraint,
	...
	if (override_fk_column) {
		...
		for (auto kc = constraint.fk_attrs->head; kc; kc = kc->next) {
			auto value = Transformer::PGPointerCast<sabot_sql_libpgquery::PGValue>(kc->data.ptr_value);
...
unique_ptr<Constraint> Transformer::TransformConstraint(sabot_sql_libpgquery::PGConstraint &constraint) {
	switch (constraint.contype) {
	case sabot_sql_libpgquery::PG_CONSTR_UNIQUE:
	case sabot_sql_libpgquery::PG_CONSTR_PRIMARY: {
		bool is_primary_key = constraint.contype == sabot_sql_libpgquery::PG_CONSTR_PRIMARY;
		if (!constraint.keys) {
			throw ParserException("UNIQUE USING INDEX is not supported");
		}
		vector<string> columns;
		for (auto kc = constraint.keys->head; kc; kc = kc->next) {
			auto value = PGPointerCast<sabot_sql_libpgquery::PGValue>(kc->data.ptr_value);
			columns.emplace_back(value->val.str);
		}
		return make_uniq<UniqueConstraint>(columns, is_primary_key);
	}
	case sabot_sql_libpgquery::PG_CONSTR_CHECK: {
	...
	case sabot_sql_libpgquery::PG_CONSTR_FOREIGN:
...
unique_ptr<Constraint> Transformer::TransformConstraint(sabot_sql_libpgquery::PGConstraint &constraint,
	...
	switch (constraint.contype) {
	case sabot_sql_libpgquery::PG_CONSTR_NOTNULL:
	...
	case sabot_sql_libpgquery::PG_CONSTR_NULL:
		return nullptr;
	case sabot_sql_libpgquery::PG_CONSTR_GENERATED_VIRTUAL: {
	...
	case sabot_sql_libpgquery::PG_CONSTR_GENERATED_STORED:
		throw InvalidInputException("Can not create a STORED generated column!");
	case sabot_sql_libpgquery::PG_CONSTR_DEFAULT:
		column.SetDefaultValue(TransformExpression(constraint.raw_expr));
		return nullptr;
	case sabot_sql_libpgquery::PG_CONSTR_COMPRESSION:
