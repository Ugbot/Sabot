...
string PragmaShowTables(const string &database, const string &schema) {
	...
	(
		...
		FROM sabot_sql_tables
		)EOF" + where_clause + R"EOF(
	), "views" as
	(
		SELECT view_name as "name"
		FROM sabot_sql_views
		)EOF" + where_clause + R"EOF(
	), db_objects as
	(
...
string PragmaShowTablesExpanded() {
	...
	FROM sabot_sql_tables t
	JOIN sabot_sql_columns c
	USING (table_oid)
...
string PragmaShowDatabases() {
	return "SELECT database_name FROM sabot_sql_databases() WHERE NOT internal ORDER BY database_name;";
}
...
string PragmaShowVariables() {
	return "SELECT * FROM sabot_sql_variables() ORDER BY name";
}
...
static string PragmaExtensionVersions(ClientContext &context, const FunctionParameters &parameters) {
	return "select extension_name, extension_version, install_mode, installed_from from sabot_sql_extensions() where "
	       "installed";
}
