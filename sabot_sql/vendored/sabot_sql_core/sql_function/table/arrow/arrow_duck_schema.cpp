...
const arrow_column_map_t &ArrowTableSchema::GetColumns() const {
	return arrow_convert_data;
}
vector<LogicalType> &ArrowTableSchema::GetTypes() {
	return types;
}

vector<string> &ArrowTableSchema::GetNames() {
	return column_names;
}
...
void ArrowType::SetRunEndEncoded() {
	...
	// Override the sabot_sql type to the actual type
	type = actual_type;
...
unique_ptr<ArrowType> ArrowType::GetTypeFromFormat(DBConfig &config, ArrowSchema &schema, string &format) {
	...
	if (format == "+l") {
		...
		if (schema.n_children == 0) {
			throw InvalidInputException(
			    "Attempted to convert a STRUCT with no fields to SabotSQL which is not supported");
		}
		...
		vector<shared_ptr<ArrowType>> children;
		if (schema.n_children == 0) {
			throw InvalidInputException("Attempted to convert a UNION with no fields to SabotSQL which is not supported");
		}
		for (idx_t type_idx = 0; type_idx < static_cast<idx_t>(schema.n_children); type_idx++) {
...
LogicalType ArrowType::GetDuckType(bool use_dictionary) const {
	...
	if (!use_dictionary) {
		if (extension_data) {
			return extension_data->GetSabotSQLType();
		}
		return type;
	}
...
ArrowTypeExtensionData::GetExtensionTypes(ClientContext &context, const vector<LogicalType> &sabot_sql_types) {
	unordered_map<idx_t, const shared_ptr<ArrowTypeExtensionData>> extension_types;
	const auto &db_config = DBConfig::GetConfig(context);
	for (idx_t i = 0; i < sabot_sql_types.size(); i++) {
		if (db_config.HasArrowExtension(sabot_sql_types[i])) {
			extension_types.insert({i, db_config.GetArrowExtension(sabot_sql_types[i]).GetTypeExtension()});
		}
	}
	return extension_types;
}

LogicalType ArrowTypeExtensionData::GetSabotSQLType() const {
	return sabot_sql_type;
}
