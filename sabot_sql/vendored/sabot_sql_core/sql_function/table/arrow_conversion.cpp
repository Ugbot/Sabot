...
#include "sabot_sql/common/types/arrow_string_view_type.hpp"
#include "sabot_sql/common/types/hugeint.hpp"
#include "sabot_sql/function/scalar/nested_functions.hpp"
#include "sabot_sql/function/table/arrow.hpp"

#include "sabot_sql/common/bswap.hpp"
...
static void GetValidityMask(ValidityMask &mask, ArrowArray &array, idx_t chunk_offset, idx_t size,
	...
	// In certains we don't need to or cannot copy arrow's validity mask to sabot_sql.
	//
	// The conditions where we do want to copy arrow's mask to sabot_sql are:
	// 1. nulls exist
...
void ArrowToSabotSQLConversion::SetValidityMask(Vector &vector, ArrowArray &array, idx_t chunk_offset, idx_t size,
...
static void ArrowToSabotSQLList(Vector &vector, ArrowArray &array, idx_t chunk_offset, ArrowArrayScanState &array_state,
                              idx_t size, const ArrowType &arrow_type, int64_t nested_offset,
                              const ValidityMask *parent_mask, int64_t parent_offset) {
	auto &list_info = arrow_type.GetTypeInfo<ArrowListInfo>();
	ArrowToSabotSQLConversion::SetValidityMask(vector, array, chunk_offset, size, parent_offset, nested_offset);
	...
	ArrowToSabotSQLConversion::SetValidityMask(child_vector, *array.children[0], chunk_offset, list_size, array.offset,
	...
	if (list_size == 0 && start_offset == 0) {
		...
		ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(child_vector, child_array, chunk_offset, child_state, list_size,
		                                             child_type, -1);
		return;
	}
	...
	case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
		...
		ArrowToSabotSQLConversion::ColumnArrowToSabotSQLDictionary(child_vector, child_array, chunk_offset, child_state,
		                                                       list_size, child_type,
		                                                       NumericCast<int64_t>(start_offset));
		break;
	case ArrowArrayPhysicalType::RUN_END_ENCODED:
		ArrowToSabotSQLConversion::ColumnArrowToSabotSQLRunEndEncoded(child_vector, child_array, chunk_offset, child_state,
		                                                          list_size, child_type,
		                                                          NumericCast<int64_t>(start_offset));
		break;
...
static void ArrowToSabotSQLArray(Vector &vector, ArrowArray &array, idx_t chunk_offset, ArrowArrayScanState &array_state,
                               idx_t size, const ArrowType &arrow_type, int64_t nested_offset,
                               const ValidityMask *parent_mask, int64_t parent_offset) {
	...
	ArrowToSabotSQLConversion::SetValidityMask(child_vector, *array.children[0], chunk_offset, child_count, array.offset,
	...
	if (child_count == 0 && child_offset == 0) {
		D_ASSERT(!child_array.dictionary);
		ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(child_vector, child_array, chunk_offset, child_state, child_count,
		                                             child_type, -1);
	} else {
		if (child_array.dictionary) {
			...
			ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(child_vector, child_array, chunk_offset, child_state,
			                                             child_count, child_type, NumericCast<int64_t>(child_offset));
		}
	}
}

static void ArrowToSabotSQLMapVerify(Vector &vector, idx_t count) {
	auto valid_check = MapVector::CheckMapValidity(vector, count);
	switch (valid_check) {
	case MapInvalidReason::VALID:
		break;
	case MapInvalidReason::DUPLICATE_KEY: {
		throw InvalidInputException("Arrow map contains duplicate key, which isn't supported by SabotSQL map type");
	}
	case MapInvalidReason::NULL_KEY: {
		throw InvalidInputException("Arrow map contains NULL as map key, which isn't supported by SabotSQL map type");
	}
	default: {
...
static void SetVectorString(Vector &vector, idx_t size, char *cdata, T *offsets) {
	...
	for (idx_t row_idx = 0; row_idx < size; row_idx++) {
		...
		if (str_len > NumericLimits<uint32_t>::Maximum()) { // LCOV_EXCL_START
			throw ConversionException("SabotSQL does not support Strings over 4GB");
		} // LCOV_EXCL_STOP
...
void ArrowToSabotSQLConversion::ColumnArrowToSabotSQLRunEndEncoded(Vector &vector, const ArrowArray &array,
	...
	if (!run_end_encoding.run_ends) {
		...
		ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(*run_end_encoding.run_ends, run_ends_array, chunk_offset,
		                                             array_state, compressed_size, run_ends_type);
		auto &values = *run_end_encoding.values;
		ArrowToSabotSQLConversion::SetValidityMask(values, values_array, chunk_offset, compressed_size,
		                                         NumericCast<int64_t>(parent_offset), nested_offset);
		ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(values, values_array, chunk_offset, array_state, compressed_size,
		                                             values_type);
	}
...
void ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(Vector &vector, ArrowArray &array, idx_t chunk_offset,
	...
	if (!ignore_extensions && arrow_type.HasExtension()) {
		if (arrow_type.extension_data->arrow_to_sabot_sql) {
			// Convert the storage and then call the cast function
			Vector input_data(arrow_type.extension_data->GetInternalType());
			ColumnArrowToSabotSQL(input_data, array, chunk_offset, array_state, size, arrow_type, nested_offset,
			                    parent_mask, parent_offset, /*ignore_extensions*/ true);
			arrow_type.extension_data->arrow_to_sabot_sql(array_state.context, input_data, vector, size);
			return;
		}
	}
	...
	case LogicalTypeId::LIST: {
		ArrowToSabotSQLList(vector, array, chunk_offset, array_state, size, arrow_type, nested_offset, parent_mask,
		                  NumericCast<int64_t>(parent_offset));
		break;
	}
	case LogicalTypeId::ARRAY: {
		ArrowToSabotSQLArray(vector, array, chunk_offset, array_state, size, arrow_type, nested_offset, parent_mask,
		                   NumericCast<int64_t>(parent_offset));
		break;
	}
	case LogicalTypeId::MAP: {
		ArrowToSabotSQLList(vector, array, chunk_offset, array_state, size, arrow_type, nested_offset, parent_mask,
		                  NumericCast<int64_t>(parent_offset));
		ArrowToSabotSQLMapVerify(vector, size);
		break;
	}
	case LogicalTypeId::STRUCT: {
		...
		for (idx_t child_idx = 0; child_idx < NumericCast<idx_t>(array.n_children); child_idx++) {
			...
			ArrowToSabotSQLConversion::SetValidityMask(child_entry, child_array, chunk_offset, size, array.offset,
			                                         nested_offset);
			...
			switch (array_physical_type) {
			case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
				ArrowToSabotSQLConversion::ColumnArrowToSabotSQLDictionary(
				...
				break;
			case ArrowArrayPhysicalType::RUN_END_ENCODED:
				ColumnArrowToSabotSQLRunEndEncoded(child_entry, child_array, chunk_offset, child_state, size, child_type,
				                                 nested_offset, &struct_validity_mask,
				                                 NumericCast<uint64_t>(array.offset));
				break;
			case ArrowArrayPhysicalType::DEFAULT:
				ColumnArrowToSabotSQL(child_entry, child_array, chunk_offset, child_state, size, child_type,
	...
	case LogicalTypeId::UNION: {
		...
		sabot_sql::vector<Vector> children;
		for (idx_t child_idx = 0; child_idx < NumericCast<idx_t>(array.n_children); child_idx++) {
			...
			ArrowToSabotSQLConversion::SetValidityMask(child, child_array, chunk_offset, size,
			                                         NumericCast<int64_t>(parent_offset), nested_offset);
			auto array_physical_type = child_type.GetPhysicalType();

			switch (array_physical_type) {
			case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
				ArrowToSabotSQLConversion::ColumnArrowToSabotSQLDictionary(child, child_array, chunk_offset, child_state,
				                                                       size, child_type);
				break;
			case ArrowArrayPhysicalType::RUN_END_ENCODED:
				ArrowToSabotSQLConversion::ColumnArrowToSabotSQLRunEndEncoded(child, child_array, chunk_offset, child_state,
				                                                          size, child_type);
				break;
			case ArrowArrayPhysicalType::DEFAULT:
				ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(child, child_array, chunk_offset, child_state, size,
...
static void SetSelectionVectorLoopWithChecks(SelectionVector &sel, data_ptr_t indices_p, idx_t size) {
	...
	for (idx_t row = 0; row < size; row++) {
		if (indices[row] > NumericLimits<uint32_t>::Maximum()) {
			throw ConversionException("SabotSQL only supports indices that fit on an uint32");
		}
...
void ArrowToSabotSQLConversion::ColumnArrowToSabotSQLDictionary(Vector &vector, ArrowArray &array, idx_t chunk_offset,
                                                            ArrowArrayScanState &array_state, idx_t size,
                                                            const ArrowType &arrow_type, int64_t nested_offset,
	...
	if (array_state.CacheOutdated(array.dictionary)) {
		...
		ArrowToSabotSQLConversion::SetValidityMask(*base_vector, *array.dictionary, chunk_offset,
		...
		case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
			ColumnArrowToSabotSQLDictionary(*base_vector, *array.dictionary, chunk_offset, array_state,
			                              NumericCast<idx_t>(array.dictionary->length), dictionary_type);
			break;
		case ArrowArrayPhysicalType::RUN_END_ENCODED:
			ColumnArrowToSabotSQLRunEndEncoded(*base_vector, *array.dictionary, chunk_offset, array_state,
			                                 NumericCast<idx_t>(array.dictionary->length), dictionary_type);
			break;
		case ArrowArrayPhysicalType::DEFAULT:
			ColumnArrowToSabotSQL(*base_vector, *array.dictionary, chunk_offset, array_state,
			                    NumericCast<idx_t>(array.dictionary->length), dictionary_type);
			break;
		default:
			throw NotImplementedException("ArrowArrayPhysicalType not recognized");
...
void ArrowTableFunction::ArrowToSabotSQL(ArrowScanLocalState &scan_state, const arrow_column_map_t &arrow_convert_data,
	...
	for (idx_t idx = 0; idx < output.ColumnCount(); idx++) {
		...
		if (rowid_column_index != COLUMN_IDENTIFIER_ROW_ID) {
			if (col_idx == COLUMN_IDENTIFIER_ROW_ID) {
				...
				// that SabotSQL believes the Arrow array is in.
				col_idx += 1;
				arrow_array_idx += 1;
			}
		} else {
		...
		switch (array_physical_type) {
		case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
			ArrowToSabotSQLConversion::ColumnArrowToSabotSQLDictionary(output.data[idx], array, scan_state.chunk_offset,
			                                                       array_state, output.size(), arrow_type);
			break;
		case ArrowArrayPhysicalType::RUN_END_ENCODED:
			ArrowToSabotSQLConversion::ColumnArrowToSabotSQLRunEndEncoded(output.data[idx], array, scan_state.chunk_offset,
			                                                          array_state, output.size(), arrow_type);
			break;
		case ArrowArrayPhysicalType::DEFAULT:
			ArrowToSabotSQLConversion::SetValidityMask(output.data[idx], array, scan_state.chunk_offset, output.size(),
			                                         parent_array.offset, -1);
			ArrowToSabotSQLConversion::ColumnArrowToSabotSQL(output.data[idx], array, scan_state.chunk_offset, array_state,
