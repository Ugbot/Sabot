...
struct SabotSQLMultiFileInfo : MultiFileReaderInterface {
...
class SabotSQLFileReaderOptions : public BaseFileReaderOptions {
public:
	string schema_name;
	string table_name;
...
struct SabotSQLReadBindData : TableFunctionData {
	unique_ptr<SabotSQLFileReaderOptions> options;
	optional_idx initial_file_cardinality;

	unique_ptr<FunctionData> Copy() const override {
		auto result = make_uniq<SabotSQLReadBindData>();
		result->options = make_uniq<SabotSQLFileReaderOptions>(*options);
...
class SabotSQLReader : public BaseFileReader {
public:
	SabotSQLReader(ClientContext &context, OpenFileInfo file, const SabotSQLFileReaderOptions &options);
	~SabotSQLReader() override;
...
struct SabotSQLReadGlobalState : GlobalTableFunctionState {};

struct SabotSQLReadLocalState : LocalTableFunctionState {
	unique_ptr<LocalTableFunctionState> local_state;
	shared_ptr<AttachedDatabaseWrapper> attached_database;
};

string SabotSQLFileReaderOptions::GetCandidates(const vector<reference<TableCatalogEntry>> &tables) const {
	if (tables.empty()) {
		return string();
	}
...
bool SabotSQLFileReaderOptions::HasSelection() const {
	if (!table_name.empty()) {
		return true;
	}
...
string SabotSQLFileReaderOptions::PrintOptions() const {
	string options;
	if (!schema_name.empty()) {
...
bool SabotSQLFileReaderOptions::Matches(TableCatalogEntry &table) const {
...
SabotSQLReader::SabotSQLReader(ClientContext &context_p, OpenFileInfo file_p, const SabotSQLFileReaderOptions &options)
...
SabotSQLReader::~SabotSQLReader() {
}

AttachedDatabase &SabotSQLReader::GetAttachedDatabase() {
	if (!db_wrapper) {
		auto &db_manager = DatabaseManager::Get(context);
		AttachInfo info;
		info.path = file.path;
		// use invalid UTF-8 so that a conflicting database name cannot be attached by a user
		info.name = "\x80__sabot_sql_reader_" + info.path;
...
TableCatalogEntry &SabotSQLReader::GetTableEntry() {
	auto &attached = GetAttachedDatabase();
...
bool SabotSQLReader::TryInitializeScan(ClientContext &context, GlobalTableFunctionState &gstate,
                                     LocalTableFunctionState &lstate_p) {
	auto &lstate = lstate_p.Cast<SabotSQLReadLocalState>();
	if (finished) {
	...
	if (!global_state) {
		...
		for (auto &col : column_indexes) {
			if (col.GetPrimaryIndex() >= column_count) {
				...
				if (column.Generated()) {
					throw NotImplementedException("Unsupported: read_sabot_sql cannot read generated column %s",
					                              column.Name());
				}
			}
		}
...
void SabotSQLReader::Scan(ClientContext &context, GlobalTableFunctionState &gstate_p, LocalTableFunctionState &lstate_p,
                        DataChunk &chunk) {
	chunk.Reset();
	auto &lstate = lstate_p.Cast<SabotSQLReadLocalState>();
...
void SabotSQLReader::FinishFile(ClientContext &context, GlobalTableFunctionState &gstate) {
	db_wrapper.reset();
}

optional_idx SabotSQLReader::NumRows() {
	auto &table_entry = GetTableEntry();
	return table_entry.GetStorage().GetTotalRows();
}

unique_ptr<BaseStatistics> SabotSQLReader::GetStatistics(ClientContext &context, const string &name) {
...
double SabotSQLReader::GetProgressInFile(ClientContext &context) {
...
void SabotSQLReader::AddVirtualColumn(column_t virtual_column_id) {
	if (virtual_column_id != COLUMN_IDENTIFIER_ROW_ID) {
		throw InternalException("Unsupported virtual column id %d for sabot_sql reader", virtual_column_id);
	}
}

unique_ptr<MultiFileReaderInterface> SabotSQLMultiFileInfo::CreateInterface(ClientContext &context) {
	return make_uniq<SabotSQLMultiFileInfo>();
}

unique_ptr<BaseFileReaderOptions> SabotSQLMultiFileInfo::InitializeOptions(ClientContext &context,
                                                                         optional_ptr<TableFunctionInfo> info) {
	return make_uniq<SabotSQLFileReaderOptions>();
}

bool SabotSQLMultiFileInfo::ParseCopyOption(ClientContext &context, const string &key, const vector<Value> &values,
                                          BaseFileReaderOptions &options, vector<string> &expected_names,
                                          vector<LogicalType> &expected_types) {
	return false;
}

bool SabotSQLMultiFileInfo::ParseOption(ClientContext &context, const string &key, const Value &val,
                                      MultiFileOptions &file_options, BaseFileReaderOptions &options_p) {
	auto &options = options_p.Cast<SabotSQLFileReaderOptions>();
	if (key == "schema_name") {
...
void SabotSQLMultiFileInfo::FinalizeCopyBind(ClientContext &context, BaseFileReaderOptions &options,
                                           const vector<string> &expected_names,
                                           const vector<LogicalType> &expected_types) {
	throw InternalException("Unimplemented method in SabotSQLMultiFileInfo");
}

void SabotSQLMultiFileInfo::FinalizeBindData(MultiFileBindData &multi_file_data) {
	auto &bind_data = multi_file_data.bind_data->Cast<SabotSQLReadBindData>();
	if (multi_file_data.initial_reader) {
		auto &initial_reader = multi_file_data.initial_reader->Cast<SabotSQLReader>();
		bind_data.initial_file_cardinality = initial_reader.NumRows();
	}
}

unique_ptr<TableFunctionData> SabotSQLMultiFileInfo::InitializeBindData(MultiFileBindData &multi_file_data,
                                                                      unique_ptr<BaseFileReaderOptions> options_p) {
	auto result = make_uniq<SabotSQLReadBindData>();
	result->options = unique_ptr_cast<BaseFileReaderOptions, SabotSQLFileReaderOptions>(std::move(options_p));
	return std::move(result);
}

void SabotSQLMultiFileInfo::BindReader(ClientContext &context, vector<LogicalType> &return_types, vector<string> &names,
                                     MultiFileBindData &bind_data) {
	auto &sabot_sql_bind_data = bind_data.bind_data->Cast<SabotSQLReadBindData>();
	bind_data.reader_bind =
	    bind_data.multi_file_reader->BindReader(context, return_types, names, *bind_data.file_list, bind_data,
	                                            *sabot_sql_bind_data.options, bind_data.file_options);
}

unique_ptr<GlobalTableFunctionState> SabotSQLMultiFileInfo::InitializeGlobalState(ClientContext &context,
                                                                                MultiFileBindData &bind_data,
                                                                                MultiFileGlobalState &global_state) {
	return make_uniq<SabotSQLReadGlobalState>();
}

unique_ptr<LocalTableFunctionState> SabotSQLMultiFileInfo::InitializeLocalState(ExecutionContext &,
                                                                              GlobalTableFunctionState &) {
	return make_uniq<SabotSQLReadLocalState>();
}

struct SabotSQLReaderUnionData : BaseUnionData {
	explicit SabotSQLReaderUnionData(OpenFileInfo file_p) : BaseUnionData(std::move(file_p)) {
	}
};

shared_ptr<BaseFileReader> SabotSQLMultiFileInfo::CreateReader(ClientContext &context, GlobalTableFunctionState &gstate,
                                                             BaseUnionData &union_data_p,
                                                             const MultiFileBindData &multi_bind_data) {
	auto &union_data = union_data_p.Cast<SabotSQLReaderUnionData>();
	auto &bind_data = multi_bind_data.bind_data->Cast<SabotSQLReadBindData>();
	return make_shared_ptr<SabotSQLReader>(context, union_data.file, *bind_data.options);
}
...
shared_ptr<BaseFileReader> SabotSQLMultiFileInfo::CreateReader(ClientContext &context, GlobalTableFunctionState &gstate,
                                                             ...
                                                             const MultiFileBindData &multi_bind_data) {
	auto &bind_data = multi_bind_data.bind_data->Cast<SabotSQLReadBindData>();
	return make_shared_ptr<SabotSQLReader>(context, file, *bind_data.options);
}

shared_ptr<BaseFileReader> SabotSQLMultiFileInfo::CreateReader(ClientContext &context, const OpenFileInfo &file,
                                                             BaseFileReaderOptions &options,
                                                             const MultiFileOptions &file_options) {
	return make_shared_ptr<SabotSQLReader>(context, file, options.Cast<SabotSQLFileReaderOptions>());
}

shared_ptr<BaseUnionData> SabotSQLReader::GetUnionData(idx_t file_idx) {
	auto result = make_uniq<SabotSQLReaderUnionData>(file);
...
void SabotSQLMultiFileInfo::FinishReading(ClientContext &context, GlobalTableFunctionState &global_state,
                                        LocalTableFunctionState &lstate_p) {
	auto &lstate = lstate_p.Cast<SabotSQLReadLocalState>();
	lstate.attached_database.reset();
}

unique_ptr<NodeStatistics> SabotSQLMultiFileInfo::GetCardinality(const MultiFileBindData &bind_data_p, idx_t file_count) {
	auto &bind_data = bind_data_p.bind_data->Cast<SabotSQLReadBindData>();
...
unique_ptr<MultiFileReaderInterface> SabotSQLMultiFileInfo::Copy() {
	return make_uniq<SabotSQLMultiFileInfo>();
}

FileGlobInput SabotSQLMultiFileInfo::GetGlobInput() {
	return FileGlobInput(FileGlobOptions::FALLBACK_GLOB, "db");
}

void SabotSQLMultiFileInfo::GetVirtualColumns(ClientContext &, MultiFileBindData &, virtual_column_map_t &result) {
	result.insert(make_pair(COLUMN_IDENTIFIER_ROW_ID, TableColumn("rowid", LogicalType::BIGINT)));
}

void ReadSabotSQLAddNamedParameters(TableFunction &table_function) {
...
static vector<column_t> SabotSQLGetRowIdColumns(ClientContext &, optional_ptr<FunctionData>) {
...
static bool SabotSQLScanPushdownExpression(ClientContext &context, const LogicalGet &get, Expression &expr) {
	return true;
}

TableFunction ReadSabotSQLTableFunction::GetFunction() {
	MultiFileFunction<SabotSQLMultiFileInfo> read_sabot_sql("read_sabot_sql");
	read_sabot_sql.statistics = MultiFileFunction<SabotSQLMultiFileInfo>::MultiFileScanStats;
	read_sabot_sql.get_row_id_columns = SabotSQLGetRowIdColumns;
	read_sabot_sql.pushdown_expression = SabotSQLScanPushdownExpression;
	read_sabot_sql.filter_pushdown = true;
	read_sabot_sql.filter_prune = true;
	read_sabot_sql.late_materialization = true;
	ReadSabotSQLAddNamedParameters(read_sabot_sql);
	return static_cast<TableFunction>(read_sabot_sql);
}

unique_ptr<TableRef> ReadSabotSQLTableFunction::ReplacementScan(ClientContext &context, ReplacementScanInput &input,
	...
	if (!StringUtil::EndsWith(lower_name, ".db") && !StringUtil::Contains(lower_name, ".db?") &&
	    ...
	    !StringUtil::EndsWith(lower_name, ".sabot_sql") && !StringUtil::Contains(lower_name, ".sabot_sql?")) {
		return nullptr;
	}
	auto table_function = make_uniq<TableFunctionRef>();
	vector<unique_ptr<ParsedExpression>> children;
	children.push_back(make_uniq<ConstantExpression>(Value(table_name)));
	table_function->function = make_uniq<FunctionExpression>("read_sabot_sql", std::move(children));
