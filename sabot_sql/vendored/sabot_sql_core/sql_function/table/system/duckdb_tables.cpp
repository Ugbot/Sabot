...
struct SabotSQLTablesData : public GlobalTableFunctionState {
	SabotSQLTablesData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLTablesBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLTablesInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLTablesData>();
...
void SabotSQLTablesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLTablesData>();
...
void SabotSQLTablesFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_tables", {}, SabotSQLTablesFunction, SabotSQLTablesBind, SabotSQLTablesInit));
}
