...
struct SabotSQLSecretsData : public GlobalTableFunctionState {
	SabotSQLSecretsData() : offset(0) {
	}
	idx_t offset;
	sabot_sql::vector<sabot_sql::SecretEntry> secrets;
};

struct SabotSQLSecretsBindData : public FunctionData {
public:
	unique_ptr<FunctionData> Copy() const override {
		return make_uniq<SabotSQLSecretsBindData>();
	};

	bool Equals(const FunctionData &other_p) const override {
		auto &other = other_p.Cast<SabotSQLSecretsBindData>();
		return redact == other.redact;
	}
	SecretDisplayType redact = SecretDisplayType::REDACTED;
};

static unique_ptr<FunctionData> SabotSQLSecretsBind(ClientContext &context, TableFunctionBindInput &input,
                                                  vector<LogicalType> &return_types, vector<string> &names) {
	auto result = make_uniq<SabotSQLSecretsBindData>();
...
unique_ptr<GlobalTableFunctionState> SabotSQLSecretsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLSecretsData>();
	return std::move(result);
}

void SabotSQLSecretsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLSecretsData>();
	auto &bind_data = data_p.bind_data->Cast<SabotSQLSecretsBindData>();
...
void SabotSQLSecretsFun::RegisterFunction(BuiltinFunctions &set) {
	TableFunctionSet functions("sabot_sql_secrets");
	auto fun = TableFunction({}, SabotSQLSecretsFunction, SabotSQLSecretsBind, SabotSQLSecretsInit);
