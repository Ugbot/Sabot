...
struct SabotSQLPreparedStatementsData : public GlobalTableFunctionState {
	SabotSQLPreparedStatementsData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLPreparedStatementsBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLPreparedStatementsInit(ClientContext &context,
                                                                  TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLPreparedStatementsData>();
...
void SabotSQLPreparedStatementsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLPreparedStatementsData>();
...
void SabotSQLPreparedStatementsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_prepared_statements", {}, SabotSQLPreparedStatementsFunction,
	                              SabotSQLPreparedStatementsBind, SabotSQLPreparedStatementsInit));
}
