...
#include "sabot_sql/catalog/catalog_entry/duck_table_entry.hpp"
...
#include "sabot_sql/parser/constraints/foreign_key_constraint.hpp"
...
#include "sabot_sql/planner/constraints/bound_check_constraint.hpp"
...
struct SabotSQLConstraintsData : public GlobalTableFunctionState {
	SabotSQLConstraintsData() : offset(0), constraint_offset(0), unique_constraint_offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLConstraintsBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLConstraintsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLConstraintsData>();
...
void SabotSQLConstraintsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLConstraintsData>();
	...
	while (data.offset < data.entries.size() && count < STANDARD_VECTOR_SIZE) {
		...
		for (; data.constraint_offset < constraints.size() && count < STANDARD_VECTOR_SIZE; data.constraint_offset++) {
			...
			default:
				throw NotImplementedException("Unimplemented constraint for sabot_sql_constraints");
			}
...
void SabotSQLConstraintsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_constraints", {}, SabotSQLConstraintsFunction, SabotSQLConstraintsBind,
	                              SabotSQLConstraintsInit));
}
