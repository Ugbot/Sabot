...
struct SabotSQLDatabasesData : public GlobalTableFunctionState {
	SabotSQLDatabasesData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLDatabasesBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLDatabasesInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLDatabasesData>();
...
void SabotSQLDatabasesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLDatabasesData>();
...
void SabotSQLDatabasesFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(
	    TableFunction("sabot_sql_databases", {}, SabotSQLDatabasesFunction, SabotSQLDatabasesBind, SabotSQLDatabasesInit));
}
