...
#include "sabot_sql/catalog/duck_catalog.hpp"
...
struct SabotSQLDependenciesData : public GlobalTableFunctionState {
	SabotSQLDependenciesData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLDependenciesBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLDependenciesInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLDependenciesData>();
...
void SabotSQLDependenciesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLDependenciesData>();
...
void SabotSQLDependenciesFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_dependencies", {}, SabotSQLDependenciesFunction, SabotSQLDependenciesBind,
	                              SabotSQLDependenciesInit));
}
