...
struct SabotSQLTypesData : public GlobalTableFunctionState {
	SabotSQLTypesData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLTypesBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLTypesInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLTypesData>();
...
void SabotSQLTypesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLTypesData>();
...
void SabotSQLTypesFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_types", {}, SabotSQLTypesFunction, SabotSQLTypesBind, SabotSQLTypesInit));
}
