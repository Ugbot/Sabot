...
struct SabotSQLSequencesData : public GlobalTableFunctionState {
	SabotSQLSequencesData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLSequencesBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLSequencesInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLSequencesData>();
...
void SabotSQLSequencesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLSequencesData>();
...
void SabotSQLSequencesFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(
	    TableFunction("sabot_sql_sequences", {}, SabotSQLSequencesFunction, SabotSQLSequencesBind, SabotSQLSequencesInit));
}
