...
#include "sabot_sql/catalog/catalog_entry/index_catalog_entry.hpp"
...
struct SabotSQLIndexesData : public GlobalTableFunctionState {
	SabotSQLIndexesData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLIndexesBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLIndexesInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLIndexesData>();
...
void SabotSQLIndexesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLIndexesData>();
...
void SabotSQLIndexesFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_indexes", {}, SabotSQLIndexesFunction, SabotSQLIndexesBind, SabotSQLIndexesInit));
}
