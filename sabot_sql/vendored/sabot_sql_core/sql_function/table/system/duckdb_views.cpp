...
struct SabotSQLViewsData : public GlobalTableFunctionState {
	SabotSQLViewsData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLViewsBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLViewsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLViewsData>();
...
void SabotSQLViewsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLViewsData>();
...
void SabotSQLViewsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_views", {}, SabotSQLViewsFunction, SabotSQLViewsBind, SabotSQLViewsInit));
}
