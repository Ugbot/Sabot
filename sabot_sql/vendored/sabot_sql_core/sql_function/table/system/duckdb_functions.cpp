...
#include "sabot_sql/catalog/catalog_entry/scalar_function_catalog_entry.hpp"
#include "sabot_sql/catalog/catalog_entry/aggregate_function_catalog_entry.hpp"
...
#include "sabot_sql/catalog/catalog_entry/table_function_catalog_entry.hpp"
...
#include "sabot_sql/common/algorithm.hpp"
...
struct SabotSQLFunctionsData : public GlobalTableFunctionState {
	SabotSQLFunctionsData() : offset(0), offset_in_entry(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLFunctionsBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLFunctionsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLFunctionsData>();
...
void SabotSQLFunctionsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLFunctionsData>();
	...
	while (data.offset < data.entries.size() && count < STANDARD_VECTOR_SIZE) {
		...
		case CatalogType::PRAGMA_FUNCTION_ENTRY:
			...
			break;
		default:
			throw InternalException("FIXME: unrecognized function type in sabot_sql_functions");
		}
		if (finished) {
...
void SabotSQLFunctionsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(
	    TableFunction("sabot_sql_functions", {}, SabotSQLFunctionsFunction, SabotSQLFunctionsBind, SabotSQLFunctionsInit));
}
