...
struct SabotSQLExtensionsData : public GlobalTableFunctionState {
	SabotSQLExtensionsData() : offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLExtensionsBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> SabotSQLExtensionsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLExtensionsData>();

	auto &fs = FileSystem::GetFileSystem(context);
	auto &db = DatabaseInstance::GetDatabase(context);

	// Firstly, we go over all Default Extensions: sabot_sql_extensions always prints those, installed/loaded or not
...
void SabotSQLExtensionsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLExtensionsData>();
...
void SabotSQLExtensionsFun::RegisterFunction(BuiltinFunctions &set) {
	TableFunctionSet functions("sabot_sql_extensions");
	functions.AddFunction(TableFunction({}, SabotSQLExtensionsFunction, SabotSQLExtensionsBind, SabotSQLExtensionsInit));
