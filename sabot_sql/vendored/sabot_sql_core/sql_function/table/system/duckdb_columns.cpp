...
struct SabotSQLColumnsData : public GlobalTableFunctionState {
	SabotSQLColumnsData() : offset(0), column_offset(0) {
	}
...
static unique_ptr<FunctionData> SabotSQLColumnsBind(ClientContext &context, TableFunctionBindInput &input,
...
static unique_ptr<GlobalTableFunctionState> SabotSQLColumnsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<SabotSQLColumnsData>();
...
unique_ptr<ColumnHelper> ColumnHelper::Create(CatalogEntry &entry) {
	...
	default:
		throw NotImplementedException("Unsupported catalog type for sabot_sql_columns");
	}
}
...
static void SabotSQLColumnsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<SabotSQLColumnsData>();
...
void SabotSQLColumnsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("sabot_sql_columns", {}, SabotSQLColumnsFunction, SabotSQLColumnsBind, SabotSQLColumnsInit));
}
