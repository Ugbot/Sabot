...
bool ExtractAll(sabot_sql_re2::StringPiece &input, sabot_sql_re2::RE2 &pattern, idx_t *startpos,
                sabot_sql_re2::StringPiece *groups, int ngroups) {
...
void ExtractSingleTuple(const string_t &string, sabot_sql_re2::RE2 &pattern, int32_t group, RegexStringPieceArgs &args,
...
sabot_sql_re2::RE2 &GetPattern(const RegexpBaseBindData &info, ExpressionState &state,
                            unique_ptr<sabot_sql_re2::RE2> &pattern_p) {
	if (info.constant_pattern) {
...
void RegexpExtractAll::Execute(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	unique_ptr<sabot_sql_re2::RE2> stored_re;
	if (!info.constant_pattern) {
	...
	for (idx_t row = 0; row < tuple_count; row++) {
		...
		if (!info.constant_pattern) {
			...
			if (!pattern_data.validity.RowIsValid(pattern_idx)) {
				...
				stored_re = make_uniq<sabot_sql_re2::RE2>(pattern_strpiece, info.options);
