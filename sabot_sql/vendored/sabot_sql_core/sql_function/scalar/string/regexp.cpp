#include "sabot_sql/function/scalar/regexp.hpp"

#include "sabot_sql/common/exception.hpp"
#include "sabot_sql/common/vector_operations/binary_executor.hpp"
#include "sabot_sql/common/vector_operations/ternary_executor.hpp"
...
static bool RegexOptionsEquals(const sabot_sql_re2::RE2::Options &opt_a, const sabot_sql_re2::RE2::Options &opt_b) {
	return opt_a.case_sensitive() == opt_b.case_sensitive();
}

RegexpBaseBindData::RegexpBaseBindData() : constant_pattern(false) {
}
RegexpBaseBindData::RegexpBaseBindData(sabot_sql_re2::RE2::Options options, string constant_string_p,
...
// Regexp Matches
//===--------------------------------------------------------------------===//
RegexpMatchesBindData::RegexpMatchesBindData(sabot_sql_re2::RE2::Options options, string constant_string_p,
                                             bool constant_pattern)
...
struct RegexPartialMatch {
	static inline bool Operation(const sabot_sql_re2::StringPiece &input, sabot_sql_re2::RE2 &re) {
		return sabot_sql_re2::RE2::PartialMatch(input, re);
	}
};

struct RegexFullMatch {
	static inline bool Operation(const sabot_sql_re2::StringPiece &input, sabot_sql_re2::RE2 &re) {
		return sabot_sql_re2::RE2::FullMatch(input, re);
	}
};
...
RegexpReplaceBindData::RegexpReplaceBindData(sabot_sql_re2::RE2::Options options, string constant_string_p,
...
RegexpExtractBindData::RegexpExtractBindData(sabot_sql_re2::RE2::Options options, string constant_string_p,
...
static void RegexExtractStructFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	vector<sabot_sql_re2::StringPiece> ws(groupSize);
	...
	if (input.GetVectorType() == VectorType::CONSTANT_VECTOR) {
		...
		if (ConstantVector::IsNull(input)) {
			...
			auto match = sabot_sql_re2::RE2::PartialMatchN(str, lstate.constant_pattern, groups.data(),
...
static unique_ptr<FunctionData> RegexExtractBind(ClientContext &context, ScalarFunction &bound_function,
	...
	sabot_sql_re2::RE2::Options options;
	...
	if (arguments.size() >= 3) {
		...
		if (group.IsNull()) {
			...
			sabot_sql_re2::StringPiece constant_piece(constant_string.c_str(), constant_string.size());
