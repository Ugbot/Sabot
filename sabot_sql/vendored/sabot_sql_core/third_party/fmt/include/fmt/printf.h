 ...
 public:
  ...
  int operator()(T value) {
    if (!int_checker<std::numeric_limits<T>::is_signed || std::is_same<T, int128_t>::value>::fits_in_int(value))
      FMT_THROW(sabot_sql::InvalidInputException("number is too big"));
    return (std::max)(static_cast<int>(value), 0);
  }

  template <typename T, FMT_ENABLE_IF(!is_integral<T>::value)>
  int operator()(T) {
    FMT_THROW(sabot_sql::InvalidInputException("precision is not integer"));
    return 0;
  }
};
...
template <typename T, typename Context> class arg_converter {
 private:
  ...
  To Cast(From x) {
    return sabot_sql::Cast::Operation<From, To>(x);
  }
  template <typename To, typename From>
...
template <typename Char> class printf_width_handler {
 ...
 public:
  ...
  unsigned operator()(T value) {
    ...
    if (width > int_max) FMT_THROW(sabot_sql::InvalidInputException("number is too big"));
    return static_cast<unsigned>(width);
  }

  template <typename T, FMT_ENABLE_IF(!is_integral<T>::value)>
  unsigned operator()(T) {
    FMT_THROW(sabot_sql::InvalidInputException("width is not integer"));
    return 0;
  }
};
...
class printf_arg_formatter : public internal::arg_formatter_base<Range> {
 ...
 public:
  ...
  template <typename T, FMT_ENABLE_IF(sabot_sql_fmt::internal::is_integral<T>::value)>
...
OutputIt basic_printf_context<OutputIt, Char>::format() {
  ...
  while (it != end) {
    ...
    // Parse type.
    if (it == end) FMT_THROW(sabot_sql::InvalidInputException("invalid format string"));
