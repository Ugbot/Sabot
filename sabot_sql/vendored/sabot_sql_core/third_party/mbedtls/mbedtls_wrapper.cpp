...
#include "sabot_sql/common/random_engine.hpp"
#include "sabot_sql/common/types/timestamp.hpp"

#include <stdexcept>

using namespace std;
using namespace sabot_sql_mbedtls;
...
void MbedTlsWrapper::SHA256State::AddBytes(sabot_sql::const_data_ptr_t input_bytes, sabot_sql::idx_t len) {
	auto context = reinterpret_cast<mbedtls_sha256_context *>(sha_context);
...
void MbedTlsWrapper::SHA256State::AddBytes(sabot_sql::data_ptr_t input_bytes, sabot_sql::idx_t len) {
	AddBytes(sabot_sql::const_data_ptr_t(input_bytes), len);
}
...
void MbedTlsWrapper::SHA256State::FinalizeDerivedKey(sabot_sql::data_ptr_t hash) {
	auto context = reinterpret_cast<mbedtls_sha256_context *>(sha_context);

	if (mbedtls_sha256_finish(context, (sabot_sql::data_ptr_t)hash)) {
		throw std::runtime_error("SHA256 Error");
	}
}
...
const mbedtls_cipher_info_t *MbedTlsWrapper::AESStateMBEDTLS::GetCipher(size_t key_len){
	...
	switch(cipher){
		case sabot_sql::EncryptionTypes::CipherType::GCM:
		    switch (key_len) {
		    case 16:
		...
		case sabot_sql::EncryptionTypes::CipherType::CTR:
		    switch (key_len) {
		    case 16:
		...
		case sabot_sql::EncryptionTypes::CipherType::CBC:
			switch (key_len) {
			case 16:
		...
		default:
				throw sabot_sql::InternalException("Invalid Encryption/Decryption Cipher: %s", sabot_sql::EncryptionTypes::CipherToString(cipher));
	}
}

MbedTlsWrapper::AESStateMBEDTLS::AESStateMBEDTLS(sabot_sql::EncryptionTypes::CipherType cipher_p, sabot_sql::idx_t key_len) : EncryptionState(cipher_p, key_len), context(sabot_sql::make_uniq<mbedtls_cipher_context_t>()) {
	...
	if (cipher == sabot_sql::EncryptionTypes::CBC && mbedtls_cipher_set_padding_mode(context.get(), MBEDTLS_PADDING_PKCS7)) {
...
void MbedTlsWrapper::AESStateMBEDTLS::GenerateRandomDataStatic(sabot_sql::data_ptr_t data, sabot_sql::idx_t len) {
	sabot_sql::RandomEngine random_engine;

	while (len) {
		const auto random_integer = random_engine.NextRandomInteger();
		const auto next = sabot_sql::MinValue<sabot_sql::idx_t>(len, sizeof(random_integer));
		memcpy(data, sabot_sql::const_data_ptr_cast(&random_integer), next);
		data += next;
		len -= next;
	}
}

void MbedTlsWrapper::AESStateMBEDTLS::GenerateRandomData(sabot_sql::data_ptr_t data, sabot_sql::idx_t len) {
	GenerateRandomDataStatic(data, len);
}

void MbedTlsWrapper::AESStateMBEDTLS::InitializeInternal(sabot_sql::const_data_ptr_t iv, sabot_sql::idx_t iv_len, sabot_sql::const_data_ptr_t aad, sabot_sql::idx_t aad_len){
...
void MbedTlsWrapper::AESStateMBEDTLS::InitializeEncryption(sabot_sql::const_data_ptr_t iv, sabot_sql::idx_t iv_len, sabot_sql::const_data_ptr_t key, sabot_sql::idx_t key_len_p, sabot_sql::const_data_ptr_t aad, sabot_sql::idx_t aad_len) {
	mode = sabot_sql::EncryptionTypes::ENCRYPT;

	if (key_len_p != key_len) {
		throw sabot_sql::InternalException("Invalid encryption key length, expected %llu, got %llu", key_len, key_len_p);
	}
...
void MbedTlsWrapper::AESStateMBEDTLS::InitializeDecryption(sabot_sql::const_data_ptr_t iv, sabot_sql::idx_t iv_len, sabot_sql::const_data_ptr_t key, sabot_sql::idx_t key_len_p, sabot_sql::const_data_ptr_t aad, sabot_sql::idx_t aad_len) {
	mode = sabot_sql::EncryptionTypes::DECRYPT;
...
size_t MbedTlsWrapper::AESStateMBEDTLS::Process(sabot_sql::const_data_ptr_t in, sabot_sql::idx_t in_len, sabot_sql::data_ptr_t out,
                                                   sabot_sql::idx_t out_len) {

	// GCM works in-place, CTR and CBC don't
	auto use_out_copy = in == out && cipher != sabot_sql::EncryptionTypes::CipherType::GCM;

	auto out_ptr = out;
	std::unique_ptr<sabot_sql::data_t[]> out_copy;
	if (use_out_copy) {
		out_copy.reset(new sabot_sql::data_t[out_len]);
		out_ptr = out_copy.get();
	}

	size_t out_len_res = sabot_sql::NumericCast<size_t>(out_len);
...
void MbedTlsWrapper::AESStateMBEDTLS::FinalizeGCM(sabot_sql::data_ptr_t tag, sabot_sql::idx_t tag_len){

	switch (mode) {

	case sabot_sql::EncryptionTypes::ENCRYPT: {
	...
	case sabot_sql::EncryptionTypes::DECRYPT: {
		if (mbedtls_cipher_check_tag(context.get(), tag, tag_len)) {
			throw sabot_sql::InvalidInputException(
	...
	default:
		throw sabot_sql::InternalException("Unhandled encryption mode %d", static_cast<int>(mode));
	}
}

size_t MbedTlsWrapper::AESStateMBEDTLS::Finalize(sabot_sql::data_ptr_t out, sabot_sql::idx_t out_len, sabot_sql::data_ptr_t tag,
													sabot_sql::idx_t tag_len) {
	...
	if (cipher == sabot_sql::EncryptionTypes::GCM) {
