...
#define CPPHTTPLIB_NAMESPACE sabot_sql_httplib_openssl
#else
#define CPPHTTPLIB_NAMESPACE sabot_sql_httplib
#endif
...
#include "sabot_sql/common/re2_regex.hpp"
...
typename std::enable_if<!std::is_array<T>::value, sabot_sql::unique_ptr<T>>::type
make_unique(Args &&...args) {
  return sabot_sql::unique_ptr<T>(new T(std::forward<Args>(args)...));
}
...
make_unique(std::size_t n) {
  ...
  return sabot_sql::unique_ptr<T>(new RT[n]);
}
...
using Match = sabot_sql_re2::Match;
using Regex = sabot_sql_re2::Regex;
...
private:
  ...
  static sabot_sql::unique_ptr<detail::MatcherBase>
...
public:
  Result() = default;
  Result(sabot_sql::unique_ptr<Response> &&res, Error err,
...
private:
  sabot_sql::unique_ptr<Response> res_;
...
private:
  ...
  bool handle_request(Stream &strm, Request &req, Response &res,
                      bool close_connection, Error &error);
  sabot_sql::unique_ptr<Response> send_with_content_provider(
...
private:
  sabot_sql::unique_ptr<ClientImpl> cli_;
...
inline std::string file_extension(const std::string &path) {
  ...
  if (sabot_sql_re2::RegexSearch(path, m, re)) { return m[1].str(); }
  return std::string();
}
...
bool prepare_content_receiver(T &x, int &status,
  ...
  if (decompress) {
    ...
    sabot_sql::unique_ptr<decompressor> decompressor;
        ...
        while (pos < buf_size()) {
          ...
          if (start_with_case_ignore(header, header_content_type)) {
            ...
            const Regex re_content_disposition(
                ...
                sabot_sql_re2::RegexOptions::CASE_INSENSITIVE);
            ...
            if (RegexMatch(header, m, re_content_disposition)) {
              ...
              if (it != params.end()) {
                ...
                const Regex re_rfc5987_encoding(
                    R"~(^UTF-8''(.+?)$)~", sabot_sql_re2::RegexOptions::CASE_INSENSITIVE);
...
inline std::string message_digest(const std::string &s, const EVP_MD *algo) {
  auto context = sabot_sql::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_free)>(
...
template <typename T>
using CFObjectPtr =
    sabot_sql::unique_ptr<typename std::remove_pointer<T>::type, void (*)(CFTypeRef)>;
  ...
  if (res.has_header(auth_key)) {
    ...
    if (pos != std::string::npos) {
      ...
      if (type == "Basic") {
        ...
        s = s.substr(pos + 1);
        auto matches = sabot_sql_re2::RegexFindAll(s, re);
        for (auto &m : matches) {
...
inline sabot_sql::unique_ptr<detail::MatcherBase>
...
Server::write_content_with_provider(Stream &strm, const Request &req,
  ...
  if (res.content_length_ > 0) {
    ...
    if (res.is_chunked_content_provider_) {
      ...
      sabot_sql::unique_ptr<detail::compressor> compressor;
    ...
    if (!req.path.compare(0, entry.mount_point.size(), entry.mount_point)) {
      ...
      if (detail::is_valid_path(sub_path)) {
        ...
        if (detail::is_file(path)) {
          ...
          auto mm = sabot_sql::make_shared_ptr<detail::mmap>(path.c_str());
...
inline bool Server::listen_internal() {
  ...
  {
    sabot_sql::unique_ptr<TaskQueue> task_queue(new_task_queue());
...
inline bool Server::dispatch_request(Request &req, Response &res,
  ...
  for (const auto &x : handlers) {
    ...
    if (sabot_sql_re2::RegexMatch(req.path, req.matches, pattern)) {
      handler(req, res);
      return true;
    }
  }
...
inline sabot_sql::unique_ptr<Response> ClientImpl::send_with_content_provider(
    Request &req, const char *body, size_t content_length,
    ContentProvider content_provider,
    ContentProviderWithoutLength content_provider_without_length,
    const std::string &content_type, Error &error) {
