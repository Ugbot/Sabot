...
using namespace sabot_sql_brotli;
...
BROTLI_BOOL sabot_sql_brotli::BrotliEncoderSetParameter(
...
BrotliEncoderState* sabot_sql_brotli::BrotliEncoderCreateInstance(
...
void sabot_sql_brotli::BrotliEncoderDestroyInstance(BrotliEncoderState* state) {
  if (!state) {
    return;
  } else {
...
size_t sabot_sql_brotli::BrotliEncoderMaxCompressedSize(size_t input_size) {
...
BROTLI_BOOL sabot_sql_brotli::BrotliEncoderCompress(
...
BROTLI_BOOL sabot_sql_brotli::BrotliEncoderCompressStream(
    BrotliEncoderState* s, BrotliEncoderOperation op, size_t* available_in,
    const uint8_t** next_in, size_t* available_out, uint8_t** next_out,
    size_t* total_out) {
...
BROTLI_BOOL sabot_sql_brotli::BrotliEncoderIsFinished(BrotliEncoderState* s) {
  return TO_BROTLI_BOOL(s->stream_state_ == BROTLI_STREAM_FINISHED &&
      !BrotliEncoderHasMoreOutput(s));
}

BROTLI_BOOL sabot_sql_brotli::BrotliEncoderHasMoreOutput(BrotliEncoderState* s) {
...
uint32_t sabot_sql_brotli::BrotliEncoderVersion(void) {
  return BROTLI_VERSION;
}

BrotliEncoderPreparedDictionary*  sabot_sql_brotli::BrotliEncoderPrepareDictionary(
...
size_t  sabot_sql_brotli::BrotliEncoderEstimatePeakMemoryUsage(int quality, int lgwin,
                                            size_t input_size) {
  BrotliEncoderParams params;
...
size_t sabot_sql_brotli::BrotliEncoderGetPreparedDictionarySize(
