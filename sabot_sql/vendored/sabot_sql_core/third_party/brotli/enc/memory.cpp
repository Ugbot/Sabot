...
void sabot_sql_brotli::BrotliInitMemoryManager(
    MemoryManager* m, brotli_alloc_func alloc_func, brotli_free_func free_func,
    void* opaque) {
  if (!alloc_func) {
    m->alloc_func = sabot_sql_brotli::BrotliDefaultAllocFunc;
    m->free_func =  sabot_sql_brotli::BrotliDefaultFreeFunc;
    m->opaque = 0;
  } else {
...
void* sabot_sql_brotli::BrotliAllocate(MemoryManager* m, size_t n) {
  void* result = m->alloc_func(m->opaque, n);
  if (!result) exit(EXIT_FAILURE);
  return result;
}

void sabot_sql_brotli::BrotliFree(MemoryManager* m, void* p) {
  m->free_func(m->opaque, p);
}

void sabot_sql_brotli::BrotliWipeOutMemoryManager(MemoryManager* m) {
  BROTLI_UNUSED(m);
}
...
void* sabot_sql_brotli::BrotliBootstrapAlloc(size_t size,
...
void sabot_sql_brotli::BrotliBootstrapFree(void* address, MemoryManager* m) {
  if (!address) {
    /* Should not happen! */
    return;
  } else {
