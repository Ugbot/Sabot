...
void dbgen(sabot_sql::SabotSQL &db, uint32_t sf, std::string schema, std::string suffix) {
	sabot_sql::unique_ptr<CBaseLoaderFactory> pLoaderFactory; // class factory that creates table loaders
	CGenerateAndLoadStandardOutput Output;
	sabot_sql::unique_ptr<CGenerateAndLoad> pGenerateAndLoad;
	...
	pLoaderFactory = make_uniq<SabotSQLLoaderFactory>(con, schema, suffix);
