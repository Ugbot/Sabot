...
FROM histogram_values(source, col_name, bin_count := bin_count, technique := technique);
	...
	{DEFAULT_SCHEMA, "sabot_sql_logs_parsed", {"log_type"}, {}, R"(
SELECT * EXCLUDE (message), UNNEST(parse_sabot_sql_log_message(log_type, message))
FROM sabot_sql_logs(denormalized_table=1)
WHERE type = log_type
)"},
