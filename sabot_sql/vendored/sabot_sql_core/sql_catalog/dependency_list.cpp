...
uint64_t LogicalDependencyHashFunction::operator()(const LogicalDependency &a) const {
	...
	hash_t hash = sabot_sql::Hash(name.c_str());
	hash = CombineHash(hash, sabot_sql::Hash(schema.c_str()));
	hash = CombineHash(hash, sabot_sql::Hash(catalog.c_str()));
	hash = CombineHash(hash, sabot_sql::Hash<uint8_t>(static_cast<uint8_t>(type)));
	return hash;
}
