...
#include "sabot_sql/common/cgroups.hpp"
...
#include "sabot_sql/main/settings.hpp"
#include "sabot_sql/storage/storage_extension.hpp"
#include "sabot_sql/common/serializer/serializer.hpp"

#ifndef SABOT_SQL_NO_THREADS
#include "sabot_sql/common/thread.hpp"
#endif
...
#define SABOT_SQL_SETTING(_PARAM)                                                                                         \
	{                                                                                                                  \
...
#define SABOT_SQL_SETTING_CALLBACK(_PARAM)                                                                                \
	{                                                                                                                  \
		_PARAM::Name, _PARAM::Description, _PARAM::InputType, nullptr, nullptr, nullptr, nullptr, nullptr,             \
		    _PARAM::DefaultScope, _PARAM::DefaultValue, _PARAM::OnSet                                                  \
	}
#define SABOT_SQL_GLOBAL(_PARAM)                                                                                          \
	{                                                                                                                  \
...
#define SABOT_SQL_GLOBAL_LOCAL(_PARAM)                                                                                    \
	{                                                                                                                  \
...
#define SABOT_SQL_SETTING_ALIAS(_ALIAS, _SETTING_INDEX)                                                                   \
...
static const ConfigurationOption internal_options[] = {
    ...
    SABOT_SQL_GLOBAL(AccessModeSetting),
    SABOT_SQL_GLOBAL(AllocatorBackgroundThreadsSetting),
    SABOT_SQL_GLOBAL(AllocatorBulkDeallocationFlushThresholdSetting),
    SABOT_SQL_GLOBAL(AllocatorFlushThresholdSetting),
    SABOT_SQL_GLOBAL(AllowCommunityExtensionsSetting),
    SABOT_SQL_SETTING(AllowExtensionsMetadataMismatchSetting),
    SABOT_SQL_GLOBAL(AllowPersistentSecretsSetting),
    SABOT_SQL_GLOBAL(AllowUnredactedSecretsSetting),
    SABOT_SQL_GLOBAL(AllowUnsignedExtensionsSetting),
    SABOT_SQL_GLOBAL(AllowedDirectoriesSetting),
    SABOT_SQL_GLOBAL(AllowedPathsSetting),
    SABOT_SQL_SETTING(ArrowLargeBufferSizeSetting),
    SABOT_SQL_SETTING(ArrowLosslessConversionSetting),
    SABOT_SQL_SETTING(ArrowOutputListViewSetting),
    SABOT_SQL_SETTING_CALLBACK(ArrowOutputVersionSetting),
    SABOT_SQL_SETTING(AsofLoopJoinThresholdSetting),
    SABOT_SQL_GLOBAL(AutoinstallExtensionRepositorySetting),
    SABOT_SQL_GLOBAL(AutoinstallKnownExtensionsSetting),
    SABOT_SQL_GLOBAL(AutoloadKnownExtensionsSetting),
    SABOT_SQL_SETTING(CatalogErrorMaxSchemasSetting),
    SABOT_SQL_GLOBAL(CheckpointThresholdSetting),
    SABOT_SQL_GLOBAL(CustomExtensionRepositorySetting),
    SABOT_SQL_LOCAL(CustomProfilingSettingsSetting),
    SABOT_SQL_GLOBAL(CustomUserAgentSetting),
    SABOT_SQL_SETTING(DebugAsofIejoinSetting),
    SABOT_SQL_SETTING_CALLBACK(DebugCheckpointAbortSetting),
    SABOT_SQL_LOCAL(DebugForceExternalSetting),
    SABOT_SQL_SETTING(DebugForceNoCrossProductSetting),
    SABOT_SQL_SETTING(DebugSkipCheckpointOnCommitSetting),
    SABOT_SQL_SETTING_CALLBACK(DebugVerifyVectorSetting),
    SABOT_SQL_SETTING_CALLBACK(DebugWindowModeSetting),
    SABOT_SQL_GLOBAL(DefaultBlockSizeSetting),
    SABOT_SQL_SETTING_CALLBACK(DefaultCollationSetting),
    SABOT_SQL_SETTING_CALLBACK(DefaultNullOrderSetting),
    SABOT_SQL_SETTING_CALLBACK(DefaultOrderSetting),
    SABOT_SQL_GLOBAL(DefaultSecretStorageSetting),
    SABOT_SQL_GLOBAL(DisableDatabaseInvalidationSetting),
    SABOT_SQL_SETTING(DisableTimestamptzCastsSetting),
    SABOT_SQL_GLOBAL(DisabledCompressionMethodsSetting),
    SABOT_SQL_GLOBAL(DisabledFilesystemsSetting),
    SABOT_SQL_GLOBAL(DisabledLogTypes),
    SABOT_SQL_GLOBAL(DisabledOptimizersSetting),
    SABOT_SQL_GLOBAL(SabotSQLAPISetting),
    SABOT_SQL_SETTING(DynamicOrFilterThresholdSetting),
    SABOT_SQL_GLOBAL(EnableExternalAccessSetting),
    SABOT_SQL_GLOBAL(EnableExternalFileCacheSetting),
    SABOT_SQL_SETTING(EnableFSSTVectorsSetting),
    SABOT_SQL_LOCAL(EnableHTTPLoggingSetting),
    SABOT_SQL_GLOBAL(EnableHTTPMetadataCacheSetting),
    SABOT_SQL_GLOBAL(EnableLogging),
    SABOT_SQL_SETTING(EnableMacroDependenciesSetting),
    SABOT_SQL_SETTING(EnableObjectCacheSetting),
    SABOT_SQL_LOCAL(EnableProfilingSetting),
    SABOT_SQL_LOCAL(EnableProgressBarSetting),
    SABOT_SQL_LOCAL(EnableProgressBarPrintSetting),
    SABOT_SQL_SETTING(EnableViewDependenciesSetting),
    SABOT_SQL_GLOBAL(EnabledLogTypes),
    SABOT_SQL_LOCAL(ErrorsAsJSONSetting),
    SABOT_SQL_SETTING(ExperimentalMetadataReuseSetting),
    SABOT_SQL_LOCAL(ExplainOutputSetting),
    SABOT_SQL_GLOBAL(ExtensionDirectorySetting),
    SABOT_SQL_GLOBAL(ExternalThreadsSetting),
    SABOT_SQL_LOCAL(FileSearchPathSetting),
    SABOT_SQL_GLOBAL(ForceBitpackingModeSetting),
    SABOT_SQL_GLOBAL(ForceCompressionSetting),
    SABOT_SQL_LOCAL(HomeDirectorySetting),
    SABOT_SQL_LOCAL(HTTPLoggingOutputSetting),
    SABOT_SQL_GLOBAL(HTTPProxySetting),
    SABOT_SQL_GLOBAL(HTTPProxyPasswordSetting),
    SABOT_SQL_GLOBAL(HTTPProxyUsernameSetting),
    SABOT_SQL_SETTING(IeeeFloatingPointOpsSetting),
    SABOT_SQL_SETTING(ImmediateTransactionModeSetting),
    SABOT_SQL_SETTING(IndexScanMaxCountSetting),
    SABOT_SQL_SETTING_CALLBACK(IndexScanPercentageSetting),
    SABOT_SQL_SETTING(IntegerDivisionSetting),
    SABOT_SQL_LOCAL(LambdaSyntaxSetting),
    SABOT_SQL_SETTING(LateMaterializationMaxRowsSetting),
    SABOT_SQL_GLOBAL(LockConfigurationSetting),
    SABOT_SQL_LOCAL(LogQueryPathSetting),
    SABOT_SQL_GLOBAL(LoggingLevel),
    SABOT_SQL_GLOBAL(LoggingMode),
    SABOT_SQL_GLOBAL(LoggingStorage),
    SABOT_SQL_LOCAL(MaxExpressionDepthSetting),
    SABOT_SQL_GLOBAL(MaxMemorySetting),
    SABOT_SQL_GLOBAL(MaxTempDirectorySizeSetting),
    SABOT_SQL_SETTING(MaxVacuumTasksSetting),
    SABOT_SQL_SETTING(MergeJoinThresholdSetting),
    SABOT_SQL_SETTING(NestedLoopJoinThresholdSetting),
    SABOT_SQL_SETTING(OldImplicitCastingSetting),
    SABOT_SQL_SETTING(OrderByNonIntegerLiteralSetting),
    SABOT_SQL_SETTING_CALLBACK(OrderedAggregateThresholdSetting),
    SABOT_SQL_SETTING(PartitionedWriteFlushThresholdSetting),
    SABOT_SQL_SETTING(PartitionedWriteMaxOpenFilesSetting),
    SABOT_SQL_GLOBAL(PasswordSetting),
    SABOT_SQL_SETTING_CALLBACK(PerfectHtThresholdSetting),
    SABOT_SQL_GLOBAL(PinThreadsSetting),
    SABOT_SQL_SETTING(PivotFilterThresholdSetting),
    SABOT_SQL_SETTING(PivotLimitSetting),
    SABOT_SQL_SETTING(PreferRangeJoinsSetting),
    SABOT_SQL_SETTING(PreserveIdentifierCaseSetting),
    SABOT_SQL_SETTING(PreserveInsertionOrderSetting),
    SABOT_SQL_SETTING(ProduceArrowStringViewSetting),
    SABOT_SQL_LOCAL(ProfileOutputSetting),
    SABOT_SQL_LOCAL(ProfilingCoverageSetting),
    SABOT_SQL_LOCAL(ProfilingModeSetting),
    SABOT_SQL_LOCAL(ProgressBarTimeSetting),
    SABOT_SQL_SETTING(ScalarSubqueryErrorOnMultipleRowsSetting),
    SABOT_SQL_GLOBAL(SchedulerProcessPartialSetting),
    SABOT_SQL_LOCAL(SchemaSetting),
    SABOT_SQL_LOCAL(SearchPathSetting),
    SABOT_SQL_GLOBAL(SecretDirectorySetting),
    SABOT_SQL_SETTING_CALLBACK(StorageBlockPrefetchSetting),
    SABOT_SQL_GLOBAL(StorageCompatibilityVersionSetting),
    SABOT_SQL_LOCAL(StreamingBufferSizeSetting),
    SABOT_SQL_GLOBAL(TempDirectorySetting),
    SABOT_SQL_GLOBAL(TempFileEncryptionSetting),
    SABOT_SQL_GLOBAL(ThreadsSetting),
    SABOT_SQL_GLOBAL(UsernameSetting),
    SABOT_SQL_SETTING(WriteBufferRowGroupCountSetting),
    SABOT_SQL_GLOBAL(ZstdMinStringLengthSetting),
    FINAL_SETTING};

static const ConfigurationAlias setting_aliases[] = {SABOT_SQL_SETTING_ALIAS("memory_limit", 83),
                                                     SABOT_SQL_SETTING_ALIAS("null_order", 33),
                                                     SABOT_SQL_SETTING_ALIAS("profiling_output", 102),
                                                     SABOT_SQL_SETTING_ALIAS("user", 117),
                                                     SABOT_SQL_SETTING_ALIAS("wal_autocheckpoint", 20),
                                                     SABOT_SQL_SETTING_ALIAS("worker_threads", 116),
                                                     FINAL_ALIAS};
...
const string DBConfig::UserAgent() const {
	...
	if (!options.sabot_sql_api.empty()) {
		user_agent += " " + options.sabot_sql_api;
	}
...
SerializationCompatibility SerializationCompatibility::FromString(const string &input) {
	...
	if (!serialization_version.IsValid()) {
		...
		throw InvalidInputException("The version string '%s' is not a known SabotSQL version, valid options are: %s",
		                            input, StringUtil::Join(candidates, ", "));
	}
	SerializationCompatibility result;
	result.sabot_sql_version = input;
...
SerializationCompatibility SerializationCompatibility::Default() {
	...
	return res;
#else
#ifdef SABOT_SQL_LATEST_STORAGE
	auto res = FromString("latest");
	res.manually_set = false;
	return res;
#else
