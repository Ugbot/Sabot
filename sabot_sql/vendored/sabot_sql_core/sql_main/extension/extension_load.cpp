...
struct SabotSQLExtensionLoadState {
	explicit SabotSQLExtensionLoadState(DatabaseInstance &db_p, ExtensionInitResult &init_result_p)
	    : db(db_p), init_result(init_result_p), database_data(nullptr) {
	}

	//! Create a SabotSQLExtensionLoadState reference from a C API opaque pointer
	static SabotSQLExtensionLoadState &Get(sabot_sql_extension_info info) {
		D_ASSERT(info);
		return *reinterpret_cast<sabot_sql::SabotSQLExtensionLoadState *>(info);
	}

	//! Convert to an opaque C API pointer
	sabot_sql_extension_info ToCStruct() {
		return reinterpret_cast<sabot_sql_extension_info>(this);
	}
	...
	//! This is the sabot_sql_database struct that will be passed to the extension during initialization. Note that the
	...
	//! initialization
	sabot_sql_ext_api_v1 api_struct;
...
struct ExtensionAccess {
	...
	static sabot_sql_extension_access CreateAccessStruct() {
		return {SetError, GetDatabase, GetAPI};
	}

	//! Called by the extension to indicate failure to initialize the extension
	static void SetError(sabot_sql_extension_info info, const char *error) {
		auto &load_state = SabotSQLExtensionLoadState::Get(info);
	...
	static sabot_sql_database *GetDatabase(sabot_sql_extension_info info) {
		auto &load_state = SabotSQLExtensionLoadState::Get(info);

		try {
			// Create the sabot_sql_database
			load_state.database_data = make_uniq<DatabaseWrapper>();
			load_state.database_data->database = make_shared_ptr<SabotSQL>(load_state.db);
			return reinterpret_cast<sabot_sql_database *>(load_state.database_data.get());
	...
	static const void *GetAPI(sabot_sql_extension_info info, const char *version) {
		...
		if (load_state.init_result.abi_type == ExtensionABIType::C_STRUCT) {
			...
			// ignored because C_STRUCT_UNSTABLE extensions are tied 1:1 to sabot_sql verions meaning they will always
...
//===--------------------------------------------------------------------===//
#ifndef SABOT_SQL_DISABLE_EXTENSION_LOAD
// The C++ init function
typedef void (*ext_init_fun_t)(ExtensionLoader &);
// The C init function
typedef bool (*ext_init_c_api_fun_t)(sabot_sql_extension_info info, sabot_sql_extension_access *access);
...
static void ComputeSHA256String(const string &to_hash, string *res) {
	...
	*res = sabot_sql_mbedtls::MbedTlsWrapper::ComputeSha256Hash(to_hash);
}
...
ParsedExtensionMetaData ExtensionHelper::ParseExtensionMetaData(const char *metadata) noexcept {
	...
	if (extension_abi_metadata == "C_STRUCT") {
		...
		result.sabot_sql_capi_version = FilterZeroAtEnd(metadata_field[2]);
	} else if (extension_abi_metadata == "C_STRUCT_UNSTABLE") {
		result.abi_type = ExtensionABIType::C_STRUCT_UNSTABLE;
		result.sabot_sql_version = FilterZeroAtEnd(metadata_field[2]);
		...
		result.sabot_sql_version = "unknown";
...
ParsedExtensionMetaData ExtensionHelper::ParseExtensionMetaData(FileHandle &handle) {
	...
	if (handle.GetFileSize() < ParsedExtensionMetaData::FOOTER_SIZE) {
		throw InvalidInputException(
		    "File '%s' is not a SabotSQL extension. Valid SabotSQL extensions must be at least %llu bytes", handle.path,
...
bool ExtensionHelper::CheckExtensionSignature(FileHandle &handle, ParsedExtensionMetaData &parsed_metadata,
	...
	for (auto &key : ExtensionHelper::GetPublicKeys(allow_community_extensions)) {
		if (sabot_sql_mbedtls::MbedTlsWrapper::IsValidSha256Signature(key, parsed_metadata.signature, two_level_hash)) {
			return true;
			break;
		}
	}
...
bool ExtensionHelper::TryInitialLoad(DatabaseInstance &db, FileSystem &fs, const string &extension,
	...
	if (!ExtensionHelper::IsFullPath(extension)) {
		...
		filename = fs.JoinPath(local_path, extension_name + ".sabot_sql_extension");
#endif
	} else {
		direct_load = true;
...
void ExtensionHelper::LoadExternalExtensionInternal(DatabaseInstance &db, FileSystem &fs, const string &extension,
	...
	if (extension_init_result.abi_type == ExtensionABIType::CPP) {
		auto init_fun_name = extension_init_result.filebase + "_sabot_sql_cpp_init";
	...
	if (extension_init_result.abi_type == ExtensionABIType::C_STRUCT ||
		...
		SabotSQLExtensionLoadState load_state(db, extension_init_result);
