...
#include "sabot_sql/common/local_file_system.hpp"
#include "sabot_sql/common/serializer/binary_serializer.hpp"
...
#include "sabot_sql/common/windows_undefs.hpp"
...
#ifdef SABOT_SQL_WASM_VERSION
	return SABOT_SQL_QUOTE_DEFINE(SABOT_SQL_WASM_VERSION);
#endif
	if (IsRelease(SabotSQL::LibraryVersion())) {
		return NormalizeVersionTag(SabotSQL::LibraryVersion());
	} else {
...
const vector<string> ExtensionHelper::PathComponents() {
	return vector<string> {GetVersionDirectoryName(), SabotSQL::Platform()};
}

string ExtensionHelper::ExtensionInstallDocumentationLink(const string &extension_name) {
	auto components = PathComponents();

	string link = "https://sabot_sql.org/docs/stable/extensions/troubleshooting";
...
sabot_sql::string ExtensionHelper::DefaultExtensionFolder(FileSystem &fs) {
...
string ExtensionHelper::ExtensionDirectory(DatabaseInstance &db, FileSystem &fs) {
	...
	throw PermissionException("ExtensionDirectory functionality is not supported in sabot_sql-wasm");
#endif
...
string ExtensionHelper::ExtensionUrlTemplate(optional_ptr<const DatabaseInstance> db,
	...
	if (!version.empty()) {
		versioned_path = "/${NAME}/" + version + "/${REVISION}/${PLATFORM}/${NAME}.sabot_sql_extension";
	} else {
		versioned_path = "/${REVISION}/${PLATFORM}/${NAME}.sabot_sql_extension";
	}
...
string ExtensionHelper::ExtensionFinalizeUrlTemplate(const string &url_template, const string &extension_name) {
	...
	url = StringUtil::Replace(url, "${PLATFORM}", SabotSQL::Platform());
...
static void CheckExtensionMetadataOnInstall(DatabaseInstance &db, void *in_buffer, idx_t file_size,
	...
	if (file_size < ParsedExtensionMetaData::FOOTER_SIZE) {
		throw IOException("Failed to install '%s', file too small to be a valid SabotSQL extension!", extension_name);
	}
...
#endif // SABOT_SQL_DISABLE_EXTENSION_LOAD
...
unique_ptr<ExtensionInstallInfo> ExtensionHelper::InstallExtensionInternal(DatabaseInstance &db, FileSystem &fs,
	...
	string local_extension_path = fs.JoinPath(local_path, extension_name + ".sabot_sql_extension");
...
#ifdef DISABLE_SABOT_SQL_REMOTE_INSTALL
