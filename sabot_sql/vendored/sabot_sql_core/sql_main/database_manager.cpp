...
#include "sabot_sql/transaction/duck_transaction_manager.hpp"
#include "sabot_sql/parser/parsed_data/alter_database_info.hpp"
...
shared_ptr<AttachedDatabase> DatabaseManager::AttachDatabase(ClientContext &context, AttachInfo &info,
	...
	if (options.db_type.empty() || StringUtil::CIEquals(options.db_type, "sabot_sql")) {
	...
	if (!options.db_type.empty()) {
		// we only need to prevent duplicate opening of SabotSQL files
		// if this is not a SabotSQL file but e.g. a CSV or Parquet file, we don't need to do this duplicate protection
...
void DatabaseManager::GetDatabaseType(ClientContext &context, AttachInfo &info, const DBConfig &config,
	...
	// Test if the database is a SabotSQL database file.
	if (StringUtil::CIEquals(options.db_type, "sabot_sql")) {
		options.db_type = "";
		return;
	}
