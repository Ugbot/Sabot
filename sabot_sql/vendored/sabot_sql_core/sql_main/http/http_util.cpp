...
#ifndef DISABLE_SABOT_SQL_REMOTE_INSTALL
...
HTTPHeaders::HTTPHeaders(DatabaseInstance &db) {
	headers.insert({"User-Agent", StringUtil::Format("%s %s", db.config.UserAgent(), SabotSQL::SourceID())});
}
...
unique_ptr<HTTPResponse> TransformResponse(sabot_sql_httplib::Result &res) {
	auto status_code = HTTPUtil::ToStatusCode(res ? res->status : 0);
	auto result = make_uniq<HTTPResponse>(status_code);
	if (res.error() == sabot_sql_httplib::Error::Success) {
...
public:
	HTTPLibClient(HTTPParams &http_params, const string &proto_host_port) {
		...
		client = make_uniq<sabot_sql_httplib::Client>(proto_host_port);
	...
	unique_ptr<HTTPResponse> Get(GetRequestInfo &info) override {
		...
		if (!info.response_handler && !info.content_handler) {
			...
			return TransformResult(client->Get(
			    info.path, headers,
			    [&](const sabot_sql_httplib::Response &response) {
	...
	unique_ptr<sabot_sql_httplib::Client> client;

private:
	sabot_sql_httplib::Headers TransformHeaders(const HTTPHeaders &header_map, const HTTPParams &params) {
		sabot_sql_httplib::Headers headers;
	...
	unique_ptr<HTTPResponse> TransformResponse(const sabot_sql_httplib::Response &response) {
	...
	unique_ptr<HTTPResponse> TransformResult(const sabot_sql_httplib::Result &res) {
		if (res.error() == sabot_sql_httplib::Error::Success) {
			auto &response = res.value();
...
sabot_sql::unique_ptr<HTTPResponse>
