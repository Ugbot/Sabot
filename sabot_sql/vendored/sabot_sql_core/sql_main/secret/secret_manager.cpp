...
#include "sabot_sql/main/secret/default_secrets.hpp"
...
void SecretManager::Initialize(DatabaseInstance &db) {
	...
	vector<string> path_components = {".sabot_sql", "stored_secrets"};
...
unique_ptr<SecretEntry> SecretManager::RegisterSecretInternal(CatalogTransaction transaction,
	...
	if (!backend) {
		if (!config.allow_persistent_secrets &&
			...
			throw InvalidInputException("Persistent secrets are disabled. Restart SabotSQL and enable persistent secrets "
		...
		if (backend->persistent) {
			if (!config.allow_persistent_secrets) {
				throw InvalidInputException(
				    "Persistent secrets are currently disabled. To enable them, restart sabot_sql and "
...
unique_ptr<CatalogEntry> DefaultSecretGenerator::CreateDefaultEntryInternal(const string &entry_name) {
	...
	string secret_path = fs.JoinPath(base_secret_path, entry_name + ".sabot_sql_secret");
