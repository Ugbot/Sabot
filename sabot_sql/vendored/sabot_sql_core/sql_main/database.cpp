...
#include "sabot_sql/execution/operator/helper/physical_set.hpp"
...
#include "sabot_sql/main/database_file_opener.hpp"
#include "sabot_sql/main/database_manager.hpp"
#include "sabot_sql/main/database_path_and_type.hpp"
...
#include "sabot_sql/planner/collation_binding.hpp"
#include "sabot_sql/planner/extension_callback.hpp"
#include "sabot_sql/storage/object_cache.hpp"
#include "sabot_sql/storage/standard_buffer_manager.hpp"
#include "sabot_sql/storage/storage_extension.hpp"
#include "sabot_sql/storage/storage_manager.hpp"
#include "sabot_sql/transaction/transaction_manager.hpp"
#include "sabot_sql/main/capi/extension_api.hpp"
#include "sabot_sql/storage/external_file_cache.hpp"
#include "sabot_sql/storage/compression/empty_validity.hpp"
#include "sabot_sql/logging/logger.hpp"
#include "sabot_sql/common/http_util.hpp"
#include "mbedtls_wrapper.hpp"
#include "sabot_sql/main/database_file_path_manager.hpp"
...
shared_ptr<AttachedDatabase> DatabaseInstance::CreateAttachedDatabase(ClientContext &context, AttachInfo &info,
	...
	// An empty db_type defaults to a sabot_sql database file.
	attached_database = make_shared_ptr<AttachedDatabase>(*this, catalog, info.name, info.path, options);
	return attached_database;
}
...
static sabot_sql_ext_api_v1 CreateAPIv1Wrapper() {
	return CreateAPIv1();
}
...
void DatabaseInstance::Initialize(const char *database_path, DBConfig *user_config) {
	...
	if (!config.options.database_type.empty() && !StringUtil::CIEquals(config.options.database_type, "sabot_sql")) {
...
SabotSQL::SabotSQL(const char *path, DBConfig *new_config) : instance(make_shared_ptr<DatabaseInstance>()) {
...
SabotSQL::SabotSQL(const string &path, DBConfig *config) : SabotSQL(path.c_str(), config) {
}

SabotSQL::SabotSQL(DatabaseInstance &instance_p) : instance(instance_p.shared_from_this()) {
}
...
FileSystem &SabotSQL::GetFileSystem() {
...
void DatabaseInstance::Configure(DBConfig &new_config, const char *database_path) {
	...
	if (config.options.sabot_sql_api.empty()) {
		config.SetOptionByName("sabot_sql_api", "cpp");
	}
...
idx_t SabotSQL::NumberOfThreads() {
...
bool SabotSQL::ExtensionIsLoaded(const std::string &name) {
...
shared_ptr<EncryptionUtil> DatabaseInstance::GetEncryptionUtil() const {
	...
	return make_shared_ptr<sabot_sql_mbedtls::MbedTlsWrapper::AESStateMBEDTLSFactory>();
}
...
const sabot_sql_ext_api_v1 DatabaseInstance::GetExtensionAPIV1() {
