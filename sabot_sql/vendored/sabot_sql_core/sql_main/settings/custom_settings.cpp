...
const string GetDefaultUserAgent() {
	return StringUtil::Format("sabot_sql/%s(%s)", SabotSQL::LibraryVersion(), SabotSQL::Platform());
}
...
// Duckdb Api
//===----------------------------------------------------------------------===//
void SabotSQLAPISetting::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	auto new_value = input.GetValue<string>();
	if (db) {
		throw InvalidInputException("Cannot change sabot_sql_api setting while database is running");
	}
	config.options.sabot_sql_api = new_value;
}

void SabotSQLAPISetting::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (db) {
		throw InvalidInputException("Cannot change sabot_sql_api setting while database is running");
	}
	config.options.sabot_sql_api = GetDefaultUserAgent();
}

Value SabotSQLAPISetting::GetSetting(const ClientContext &context) {
	auto &config = DBConfig::GetConfig(context);
	return Value(config.options.sabot_sql_api);
}
...
void HTTPLoggingOutputSetting::SetLocal(ClientContext &context, const Value &input) {
	throw NotImplementedException("This setting is deprecated and can no longer be used. Check out the SabotSQL docs on "
...
Value StorageCompatibilityVersionSetting::GetSetting(const ClientContext &context) {
	...
	auto &version_name = config.options.serialization_compatibility.sabot_sql_version;
