...
#include "sabot_sql/optimizer/matcher/type_matcher_id.hpp"
#include "sabot_sql/optimizer/expression_rewriter.hpp"
...
unique_ptr<Expression> TimeStampComparison::Apply(LogicalOperator &op, vector<reference<Expression>> &bindings,
	...
	if (ExpressionExecutor::TryEvaluateScalar(context, *cast_constant, result)) {
		...
		auto original_val = result.GetValue<sabot_sql::date_t>();
		...
		auto date_t_copy = result.GetValue<sabot_sql::date_t>();
