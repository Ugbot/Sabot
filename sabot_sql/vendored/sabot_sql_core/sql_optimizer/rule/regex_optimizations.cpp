...
static LikeString GetLikeStringEscaped(sabot_sql_re2::Regexp *regexp, bool contains = false) {
	D_ASSERT(regexp->op() == sabot_sql_re2::kRegexpLiteralString || regexp->op() == sabot_sql_re2::kRegexpLiteral);
	LikeString ret;

	if (regexp->parse_flags() & sabot_sql_re2::Regexp::FoldCase ||
	    !(regexp->parse_flags() & sabot_sql_re2::Regexp::OneLine)) {
	...
	if (regexp->op() == sabot_sql_re2::kRegexpLiteralString) {
...
static LikeString LikeMatchFromRegex(sabot_sql_re2::RE2 &pattern) {
	...
	while (cur_sub_index < num_subs) {
		...
		case sabot_sql_re2::kRegexpAnyChar:
			if (cur_sub_index == 0) {
				ret.like_string += "%";
			}
			ret.like_string += "_";
			...
			break;
		case sabot_sql_re2::kRegexpStar:
			// .* is a Star operator is a anyChar operator as a child.
			// any other child operator would represent a pattern LIKE cannot match.
			if (subs[cur_sub_index]->nsub() == 1 && subs[cur_sub_index]->sub()[0]->op() == sabot_sql_re2::kRegexpAnyChar) {
				ret.like_string += "%";
				break;
			}
			ret.exists = false;
			return ret;
		case sabot_sql_re2::kRegexpLiteralString:
		case sabot_sql_re2::kRegexpLiteral: {
		...
		case sabot_sql_re2::kRegexpEndText:
		case sabot_sql_re2::kRegexpEmptyMatch:
		case sabot_sql_re2::kRegexpBeginText: {
			break;
		}
		default:
...
unique_ptr<Expression> RegexOptimizationRule::Apply(LogicalOperator &op, vector<reference<Expression>> &bindings,
	...
	sabot_sql_re2::RE2::Options parsed_options = regexp_bind_data.options;
	...
	sabot_sql_re2::RE2 pattern(patt_str, parsed_options);
	if (!pattern.ok()) {
	...
	if (pattern.Regexp()->op() == sabot_sql_re2::kRegexpLiteralString ||
	    pattern.Regexp()->op() == sabot_sql_re2::kRegexpLiteral) {
		// convert to contains.
	...
	} else if (pattern.Regexp()->op() == sabot_sql_re2::kRegexpConcat) {
