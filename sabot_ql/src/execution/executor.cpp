...
arrow::Result<std::shared_ptr<arrow::Table>> QueryExecutor::Execute(
    ...
    SABOT_LOG_EXECUTOR("Execute: Starting");

    if (!root_operator) {
        SABOT_LOG_EXECUTOR("ERROR: root_operator is null!");
        return arrow::Status::Invalid("root_operator is null");
    }
    SABOT_LOG_EXECUTOR("Root operator type: " << root_operator->ToString());

    stats_ = QueryStats{};  // Reset statistics
    SABOT_LOG_EXECUTOR("Stats reset");

    auto start = std::chrono::high_resolution_clock::now();

    // Execute operator pipeline
    SABOT_LOG_EXECUTOR("Calling root_operator->GetAllResults()");
    ARROW_ASSIGN_OR_RAISE(auto result_table, root_operator->GetAllResults());
    SABOT_LOG_EXECUTOR("GetAllResults() returned");
